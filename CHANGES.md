...
**原行为与原因**：远程书此前只能用翻译面板，页内双语勾选框是隐藏的——双语需要改写一个真实的本地文件，而远程书是在线流式打开的，没有本地文件可改写。

**做了什么（用户视角）**：
1. **离线基底解析**：远程书打开后，应用本来就会把读过的内容缓存在本地（书架上的"已缓存 N%"徽标就是它的进度）。现在只要这本书已 100% 缓存（或用户下载过完整副本），双语引擎就从这份本地缓存组装出整本书文件，在其上完成双语改写——全程纯本地磁盘操作，不再依赖网络。
2. **入口放开**：远程书打开 AI 翻译对话框时，若这本书已 100% 缓存，"在页面内显示译文（双语对照）"勾选框正常显示；缓存未满 100% 时仍保持隐藏（与"已缓存 N%"徽标同一口径）。已支持格式中的 EPUB/HTML 远程书适用；TXT/FB2/MOBI 的远程书因离线转换链不含格式转换，保持仅面板翻译。
3. **缓存与版本一致**：组装的基底文件与块缓存同目录存放，服务器上的文件更新后缓存失效，双语版随之重建，不会读到旧内容。

**用起来的变化**：打开远程 EPUB 书（如 WebDAV 上的书）→ 稍等缓存到 100% → 菜单 → AI 翻译 → 勾选双语 → 开始：页面按"中文原文+高亮英文译文"逐段对照显示，翻页继续，与本地书体验一致；关闭双语后回到在线流式阅读。

**如何验证（P30 真机）**：
- 正例：WebDAV(HowRead-Test) 打开 book_epub.epub（184KB，秒级充满缓存）→ 双语勾选出现 → 开始双语 → 日志可见"从块缓存组装基底(18.8万字节)→ 枚举 778 段 → 注入 33 段 → 双语版换入"，页面中文/英文对照正常。
- 门控两态：88MB 的 book_bigepub.epub 打开瞬间（缓存 <100%）双语勾选隐藏；LAN 读取 14 秒读完（100%）后勾选出现。
- 回归：本地 EPUB 面板+双语 8 项全 PASS；全程 0 崩溃。

**内部佐证（辅助）**：新增 RemoteBilingualBase（完整副本 [tag 校验] 优先，其次按块位图从 data.bin 纯本地组装，cachedPercent>=100 与书架徽标同口径）；PdfContext.openTextDoc 增加远程分支并让 aiBilingualBook 的比较兼容 File 折叠路径（remote:// vs remote:/）；BilingualSession.attachForController 规范化路径并把远程书的会话书/缓存键指向本地基底；AiTranslateDialog.isBilingualFormat 远程判定改用 isRemotePathLoose（修复折叠路径漏判）。6 任务构建成功。
...
**原行为与问题**：格言文件的选择规则写反了——只有语言明确选"English"时才显示英文格言，选其它任何语言（法语、日语、韩语……乃至繁体中文）都显示中文格言。

**做了什么**：把选择规则调正——
- 语言选**简体中文或繁体中文**（以及"跟随系统"且系统语言是中文时）：显示中文格言（1041 条"原文 —— 出处"）；
- 选**其它任何语言**：显示英文格言（1042 条"原文 — Author, Work"）。
- 任一文件读取异常时自动回退到中文格言文件，保证侧边栏始终有格言显示。

**如何验证**（P30 真机实测）：简体中文下侧边栏显示"养心莫若寡欲，至乐无如读书 —— 戚继光"（中文）；切 English 显示英文格言（Henry James's…）；切 繁體中文 恢复中文格言（"旧书不厌百回读…… —— 北宋·苏轼"）；切回简体中文正常（"滥交朋友不如终日读书 —— 清·王永彬《围炉夜话》"）；全程 0 崩溃。

**内部佐证（辅助）**：MainTabs2.showRandomQuote 的语言判断由 `仅 en 用英文文件` 改为 `getAppLang() 以 zh 开头（zh-rCN/zh-rTW/系统 zh）用中文文件，否则英文文件`；格言文件本身（assets/reading_quotes.txt、reading_quotes_en.txt）未改动。6 任务构建全部成功。


## [2026-09-25] 修复：选择繁体中文时只有设置页是繁体、其余页面显示英文；所有语言全量补齐

**用户需求**：偏好设置中选"繁體中文"后，只有设置页面是繁体，其它页面显示英文；请分析原因，并保证选择其它语言时显示一致。

**原因分析（用户视角）**：切换语言后整个应用都用同一个语言资源显示，机制本身没有问题；问题出在翻译文件不全——
- 应用每次新增界面文字，只更新了"简体中文"翻译；"繁体中文"还停留在旧版（缺 176 条），其它 40 种语言更旧（各缺 300 余条）。
- 语言资源的回退规则是"缺了就显示英文（默认语言）"，而不是退回简体中文，所以选繁体时：设置页的旧文字（旧版就有翻译）显示繁体，首页/书库/我的文件等新界面文字全部回退英文。
- 附带发现：简体中文也缺 3 条（开发者名、GitHub 反馈链接、网址——这 3 条本就是品牌名/网址，英文显示无影响）。

**做了什么**：把所有语言的翻译一次性补齐到与界面文字完全一致——
1. **繁体中文**：以维护最全的简体中文为底稿，逐条转为繁体（专业简繁转换），并保留原有繁体措辞（共 989 条，与界面一一对应）。
2. **其它 40 种语言**（日/韩/德/法/俄/西/葡/阿/泰/越南等）：缺失的 300 余条新界面文字用 AI 大模型（智谱 glm-4-flash）批量翻译补齐；每条都经过程序校验（占位符 %1$s/%d 必须与原文一致、XML 标签不丢、转义正确），全部 0 失败。
3. 简体/繁体的 3 条品牌与链接类条目一并补上。

**用起来的变化**：现在选择任何语言，整个应用（首页、书库、我的文件、偏好、阅读器菜单、AI 功能等）都是同一种语言，不会再出现"半中文半英文"的混排。注意：其它语言的新增译文是 AI 翻译的，质量建议后续由熟悉该语言的用户抽查。

**如何验证**（P30 真机实测）：偏好 → 语言 → 繁體中文：首页（首頁/最近閱讀/書籤筆記/我的珍藏/閱讀統計）与设置页（書架設置/格式設置/備份配置）全部繁体；选 日本語：ホーム/マイファイル/設定 全部日语；选回 简体中文：一切如常；全程 0 崩溃。

**内部佐证（辅助）**：values-zh-rTW/strings.xml 以 values-zh-rCN 为底稿经 opencc s2tw 重建（814 条保留原措辞 + 172 条转换 + 3 条品牌）；其余 values-* 经 tmp/scripts/translate_missing2.py（键级缓存、并发、占位符/标签校验）各补 303 条；脚本与 JSON 缓存为一次性临时文件，不随代码入库。语言切换机制（MyContextWrapper/MainTabs2.attachBaseContext）未改动。6 任务构建全部成功。


## [2026-09-25] 安卓平台品牌名去空格：HowRead Pro→HowReadPro、好好读 Pro→好好读Pro

**用户需求**：安卓平台中文/英文品牌名由「好好读 Pro、HowRead Pro」改为「好好读Pro、HowReadPro」（去掉 Pro 前的空格）。仅安卓平台，鸿蒙暂未跟随。

**做了什么（用户视角）**：安卓渠道所有出现品牌显示名的地方统一去空格——
1. 手机桌面/系统设置里显示的应用名（pro、huawei 渠道，中英繁简全部语言）；
2. 应用内提到 Pro 版本的文案（fdroid 渠道的「升级去广告」介绍、问题反馈链接名、隐私政策链接名、关于页一处硬编码）——覆盖全部 40 余种语言的翻译；
3. 商店上架文案（fastlane 英文简介 2 处）；
4. 品牌速查文档 BRAND.md（安卓条目更新 + 补记此前遗漏的 huawei 渠道行；鸿蒙条目保持带空格并注明差异）与工作区 AGENTS.md 的安卓显示名提及。
- **保持不变**：fdroid 渠道显示名（HowRead Fd / 好好读 Fd，本就不含 Pro）；APK 产物文件名 `HowRead-Pro-v…` 的连字符写法是文件命名约定而非显示名（自动测试按文件名匹配产物，依赖它）。

**用起来的变化**：桌面图标名称与系统设置里显示为「好好读Pro / HowReadPro」（无空格）；应用内提及 Pro 的文案同步更新。纯显示名变化，不影响任何功能与既有数据。

**如何验证**：编译服务器标准批次 6 包（pro/fdroid/huawei × debug/release）构建成功；aapt2 检查三个 arm64 debug 包 application-label：pro/huawei 全语言为 HowReadPro（zh-CN 好好读Pro、zh-TW 好好讀Pro），fdroid 保持 HowRead Fd；MI9 安装 pro debug 包启动正常、0 崩溃。

**内部佐证（辅助）**：共 102 处替换、55 个文件（6 处 flavor app_name、build.gradle 2 处 appName 占位符、main 源集 config.xml/strings.xml/布局共 88 处含 40 个语言目录、fastlane 2 处、BRAND.md/AGENTS.md 文档）。版本号不变。

## [2026-09-25] 我的文件·书库文件夹行增加操作按钮（与远程服务器条目一致）

**用户需求**：「我的文件」页面里书库文件夹的文件夹路径条目，也要有和 WebDAV 条目一样的按钮（扫描添加书籍、编辑、删除）。

**做了什么（用户视角）**：「我的文件」→「书库文件夹」的每一行右侧新增三个操作按钮，图标和排布与 WebDAV/SMB/SFTP 条目完全一致：
1. **扫描入库**（放大镜图标）：把"这个文件夹里的书"加入书库——只扫这一个文件夹，速度快，不影响其他书；已经在书库里的书不会被重复添加、原有信息（封面/进度等）保持不变。
2. **编辑**（笔图标）：为该条目重新选择一个文件夹路径，旧路径被替换（同步配置不会把旧路径带回来）。
3. **删除**（✕图标）：从书库列表移除该文件夹——磁盘上的文件夹和书不会被删，移除前弹确认框。原来的长按删除保留，点行进入文件夹浏览的行为不变。

**用起来的变化**：以前删除入口藏在长按里不好发现，也没有"单独补扫某个文件夹"和"改路径"的入口；现在把新书放进常用文件夹后，不用全库扫描、点该行扫描图标书就进书库，并且弹提示"请稍候…"开始、"已将 N 本书加入书库"结束。

**如何验证**：我的文件 → 书库文件夹，每行右侧应有 3 个图标（与上方 WebDAV 行同款）；放一本新书到 Download 后点该行扫描图标 → 书库出现该书；再点一次扫描 → 数量不变（不重复）；点 ✕ → 弹出"从书库列表中移除该文件夹？磁盘上的文件夹不会被删除"。

**MI9 真机实测**：扫描 Download 文件夹，书籍总数 18→43（含测试书 2 本）；删除测试书文件后全库扫描 43→41（精确减 2，测试书已入库过的证明）；对已扫过的文件夹重复点扫描 41→41（无重复条目）；删除确认框、编辑路径选择器均正常；全程 0 崩溃。

**内部佐证（辅助）**：BrowseFragment2.java（netListItem 六参数行构建复用 + 新增 removeLibraryFolder / editLibraryFolder / scanSingleFolder 三个方法）；values/strings.xml、values-zh-rCN/strings.xml（新增 moon_folder_scan_added 提示文案）。6 任务构建（pro/fdroid/huawei × debug/release）全部成功。


## [2026-09-25] 鸿蒙 0.9.12：品牌名去空格 + 侧边栏格言语言跟随 + 书库文件夹行操作按钮（同步安卓 09-25 三批改动）

**做了什么（用户视角）**

对照安卓 09-25 当天在 0.9.11 同步基线之后的三批改动，逐项分析鸿蒙版并移植：

1. **品牌名去空格（安卓同日同款）**：手机桌面/系统设置里鸿蒙版应用名由「HowRead Pro / 好好读 Pro」改为「HowReadPro / 好好读Pro」，与安卓一致；应用内"关于"页的隐私政策/问题反馈链接名、官网行品牌文字同步去空格；品牌速查文档 BRAND.md 鸿蒙条目同步更新（此前条目注明"鸿蒙暂未跟随"，现已跟随）。顺带修复：构建换名脚本此前不处理 en_US 目录，英文系统下 Pro 包桌面名会漏掉 Pro 字样，现已覆盖。APK/HAP 产物文件名 HowRead-Pro-v… 的连字符写法不变（文件命名约定，自动测试依赖它）。
2. **侧边栏格言语言跟随应用语言（安卓 09-25 同款规则）**：语言选中文时侧边栏显示中文格言（如"书是进步的阶梯，终生的伴侣 —— 高尔基"），选 English 或其它语言显示英文格言（新增内置英文格言库 1042 条）；"跟随系统"时按系统语言；格言文件读取异常时自动回退中文库，侧边栏始终有格言。此前鸿蒙格言只有中文一个池、任何语言都显示中文。
3. **我的文件·书库文件夹行增加操作按钮（安卓 09-25 同款）**：每一行右侧新增"扫描入库（放大镜）/编辑（笔）/删除（✕）"三个按钮，与 WebDAV/SMB/SFTP 条目一致——点扫描只扫这个文件夹并把新书加入书库（已在库的书不重复导入，提示"已将 N 本书加入书库"）；点编辑可为该条目重新选择文件夹路径（选择器标题"选择新的文件夹路径"）；点删除先弹确认框（"磁盘上的文件夹和书不会被删除"），确认后才从列表移除。此前删除入口只有行内 ✕ 直接删、没有确认，也没有单独补扫和改路径的入口。
4. **不涉及鸿蒙（分析后备案）**：安卓"繁体中文只翻了一半/40 种语言补齐"的修复——鸿蒙界面资源只有中文+英文两套、语言选项也只有中文/English/跟随系统三项，不存在"缺条目回退英文造成混排"的问题。

**如何验证**（Pura 90 模拟器，HowRead Pro v0.9.12 debug x86_64 实测）：
- 品牌名：安装后桌面图标名称显示"好好读Pro"（无空格）；构建日志确认 base/en_US/zh_CN 三处换名全部输出 HowReadPro/好好读Pro。
- 格言：应用语言 English 时抽屉显示英文格言（Olga Tokarczuk…），切"中文"后抽屉显示中文格言（"书是进步的阶梯，终生的伴侣 —— 高尔基"），日志 quote lang=zh。
- 文件夹行：我的文件 → 书库文件夹，每行右侧 3 图标；点扫描提示"请稍候…"并结束于"已将 0 本书加入书库"（6 个文件全部已在库、零重复导入，日志 folder_scan_done）；点 ✕ 弹出确认框（文案含"磁盘上的文件夹和书不会被删除"），取消不删；点编辑弹出"选择新的文件夹路径"选择器。
- 全量回归：`python ci/autotest-hmos/run_all.py` L0+L1+L2 三层门禁 28/28 PASS（对 v0.9.12 最终包）。

**内部佐证（辅助）**：build_hap_all.sh（换名表补 en_US + 存在性判断 + import os + 恢复分支）；Index.ets（pickRandomQuote 语言规则改读 i18n 应用首选语言并回退系统语言、quotePoolEn 英文池、browseFolderRow 三按钮、scanSingleFolder/editLibFolder/confirmRemoveLibFolder、目录选择器 libFolderEdit 分支与标题）；LibrarySearch.ets（scanFolderBooks 单文件夹递归扫描）；rawfile 新增 reading_quotes_en.txt；string.json base/zh_CN 新增 folder_scan_wait/folder_scan_added/folder_remove_confirm/folder_remove_ok 四键；AppScope/app.json5（50/0.9.12）。

---

## [2026-09-25] 鸿蒙 0.9.11：同步安卓 AI 翻译提速与界面优化（流式出字、3 路并行、thinking 自适应、FAB 可拖动、关于页行式改版）

**做了什么（用户视角）**

对照安卓 9-20 之后的三批改动（AI 翻译提速、界面调整），逐项分析鸿蒙版并移植适用部分：

1. **AI 翻译"边译边出字"（安卓 9-24 同款）**：翻译面板与"发送给AI"的回答现在流式上屏——模型一边生成、屏幕一边逐字增长，不再干等整段译完；同时做了两层兼容兜底：不支持流式的服务器自动回退为整段方式，对返回非流式 JSON 的服务器也能正确解析显示。
2. **批量翻译 3 路并行（安卓 9-24 同款）**：页内双语模式的批量请求从"一批译完再下一批"改为 3 路并发推进，长书整本翻译明显提速。
3. **GLM thinking 参数自适应（安卓 9-25 同款）**：部分模型/服务会拒绝思考开关参数（如 glm-5.x 返回 400）——现在遇到一次 400 后自动记住该"模型+端点"组合（持久保存），之后所有请求自动跳过该参数，不再浪费失败往返；对智谱端点仍按官方格式正常下发思考开关。
4. **修复双语翻译重复构建失败（鸿蒙自有缺陷）**：同一本书第二次构建双语版会失败（日志报 File exists）——缓存目录/产物文件已存在时鸿蒙文件接口会抛错；已按"存在即先清理"修复。现在双语版可反复构建/还原，且第二次起全部命中翻译缓存（实测二次构建零网络请求、秒级完成）。
5. **"继续阅读"悬浮球可拖动（安卓 9-24 同款）**：FAB 可拖到屏幕任意位置（自动限制不出屏），轻点仍打开最近一本书；拖动位置只在当次使用内保留，重开应用回到默认右下角（与安卓行为一致）。
6. **关于页对齐安卓 9-24 的行式布局**：软件说明改为安卓同款"标签+链接"行式结构：开发者（LeeStudio）/更新日志/隐私政策（好好读Pro 隐私政策）/许可/问题和反馈（GitHub）/邮箱/官网。安卓的"高级功能"行因鸿蒙版无内购、无功能锁不适用，未移植。

**分析后不涉及鸿蒙的安卓改动（备案）**：① 安卓 9-20/9-22 的"同步删除复活/编辑回退"系列修复——鸿蒙同步只覆盖每本书的进度+书签（三路合并+删除自动传播内建），不存在安卓侧的配置同步面与服务器多副本前提；② 安卓 9-25 双语的"工作副本指纹/合并节流"修复——鸿蒙双语是"整本解包→注入→重打包"的一次性架构，无工作副本会话；③ 华为渠道系列改动不适用。

**测试工具升级**：ci/autotest-hmos 的本地 AI mock（tools/ai_mock.py）新增 SSE 流式回复（4 片分段推送）与"编号段落"批量翻译回复（按请求编号逐段作答），使流式显示、分段注入、缓存复用都能被自动化端到端验证。

**如何验证**（Pura 90 模拟器，HowRead Pro v0.9.11 debug x86_64 实测）：
- 流式：阅读器 → AI 翻译 → 开始翻译：译文逐字增长（mock 流式 3.2 秒分段推送；翻译中快照可见"缺尾段"的中间态，结束后完整）。
- 并行：双语构建时同一毫秒并发发出 3 个翻译请求，工人池滚动补位（40 段 8 批全部完成）。
- 注入与缓存：双语版页数 3→5（译文行真实插入）；同一本书第二次构建零网络请求、秒级完成。
- FAB：拖动后位置随手势移动且不出屏；重开应用回默认右下；轻点打开最近一本书。
- 关于页：七行齐备、链接可点、中英文案随语言切换。
- 全量回归：`python ci/autotest-hmos/run_all.py` L0+L1+L2 三层门禁 28/28 全 PASS（含 monkey 300 秒 0 崩溃；对最终修复包复跑）。

**内部佐证（辅助）**：AiClient.ets（chatCompletionStream：requestInStream+SSE 行解析+utf-8 安全分块+watchdog+isCancelled；thinking 学习跳过持久化 no_thinking；非流式 400 一次重试）、Bilingual.ets（3 工人并行池；ensureDir/removeIfExists 修 EEXIST——本镜像 mkdirSync(recursion) 与 CREATE|TRUNC 打开已存在文件均抛错）、Reader.ets（流式面板+transToken 世代守卫）、AiChat.ets（流式回答）、Index.ets（FAB PanGesture+clamp、aboutRow 行式关于页、PRIVACY_URL/FEEDBACK_URL）、string.json base/zh_CN（7 个关于页新键）、AppScope/app.json5（49/0.9.11）、ci/autotest-hmos/tools/ai_mock.py（SSE+编号段落）。

## [2026-09-25] 列表翻译面板：每段译文配对显示原文首行（原文-译文一眼对应）

**用户需求**：页面翻译（非页内双语）时，希望原文和译文对齐——至少首段能对上。先实现过"译文卡片浮在原文段落上方"的方案，实测反馈：卡片盖住原文、看不清，改用配对方案。

**最终形态**：面板回到底部列表，但每个译文块**先显示一段灰色斜体的原文首行（▎标记），紧接其下是译文**——段与段一一对应、原文与译文首尾相接，一眼即知哪段译文对应哪段原文；页面原文完全不被遮挡。面板标题仍带页码（「AI翻译 → 中文 · 第 48 页」）并跟随翻页切换，后续页在后台翻译（底部「后台翻译中 N 段…」），翻译中的段落实时逐字增长。

**实现与实测**：新增配对块布局（原文首行+译文）；面板渲染按页配对输出。MI9 真机实测（水平翻页、智谱 glm-4-flash）：翻译开始后本页段落逐个完成，面板内灰字原文首行与中文译文成对显示；翻页后面板切换新页并同步跟随；logcat 0 FATAL。此外为对齐能力打下的基础（段落首行在页面中的纵坐标定位，MuPdfPage 字符几何 + DocumentController.getParagraphTops）保留在代码中，供后续需要时使用。

**如何验证**：阅读器（水平翻页模式）→ 工具栏 AI 图标 → 不勾选双语 → 开始翻译：底部面板每段先灰字原文首行、再中文译文；翻页后面板切换并跟随。

**涉及文件**（供佐证）：TranslatePanel.java（配对渲染）、res/layout/ai_translate_pair.xml（新：原文首行+译文配对块）、TranslateSession.java/DocumentController.java/HorizontalModeController.java/MuPdfPage.java（段落纵坐标定位基础，当前面板未使用浮层）。


## [2026-09-25] 修复：双语"翻译进度提示消失了，页面上却没有译文"的问题

**用户反馈**：启动页内双语翻译后，"正在翻译中…"的进度提示走完了、消失了，当前页面却没有出现双语翻译结果。

**原因（日志+界面取证）**：三个叠加问题——
1. **段落指纹基准不一致（主因）**：翻译会话枚举"书内段落"用的文件，与构建双语版用的文件可能不同（阅读器打开的"工作副本"被重排过、且打开路径与会话之间存在时序竞争），两边指纹对不上 → 构建双语版时"注入 0 条译文"，提示却按"已并入"消失。
2. **合并饥饿**：译文流式逐段落位期间，页面合并的 3 秒倒计时被不断重置，合并被无限推迟（实测拖延 50 秒以上）。
3. **失败段被提示隐藏**：个别段落翻译失败（如网络限流）后，进度提示把它们排除在计数外，提示提前消失而页面仍是原文。

**做了什么**：
1. **打开即发布基准**：文档打开时同步"发布"实际打开的文件路径；翻译会话枚举段落前**等待该发布**（最多 8 秒）——从此会话枚举与双语构建永远用同一个文件，指纹必然一致。
2. 枚举段落时**跳过此前注入的中文对照段**（防止二次枚举把旧译文当成原文）。
3. 合并改为"首个落位后 3 秒内必合并 + 最快每 8 秒一次"（防饥饿且不频繁跳动），并用"本页落位计数"直接驱动合并。
4. 进度提示**如实显示失败段**（「本页剩 2 段（1 段失败，翻页后自动重试）」），翻页回到该页时自动重试失败段落。

**实测效果**（MI9 真机、清空全部缓存后从头启用双语）：进度提示约 22 秒消失，**消失时当前页已显示完整双语对照**（英文原文+黄底中文逐段对应，双语版页码 141→133 正常）；后续段落持续在后台补全。全程无崩溃。

**如何验证**：双语模式下启用翻译，等"正在翻译中…"提示消失——此时当前页应已出现逐段中文对照；若个别段失败，提示会明确标出，翻页回来即自动重试。

**涉及文件**（供佐证）：BilingualBuilder.java（发布打开版本接口 + 枚举跳过双语段）、PdfContext.java（打开时发布）、BilingualSession.java（等待发布、合并节流/防饥饿/落位计数、提示含失败段）。


## [2026-09-25] AI 翻译两项升级：页内双语提速贯通、翻译面板改为跟随当前页

**用户反馈的两个问题**：①阅读界面选"页内双语"翻译时速度还是很慢；②列表式 AI 翻译的结果一直刷新追加，和当前阅读的页面对不上。

**做了什么**：

1. **修复双语模式的一个底层缺陷（关键）**：构建"双语对照版"时，段落指纹（md5）按"打开副本"计算、而译文缓存按"原书"计算——某些书的副本被阅读器重排过（词里混入软连字符符号），两边指纹永远对不上，导致**翻译完成了却一个字也注入不了双语版**（日志 injected=0）。现在构建时统一剥离软连字符再算指纹，译文的缓存与双语版彻底对上。
2. **双语翻译改"流式+逐段落位"**：一批 5 段的翻译请求不再等整批完成，模型每译完一段（出现下一个编号标记）就立刻写入缓存并刷新页面——第一段译文约 5~8 秒即可上屏，其余段落每隔几秒陆续出现。
3. **双语重建节流与防饥饿**：译文流式落位期间，页面合并（重建双语版并原位换装）从"每次落位都重置 3 秒倒计时"改为"首个落位后 3 秒必合并"，并且**最快每 8 秒合并一次**——既保证本页译文尽快显示，又不会频繁跳动刷新；同时用"本页落位计数"直接驱动合并，不再依赖容易误判的匹配检查。
4. **本页段落优先**：翻到新页时先做"页面文本↔书内段落"匹配、再排翻译队列，保证当前页的段落最先翻译；列表翻译的滚动窗口同理，远离窗口的未开始段落自动放弃（省流量）。
5. **列表翻译面板改为"跟随当前页"（按用户方案）**：面板标题带页码（如「AI翻译 → 中文 · 第 26 页」），**只显示当前阅读页的译文**；后续页的翻译在后台继续，面板底部一行小字显示「后台翻译中 N 段…」；**往下翻页时面板立即切换为新页**——已译好的段落秒出、正在译的实时增长、还没轮到的显示占位。翻页时新页段落自动入队，后台译文"等你翻到时就绪"。
6. **兼容性**：部分模型/服务会拒绝思考开关参数（如 glm-5.x 返回 400）——现在记住这类"模型+端点"组合，后续请求自动跳过该参数，不再浪费一次失败往返。

**实测效果**（MI9 真机 + 智谱 glm-4-flash）：
- 双语模式：之前一批 5 段要等 16~30 秒才统一上屏（且实测中曾出现"译完也注入不了"）；现在**第一段译文流式落位（约 5 秒）、页面按 8 秒节奏逐步补全中文对照**，双语版换装后段落一一对应、黄底高亮清晰。
- 列表面板：打开翻译立即显示「第 26 页 · 本页 2 段 + 后台翻译中 4 段」，往下翻页标题即切「第 27 页」并显示该页译文，新页段落自动开始翻译；全程 logcat 无崩溃（0 FATAL）。
- 生成速度地板说明：glm-4-flash 约 25 字/秒是外部因素；整页 5 段全部译完仍需约 20~30 秒，但**用户不再需要干等——逐段逐步看到结果**。glm-5.x 系列为付费模型（当前测试账号无资源包），无法对比速度。

**如何验证**：阅读器 → 工具栏 AI 图标 → 勾选"在页面内显示译文（双语对照）"→ 开始翻译：约 5~10 秒可见首段中文，页面每 8 秒左右补全一批；不勾选则走列表面板：面板只显示当前页、带页码标题，往下翻页面板跟着切页。设置 → AI 大模型的"测试连接"、"发送给AI"（逐字增长）、翻译缓存复用均正常。

**涉及文件**（供佐证）：`com/foobnix/ai/` 下 BilingualSession.java（流式逐段落位、重建节流/防饥饿、本页优先）、BilingualBuilder.java（软连字符指纹修复）、TranslateSession.java（新：按页组织+3路并行+滚动窗口）、TranslatePanel.java（按页渲染）、AiTranslateDialog.java、AiClient.java（thinking 参数学习跳过）、DocumentWrapperUI.java / HorizontalViewActivity.java（各一行翻页挂钩）。


## [2026-09-24] 提速：AI 翻译"边译边出字"，整窗出全时间约缩短到原来的 1/3

**用户的问题**：真机使用 AI 翻译时响应很慢——点了"开始翻译"要干等很久才见到第一个字，长段落尤其明显。

**原因（实测定位）**：①译文必须等模型把整段全部生成完才一次性显示（非流式），慢在"看不到过程"；②翻译面板对窗口内多段译文逐段排队请求，一段译完才开始下一段；③选了会"思考"的 GLM 模型（glm-4.5 系）时，发送的思考开关服务端不认识，模型照常先思考十几秒再作答（智谱官方参数实测 7.5s → 关掉后 1.6s）。

**做了什么**：
1. **流式输出**：模型一边生成、译文一边上屏。翻译面板每段先出现占位块并逐字增长；选中文本的"发送给AI"回答同样逐字增长。
2. **3 路并行翻译**：面板的多段译文同时发 3 个请求（与页内双语模式同规格），结果严格按原文顺序显示，不会乱序。
3. **适配 GLM 思考开关**：对智谱等 OpenAI 兼容端点按官方格式真正下发"思考开/关"，不再让思考型模型白等。
4. **兼容性兜底**：遇到不支持流式或扩展参数的服务器自动回退为原来的整段方式（400 自动重试一次），保证"以前能用的服务器现在照样能用"；页内双语模式保持原有批量方式不变。
5. **取消更干脆**：关闭翻译面板立即中断进行中的请求，不再后台白跑耗流量。

**实测效果**（MI9 真机 + 智谱 glm-4-flash，同一本书同一位置 5 个段落窗口，优化前后对比）：
- 首字出现：从"等第一段整段译完（约 4 秒）"提前到 **约 1~3 秒即见文字逐字增长**（3 秒时已有 4 个段落块在显示）；
- 整窗出全：**54.8 秒 → 20.5 秒（约 2.7 倍）**，两轮复测结果一致；段落越多提速越明显；
- "发送给AI"：3 秒内即可读到增长中的回答（旧版此时只有"思考中"）；
- 全程 logcat 无任何崩溃（0 FATAL）。

**如何验证**：阅读器 → 工具栏 AI 图标 → 选目标语言 → 开始翻译：面板立即出现段落块并逐字增长、多段同时推进、顺序正确；同一本书隔天再译照样正常。设置 → AI 大模型的"测试连接"、页内双语模式、发送给AI 行为均正常。

**涉及文件**（供佐证）：`com/foobnix/ai/` 下 AiClient.java（SSE 流式解析+回退+GLM thinking 参数）、AiTranslator.java（3 路并行+有序输出）、TranslatePanel.java（直播块）、AiTranslateDialog.java、AiAskDialog.java（增量显示）。已知边界：翻译面板的本地缓存沿用原有"会话内复用"设计（跨会话是否落盘跟随页内双语模式，本次未改动）。

## [2026-09-23] 鸿蒙自动测试达成 28/28 全通过；修复阅读器对话框被手势区挡住的真实交互缺陷

**做了什么**：

1. **【应用缺陷修复，所有用户受益】阅读器跳页/密码等对话框的按钮点不到**——09-19 单击双击重构引入的全屏点触区/捏合手势层在命中测试上压住了后弹出的对话框：点"跳转"实际触发的是菜单开合，输入框也无法聚焦输入（自动化截图与点击日志证实）。修复：当任一对话框/面板（跳页、密码、目录、书签、缩略图、AI 翻译、设置抽屉等）打开时，三块点触区与捏合手势层自动放弃触摸响应（hitTestBehavior=None），关闭后恢复。修复前真实用户在跳页对话框里同样点不了"Go"。
2. **自动化用例修复 8 项**（ci/autotest-hmos）：语言守卫加固与文案断言双语化（语言切换失败会跨运行残留英文界面）；顶栏图标映射按现有 9 图标实测校正（书签切换在底栏）；对话框表单填写先收键盘再点按钮；输入框先清空再输入（地址框自带 http:// 前缀、端口框默认 22，追加输入会变 http://http://… 和 2222）；对话框提示文案中英双语依次尝试；SFTP 首连自动勾选信任主机密钥；WebDAV 的"添加"按钮按 Button 类型精确定位（避免点到区块头的"添加"）；语言切换后等待偏好落盘再重启。

**用起来的变化**：阅读器里跳页对话框、密码对话框等现在可以正常点击按钮和输入（此前按钮点击会误触翻页菜单）；自动化回归全绿后，上述交互有 18 个功能用例 + 7 个冒烟 + 3 个专项持续守护。

**如何验证**（Pura 90 模拟器）：`python ci/autotest-hmos/run_all.py --level L0 / L1 / L2` 三层门禁全部 PASS——L0 7/7、L1 18/18（含跳页、夜间、全文搜索、书签、WebDAV/SMB/SFTP、远程在线阅读、AI 配置、语言切换）、L2 3/3（冷启动中位约 300ms、翻页内存增长 +1.6%、monkey 271 事件零崩溃）。随后连续 3 轮全量浸泡回归 84/84 全 PASS、无任何波动（首轮浸泡曾暴露中文界面下 WebDAV 对话框"添加"按钮未被双语匹配的问题，已补上——多轮回归正是为了把"过一次"沉淀为"每次都过"）。


## [2026-09-22] 鸿蒙自动化测试：修复 5 个网络用例，提升用例健壮性

**做了什么**：测试服务器（50.23）恢复后，WebDAV/SMB/SFTP 相关的 5 个自动化用例（FN-12~16）仍失败，逐个取证（应用界面转储 + 服务器访问日志）定位出三个用例自身的问题并修复：

1. **软键盘遮挡按钮**：自动填完表单后软键盘仍弹着，盖住对话框底部的"测试连接/添加"按钮，点击全部落在键盘上（服务器日志显示请求根本没发出）。现在填完先收键盘再点按钮。
2. **输入框"追加"而不是"替换"**：应用的部分输入框自带初始值——WebDAV 地址框自带 http:// 前缀、SFTP 端口框默认 22；自动化往里输入后变成了 `http://http://…` 和 `2222`，连接必然失败。现在输入前先清空原内容。
3. **SFTP 首次连接需信任主机密钥**：用例现在自动勾选"信任任意主机密钥"后再连。
4. **远程在线阅读用例换测试文件**：产品设计中 txt/html 等小文件是"静默下载后本地打开"，只有 pdf/epub/cbz/xps 才走在线流式阅读；原用例用 txt 文件断言"在线打开"与设计不符，改用 EPUB 验证在线流式。

**结果**：FN-12（WebDAV 测试连接）、FN-13（WebDAV 保存并浏览）、FN-14（SMB 测试连接）、FN-15（SFTP 测试连接）、FN-16（远程书在线打开）全部通过；L1 通过数从 8 提升到 13/18。



## [2026-09-22] 修复：删除书签后仍被同步"复活"的问题（三个根因）

**现象**：真机上删除书签（或 AI 笔记）后，等一次同步或重启应用，被删的条目又回来了。用户在多设备 WebDAV 同步环境实测可复现。

**做了什么**：排查书签删除→同步的完整链路，确认主流程本身闭环，但存在三个复活缺口，全部修复：

1. **同名旧副本复活（主因，真机同步库中实际存在此形态）**：服务器按"书文件内容指纹"保存书籍信息且旧副本永不清理。同一本书只要换过文件版本（或多台设备上的副本字节不同），服务器上就有多份同名副本；此前删除书签只清理"当前匹配"的那一份就立刻销毁删除记录，其余旧副本里的书签在下一轮同步原样合并回来。现在删除会把该书名的**所有**服务器副本一并清理并携带删除记录，全部确认后才销毁本地删除标记。
2. **连续快速删除被整轮吞掉**：删除一个书签会触发同步，若紧接着（同步进行中的几秒内）再删一个，第二个删除会被当轮快照重新传回服务器、且其删除记录被误销毁，从此永久失效。现在轮内新发生的删除会被保护，本轮结束自动追加一轮快速同步把剩余删除收敛完毕。
3. **在线阅读的书删除永不生效**：未下载到本机的书（在线阅读）无法向服务器发布删除，删除标记 30 天过期后书签复活。现在即使本机没有书文件，也能把删除推送到服务器副本上。

顺带两项加固：同步服务器列表不完整或某份副本暂时不可读时，不再销毁删除标记（防止误消费导致下次复活）；"清除全部书签"现在也会携带逐条删除记录（否则另一台还留着这些书签的设备会把它们重新传回来）。

**用起来的变化**：删除的书签/AI 笔记在任何场景（多副本、多设备、在线书、快速连续删除）都保持已删除；书签新增、阅读进度同步不受影响。

**如何验证**（MI9 真机 + 50.23 同步服务器，Pro v1.3.13 debug arm64）：① 陈旧副本场景——服务器手工放置一份同书名旧副本（含待删书签、无删除记录）→ 阅读器内删除该书签 → 等同步轮：本地、匹配副本、旧副本三处均已删除且均携带删除记录，重启应用再等一轮仍不复活；② 连删场景——删除书签 3 秒后（同步轮进行中）再删一条，两轮后两处均保持删除；③ 回归——单删、保留其余书签、阅读进度同步全部正常；④ 全程 logcat 无 FATAL。

# [2026-09-22] 修复鸿蒙阅读器两个崩溃/卡死问题（自动测试 CI 发现）；并纠正测试框架的内存测量

**做了什么**：针对鸿蒙自动测试 CI 首轮运行发现的问题，定位根因并修复：

1. **阅读约 30 秒后闪退（原生崩溃 SIGSEGV）**——根因：退出阅读器时，还在排队中的页面渲染任务会走到一条"文档已关闭"的快速失败路径，这条路径误用了 MuPDF 引擎的异常清理机制（没有先压栈就执行弹栈），把引擎内部状态指针踩坏，下一个用到它的任务直接崩溃。修复：文档已关闭的排队任务干净退出，不再触碰异常栈。
2. **大 PDF（2600 页）快速连续翻页时应用卡死**（系统判定无响应并杀进程，之后打开任何书都是空白 0/0 页）——根因两层：每次翻页的渲染任务独占渲染锁直到整页画完，快速翻页时最多约 10 个大页渲染排队，锁被占十几秒；而翻页时界面线程又要同步查询当前页批注、TTS 取文本，只能排在渲染队列后面，等锁超过 6 秒即被判定卡死。修复：渲染队列最多保留 2 个任务，被更新的翻页动作取代的旧任务立即取消（界面层静默重试当前页）；翻页热路径上的批注查询与取文本全部改为后台异步。"大书之后空白阅读器"是进程被杀的连带现象，冻结修复后自然消失。
3. **内存报告复核：此前的"+72.4%"是测试框架测量失真，真实无泄漏**——自动化框架统计进程内存时把系统报表的明细行重复求和（总行 + 子堆行被算了两遍以上），数值虚高一倍以上。用系统真实口径复测：开书瞬间工作集 +58%（阅读引擎字体/布局的固有开销，到达后立即平台期），**翻页阶段零增长（实测 +0.1%）**，退出后不持续攀升。仍落地两项改进：① 引擎每文档缓存池上限从 256MB 收紧到 48MB（防止极端大文档时缓存失控，正常阅读无感）；② 测试框架内存测量改为读取系统报表的真实 Total 行，泄漏门禁改为盯"翻页阶段是否增长"（查泄漏的本意），开书工作集作为基线数据记录、不做门禁。

**用起来的变化**：长时间读书不再闪退；大 PDF 快速翻页不卡死，退出后马上再开别的书也正常；后台内存占用有上限约束（极端大文件连续翻页偶尔多一次重绘，肉眼基本无感）。

**如何验证**（Pura 90 模拟器，Pro v0.9.10 debug x86_64）：回归脚本 T1=开书-音量翻页 30 次-退出，循环 5 轮，全程零新崩溃记录、进程号不变（修复前必现 cppcrash，faultlogger 20260921-010408）；T2=大书快速翻页 12 次后进程存活无 appfreeze，退出立即重开正常显示 105/105（修复前必现 appfreeze 并被系统杀进程，faultlogger 20260921-003445）；正式门禁 `python ci/autotest-hmos/run_all.py --level L2 --cases PF-01,PF-02` PASS（冷启动中位 520ms；内存 home 154MB → 开书 245MB → 翻页后 245MB，翻页增长 +0.1%）。



## [2026-09-20] 修复：WebDAV 配置同步下"删除复活/编辑被回退"的同类问题（8 处）

**背景**：上一条修复了"OPDS 目录删除后复活"。以此为线索对全部同步配置做了系统审计，发现并修复了 8 处同类问题，分两类：

**A 类：删除后同步复活（本地删除标记"墓碑"缺失或未清理）**
1. **最近阅读移除/清空**：从最近阅读移除一本书、或"清空"全部最近阅读，开启同步的设备上下一轮同步会全部回来。现在删除时会写墓碑，同步后保持删除。
2. **取消收藏/清空珍藏**：同上，取消收藏的书不再被同步带回。
3. **AI 厂商配置删除**：设置→AI 大模型里删除一个厂商配置后，不再被同步加回来。
4. **偏好等处的"书库文件夹"对话框删除文件夹**：与"我的文件"里删除同样不再复活。
5. **WebDAV 服务器编辑时改 URL**：改完保存后，旧地址的服务器不会以重复条目的形式"复活"回来。

**B 类：本地编辑被服务器旧副本静默回退（合并升级为带基准快照的三路合并）**
6. **AI 厂商配置编辑**：修改厂商的模型/接口地址/切换激活后，不再被同步悄悄改回旧值。
7. **"阅读中"标记可以清掉了**：一本书标过"阅读中"后，再标"已读/未读"会真正生效，不再在同步后自动变回"阅读中"。
8. **改 URL / 改主机不再产生"幽灵重复"**：编辑 WebDAV 服务器地址或 SMB/SFTP 的主机端口后，编辑前的旧条目不会再被同步加回来变成重复项。

**配套改动**：重新添加一个删除过的目录/文件夹/厂商时会自动清除对应墓碑（删了再加不会被误吞）；最近/收藏的墓碑容量上限 512 条。

**已确认无问题的**：书签与阅读进度的删除（本就有完善的删除标记机制）、"我的文件"里直接删除 WebDAV/SMB/SFTP 服务器与书库文件夹（墓碑本就齐全）。

**怎么验证**：MI9 真机 + 50.23 同步服务器实测——清空最近阅读（31 条全部写墓碑、同步后保持为空）、AI 厂商删除与编辑、对"阅读中"的书改标记、WebDAV 改 URL、删除书库文件夹，逐项操作后等同步轮并重启应用，全部保持预期状态，无崩溃。

## [2026-09-21] 鸿蒙自动测试 CI 建成（ci/autotest-hmos）+ 应用侧 HRTEST 打桩日志；实测发现 2 个稳定性问题

**做了什么（用户视角）**：
给鸿蒙版 HowRead Pro 建了一套与安卓平行的自动测试 CI（`ci/autotest-hmos/`，按平台区分）：模拟器/真机统一接入，覆盖功能、界面 UI、性能（冷启动耗时/开书内存）与稳定性（随机 monkey），配套完整取证（截图/UI 树/日志按用例归档、失败自动三件套）与 Markdown 报告 + 门禁退出码。为让测试能精确判定"事件何时发生、耗时多少"，应用内新增了统一的 **HRTEST 打桩日志**（30+ 关键节点：冷启动就绪/书库扫描/开书完成/首页渲染/翻页/目录/书签/搜索/WebDAV 连接与下载/AI 请求/设置保存/退出确认，每条自带毫秒时间戳，debug/release 包均保留），并给底部 tab、退出按钮、跳页输入等关键控件补了稳定 id。测试书投放走新增的 `hrSeedUrl` 参数（应用自己从局域网下载入沙箱并入库，规避了系统限制——外部无法直写应用沙箱）。

**实测结果（Pura 90 模拟器，v0.9.10 debug x86_64）**：
- L0 冒烟 7 条**两轮全绿**（安装/冷启动/四 tab/开书翻页/退出确认/重装自愈/取证链路）；
- L1 功能 18 条中 8 条通过（开书事件链/音量翻页/目录/字号/阅读统计/文件浏览/收藏/语言切换），其余受阻于下述应用稳定性问题或对话框细节待打磨（证据已归档）；
- L2 性能/稳定性跑出基线并**抓到真实问题**（见下）。

**发现的应用问题（证据在设备 faultlogger，待排查）**：
1. **native 崩溃**：阅读约 30 秒时 SIGSEGV 于 `fz_do_always`（libmupdf.so）× NAPI 异步线程，崩溃时进程内存已涨到 ~1.1GB（Alice 演示 EPUB，2026-09-21 01:04）；
2. **应用冻结**：2600 页大 PDF 快速连续翻页触发 appfreeze（00:34）；且大书打开退出后再开其它书会得到 0/0 空文档（进程曾重启）；
3. 阅读器横向模式的注入滑动/两侧点击翻页无效（全屏手势层遮挡 Swiper + 翻页区行居中叠放，`Reader.ets` 3835-3970 结构）——测试侧暂以音量键作为确定性翻页通道。

**怎么验证**：模拟器在线时在 `ci/autotest-hmos/` 下 `python run_all.py`（L0 应全绿、退出码 0）；报告见 `results/<最新>/report.md`；打桩日志 `hdc shell hilog -x | grep HRTEST`。
## [2026-09-20] 修复：OPDS 目录删除后在开启同步的设备上"复活"

**现象**：开启了 WebDAV 同步的设备（如 MI9）上，在"我的文件 → 网上书库(OPDS)"里删除某个目录（例如电子佛典），过一会儿它又自己回来了。

**原因**：目录列表会通过 WebDAV 配置同步在设备与服务器之间做"并集"合并——服务器上还留着的条目会被同步补回本地。为防止这种复活，应用设计了"删除墓碑"（本机删除标记），但检查发现：①"我的文件"页的删除根本没写墓碑；②网络页删除/取消收藏写了墓碑但格式差一个尾部分号，永远匹配不上；③添加、编辑、恢复默认目录时也不清理/补写墓碑。多重叠加导致删除怎么都留不住。

**修复**：墓碑格式统一归一化；"我的文件"删除、编辑换行、恢复默认目录都正确补写墓碑；重新添加同一目录会自动清除墓碑（删了再加不会被误吞）。已删除的目录从此在同步后保持删除状态。

**怎么验证**：MI9 真机实测——删除 CBETA 电子佛典后，本机删除标记立即写入；删除动作触发的同步轮、重启应用后的周期同步轮各跑一遍，目录列表里不再出现该条目，全程无崩溃。

## [2026-09-19] 界面调整：我的文件书库子页精简、阅读单击/双击配置重构、更新日志换新地址等六项

**做了什么 / 有什么变化**：

1. **我的文件 → 书库文件夹子页工具栏精简**：点开书库文件夹后的页面，顶部只保留"返回、路径、排序、视图切换"四个控件；原来的第二行快捷入口（机身存储、DOWNLOAD、HowRead/下载）以及主页跳转、粘贴、新建文件夹按钮全部去掉，页面更清爽。普通"我的文件"页内浏览和各处选文件夹页面不受影响。
2. **网上书库（OPDS）默认列表去掉"CBETA 电子佛典"**：新安装/恢复默认后不再自带该源；已经保存过列表的设备不受影响，可手动长按删除。
3. **"单击"配置改为"阅读时单击"，只保留 3 个选项**（选择模式、上下翻页、左右翻页），演奏模式、标签管理、打开方式三个选项移除。之前恰好选了这三个被移除选项的设备，会自动回到"选择模式"（打开书时弹选择框），保证界面显示与实际行为一致。
4. **新增"阅读时双击"配置**（在阅读时单击下面）：可选项为"缩放到合适大小"（默认）和"启用切除白边"。阅读中双击屏幕中央即执行对应动作：缩放回合适大小，或切换白边裁剪。此前双击默认是"显示/隐藏界面"，升级后自动跟随新默认"缩放到合适大小"；如果之前手动改过双击动作，则保持你的选择。阅读器内的"双击(中央)"设置与这项配置是同一个配置项，两边任一处修改互相可见，选项列表里也补上了"切除白边"。
5. **阅读配置里移除"长按"配置项**：长按书架书籍维持默认行为（弹书籍菜单）。
6. **软件说明 → 更新日志链接**改为官网新地址 https://380121850.github.io/howread/what-is-new/zh.html 。

**怎么验证**：真机（MI9，v1.3.13）实测——书库文件夹子页仅剩 4 个控件、无快捷条；偏好→阅读配置里"阅读时单击/阅读时双击"两行正常、无"长按"行，单击选项只剩 3 个，双击选项为缩放到合适大小/切除白边；双击开书实测切白边动作生效、无崩溃；软件说明弹窗更新日志入口正常。

### [2026-09-20] 鸿蒙 0.9.10：移植安卓新功能与优化（远读 PDF 懒页树、阅读双击缩放、子页工具栏/排序、更新日志链接）

**做了什么（用户视角）**
- **远读 PDF 提速（MuPDF 引擎优化同步）**：安卓侧 9-19 对 MuPDF 引擎做的"懒页树 + 页图持久化"优化移植到鸿蒙——引擎库按共享源码重新编译（两 ABI），远程 PDF 首次打开后会把"页号→文件对象"映射存到缓存旁（pagemap.bin），再次打开缓存书时直接回注引擎，页树零流量、页尺寸查询零页加载，打开更快。全新增的 book_lazy.pdf（300 页）实测：整本缓存后二开自动存页图、三开自动注入。
- **平台冲突隔离**：引擎新接口在共享头文件里落在 C 语言约定块之外（安卓 C 胶水无感，鸿蒙 C++ 直接链接会失败）——在鸿蒙 NAPI 侧把引擎头文件包进 C 约定块解决，共享引擎源码与安卓代码零改动；页图的"导出/注入"被安卓平台放在主线程会自我死锁（鸿蒙实测触发应用冻结）——改为复用鸿蒙既有的后台文档任务通道执行。
- **阅读时双击缩放**：阅读页中央区域双击，在"缩放到合适大小（1.5 倍舒适档）"与放大（2 倍）之间往返，对齐安卓 v1.3.13 新增的"阅读时双击"默认行为；单击菜单、长按选文字不受影响（"切除白边"选项依赖自动裁边引擎，随后续批次放开）。
- **书库文件夹子页工具栏对齐安卓**：只留「‹ 返回 / 路径 / 排序 / 视图切换」，原来的 ⌂ 主页、↑ 向上合并进返回；新增排序按钮（名称↑→名称↓→最新→最大 循环，目录和文件都生效），返回键也能直接回根。
- **更新日志链接更新**：抽屉"软件说明"里的更新日志指向新的在线页面（与安卓 v1.3.13 同地址），替代原 GitHub CHANGES 链接。
- **回归确认**：EPUB / TXT（分块新路径）/ UTF-8 FB2 / 本地 PDF 打开渲染全部正常；GBK 编码 FB2 打不开为既有限制（旧版本同样如此，非本轮引入）。

**如何验证**（Pura 90 模拟器，HowRead-Pro v0.9.10 debug 实测）
- 远读 PDF：我的文件 → HowRead-Test → 打开 book_lazy.pdf → 返回再开，日志出现 "pagemap saved: 300 pages"，再次打开出现 "pagemap injected: 300 pages"，缓存目录出现 pagemap.bin。
- 双击缩放：阅读页中央双击放大/还原，日志 "Double-tap zoom: 3.00x / 1.50x"。
- 子页工具栏：书库文件夹子页显示 ‹/路径/名称↑/网格；点排序切"名称↓"后目录与文件列表反转；点 ‹ 回根。
- 阅读回归：EPUB2/TXT/FB2(UTF-8)/本地 PDF 均正常打开渲染。

**内部佐证（辅助）**
- 引擎：Builder 共享主树（pdf-page.c/pdf.h/html-* 等）重编 harmony/libmupdf.so（arm64/x86_64），harmony CMake 补编新文件 txt-chunk-doc.c；mupdf_napi.cpp 头文件 extern "C" 包裹（隔离 pdf.h 尾部声明）。
- 页图：NAPI 新增 7 导出 + docOpAsync op13（完成懒树+导出 JSON）/op14（注入 JSON）；Reader.ets 远程 PDF 打开后注入/整本缓存后构建 sidecar；RemoteBook.ets 读写 pagemap.bin（PGM1 大端格式对齐安卓）。
- 其余：Reader.ets 中央手势改 Exclusive 组（双击/长按/单击）；Index.ets 子页工具栏与 browseSortMode、WHATSNEW_URL；版本 48/0.9.10。

---

### [2026-09-20] 华为渠道上架材料：《权限用途说明》成文文档

**做了什么（用户视角）**
- 为华为应用市场提审准备了一份可直接照抄的《权限用途说明》（store/android/huawei/权限用途说明.md）：把华为包里的全部 24+1 项权限逐条写清"是什么、谁声明、干什么用、审核答复怎么写"，AGC 后台"敏感权限申报"逐项填写时直接复制粘贴即可。
- 关键结论（用三渠道 APK 对比+合并溯源报告钉死的，不是猜的）：审核最严格的"查询所有应用包"（QUERY_ALL_PACKAGES）和设备标识类权限（UDID/MDM/折叠屏/AD_ID 等 12 项）**全部是华为广告 SDK 自动带入的，应用自身代码没有声明、也不遍历用户应用列表**——文档里给了可直接粘贴的审核答复措辞，以及万一被驳回的回退方案（manifest 移除后复验广告）。
- 应用自身声明的权限都好解释：所有文件访问=扫描本地书籍（离线阅读器核心功能）、前台服务=TTS 熄屏朗读、通知=朗读控制面板、生物识别=书籍密码的指纹解锁、开机广播=官方 WorkManager 组件恢复任务。每条都有现成答复文案。

**如何验证**
- 打开 store/android/huawei/权限用途说明.md 对照文末附录命令，可自行复核：导出 huawei release APK 权限清单、逐项查看声明来源（blame 报告）、三渠道差分。

**内部佐证（辅助）**
- 数据源：HowRead-Huawei-v1.3.13-arm64.apk（release）合并清单 + manifest-merger blame 报告（app/build/intermediates/manifest_merge_blame_file/huaweiRelease/）；QUERY_ALL_PACKAGES←ads-base-prime、BROADCAST_STICKY←ads-splash-prime、MDM/UDID/DOWNLOAD/AD_ID/折叠屏/WIFI←ads-core-prime、RECEIVE_BOOT_COMPLETED←androidx.work:work-runtime:2.11.2、DYNAMIC_RECEIVER←androidx.core:core:1.13.0、其余 10 项为应用自身清单。

---

### [2026-09-20] 华为渠道收尾：移除 AD_ID 权限 + 启动时自动核对退款（购买态自愈）

**做了什么（用户视角）**
- 华为渠道包精简：移除了华为广告 SDK 自动带入的 AD_ID 权限（无 GMS 设备上本就不生效），华为包权限从 25 项减到 24 项，提审清单更干净。
- **退款自动回锁**：以前只有手动点"恢复购买"才会发现退款；现在每次打开应用会在后台自动向华为核对一次购买记录，华为侧查不到已购（退款）时，Pro 会自动回到锁定状态（高级功能重新上锁、广告恢复显示），无需用户做任何操作。离线时按本机记录判定，不会误锁。
- **购买后广告立即消失**：以前当次会话里已展示的横幅要等界面重建才消失；现在 Pro 解锁一生效，主界面残留的横幅立即移除，之后不再加载任何广告。
- 上述改动同时惠及 pro 渠道（同一段共享代码），fdroid 无广告无内购不受影响。

**如何验证**
- MI9 真机（无 HMS Core）：装 huawei debug 包启动无崩溃，日志确认启动时已发起购买记录核对（无 HMS 设备由 SDK 侧安全失败，应用兜底不崩溃）。
- APK 复核：华为包 24 项权限、AD_ID 零命中、"查询所有应用包"保留（广告 SDK 依赖）。
- 退款回锁的完整链路需等 AGC 配置好 IAP 沙盒后复验（退款测试单 → 重启应用 → Pro 自动回锁）。

**内部佐证（辅助）**
- hmsAds/AndroidManifest.xml 增加两处 tools:node="remove"（AD_ID + AGConnect 自启）；注意 uses-permission 的 remove 标记必须是 manifest 直接子元素，放 application 内会被 merger 当普通声明合并（本轮踩坑已注释）。BillingDelegate/Stub/Huawei delegate 新增 syncOwnershipSilently（启动静默核对，MainTabs2.onCreate 调用）；AdsFragmentActivity.onResume 未解锁分支新增 onDestroyBanner（解锁后立即清残留横幅）。

---

### [2026-09-19] 新增华为应用市场渠道（huawei flavor）：华为 IAP + 华为广告接入

**做了什么（用户视角）**
- 安卓端新增第三个构建渠道 huawei（华为应用市场 AppGallery 专用 APK）：包名与 pro 相同（com.leestudio.howread.pro.reader，与 AGC 后台创建的应用一致），应用名同样显示 HowRead Pro／好好读 Pro，版本号与 pro/fdroid 统一（1.3.13）。产物命名 HowRead-Huawei-v<版本>-<abi>.apk，已纳入标准构建批次（pro/fdroid/huawei × debug/release 共 6 个任务，全部构建通过）。
- **华为 IAP 接入**：偏好页底部和抽屉"软件说明"里的 Pro 卡片"升级"按钮，在华为渠道会拉起华为收银台购买非消耗型商品（默认商品 ID howread_pro_unlock，可在编译服务器 ~/.gradle/gradle.properties 用 hw_iapProductId 修改）。购买成功后 Pro 卡片立即变为"已激活"，全部 Pro 高级功能（WebDAV/SMB/SFTP 同步、AI 大模型/翻译、远程阅读、阅读统计等）即刻解锁，广告同步隐藏——与原 stub 流程完全同一入口，界面零改动。"恢复购买"会重查华为侧已购记录回填解锁；卸载重装后点"升级"会先查已购、已购则直接解锁不再扣款；华为侧查不到已购（退款）会自动收回解锁。离线时按本机缓存判定，不误锁已购用户。
- **华为广告 SDK（Petal Ads）接入**：横幅/插屏/激励视频三种广告位全部实现，结构与 AdMob 渠道一致（空 ID 保护：默认不出广告；配置 hw_bannerId/hw_fullId/hw_rewardId 即激活，无需改代码）。当前编译服务器已填华为官方测试广告位用于真机联调；**正式上架前必须替换为 AGC"流量运营"申请的真实媒体位 ID**（或删除这三项恢复静默）。
- 渠道行为对齐 pro 而非 fdroid：.cbr 漫画支持保留（junrar 经 libDepPro）、评分跳转走应用市场链接；华为设备无 GMS，APK 内不含任何 Google 广告代码（已验证 dex 零命中）。
- 技术取舍说明：AGConnect Gradle 插件（agcp）与本项目多 flavor 结构不兼容（任一 flavor 缺 agconnect-services.json 即在 apply 时报错），已弃用——HMS IAP 与广告 SDK 运行时均不读取该 json；agconnect-core 的自动初始化 Provider 已在华为渠道 manifest 中移除（本应用不使用 AGC 云服务，无影响）。

**如何验证**
- 服务器已出包：android/app/build/outputs/apk/huawei/{debug,release}/HowRead-Huawei-v1.3.13-<abi>.apk（5 ABI × 2 buildType，release 用 howread.keystore 签名）。
- APK 抽查（aapt2 + dex 字符串）：包名/版本/应用名正确；华为 IAP（103 处）与广告（1082 处）类在包内；Google 广告代码 0 处；AGConnect 自启 Provider 已移除。
- 真机（华为 P20/P30，有 HMS Core）：装 huawei debug 包 → 启动无崩溃 → Pro 卡片"升级" → 华为收银台沙盒扣款 → Pro 激活、高级功能可用、广告消失 → 恢复购买、卸载重装后再恢复。MI9 等无 HMS Core 设备仅验证不崩溃。
- AGC 控制台待办（用户操作）：登记 howread.keystore 的 SHA256 指纹（F0:B8:…:0F:E2）、开通华为 IAP 并配置商品（非消耗型）、流量运营申请真实广告媒体位、准备商店资料（隐私政策/备案/软著）。

**内部佐证（辅助）**
- src/huawei/（LibreraBuildConfig FLAVOR=huawei + app_name 三语）注册进 app/build.gradle；新增 src/hmsAds/（HuaweiAdsProvider、AdsProviderFactory、HuaweiBillingDelegate、BillingDelegateFactory、IapPurchaseActivity + manifest overlay）；src/main 的 BillingManager 重构为 delegate 分发（BillingDelegate 接口 + 每渠道同名 BillingDelegateFactory，admobAds/noAds 挂 StubBillingDelegate 行为不变），全部调用点零改动；新增 6 个 pro_* 文案（三语）。HMS 版本：agconnect-core 1.9.5.302 / iap 6.16.6.305 / ads-prime 3.4.90.302（developer.huawei.com/repo 已加入 settings.gradle.kts 两个仓库块）。华为 API 关键差异（对照 AAR javap 确认）：IapClient.obtainOwnedPurchases（非 queryPurchases）、Task 为 com.huawei.hmf.tasks.Task、非消耗品无需 acknowledge、PriceType 是 IapClient 内部接口、广告用 BannerAdSize.BANNER_SIZE_SMART / RewardAdStatusListener.onRewarded。

---

### [2026-09-19] 鸿蒙 0.9.9：网络浏览整页化（返回键/标签栏直返）+ OPDS 预置只留 Project Gutenberg

**做了什么（用户视角）**
- 「我的文件」里点 WebDAV 服务器（OPDS/SMB/SFTP 同样式），不再弹出居中浮层卡片，而是进入一个占满内容区的完整文件浏览页面：顶部"‹ 返回 + 服务器名 + ✕"，文件列表铺满中间；顶部标题栏和底部标签栏保持可见可用。
- 退出浏览页更直接：手机返回键一键回到「我的文件」；浏览中点底部任意标签（书库/偏好…）直接跳转过去，不用先关页面；点页内 ‹ 或 ✕ 也一样退出。之前在浏览页按返回键会退出整个应用，现在正确返回我的文件。
- 书库文件夹同样处理：点开文件夹后的浏览页，按返回键直接回到「我的文件」根，不必一级级点向上。
- OPDS 网上书库只保留 Project Gutenberg 一个预置目录（按需求移除内置的 Standard Ebooks、CBETA）；老版本升级安装后这两个预置也会自动清除，用户自己手动添加的服务器不受影响。
- 回归确认：浏览页内点开远程书在线阅读、返回后仍停在浏览页并提示"下载完成"；抽屉、日/夜横幅、格言、书库合并展示等 0.9.8 功能不受影响。

**如何验证**（Pura 90 模拟器，HowRead-Pro v0.9.9 debug 实测）
- 我的文件 → 点 HowRead-Test：整页浏览、底部标签栏可见；浏览中点"书库"标签直接跳转；重进浏览页按返回键回我的文件根。
- 点"书库"文件夹进入文件夹浏览，按返回键回根。
- 网上书库（OPDS）区只剩 Project Gutenberg 一行。

**内部佐证**：Index.ets 主列 Tabs 外包 Stack，浏览层叠加其上并留 barHeight(70) 底部透明条（hitTestBehavior Transparent 放行标签栏点击，onChange/switchTab 关闭浏览层）；onBackPress 依次处理 subPage/网络浏览/关于/文件夹浏览；Servers.ets 种子只含 Gutenberg + KEY_LEGACY 一次性升级迁移；版本 47/0.9.9。

---

### [2026-09-25（四）] 手册 AI 翻译按两种模式分述（卡片模式/页内双语，新增 1 图）

**做了什么（用户视角）**
- 按产品实际形态把 AI 翻译的两种呈现模式分开表述：**14.6 卡片模式：原文在上、译文在下**（默认方式——悬浮卡片逐段展示"原文引句+完整译文"，按页跟随、不改变原书排版，图20 即卡片模式实拍）；**14.7 页内双语翻译：译文嵌入原文之间**（勾选"在页面内显示译文（双语对照）"后，译文块直接排入书页排版流、逐段嵌在原文下方，整页如双语对照读本，图21 为嵌排效果实拍——译文以浅色底纹嵌在英文段落之间）；"使用注意事项"顺延为 14.8。
- 14.5 AI 翻译总述段同步引出"两种模式（见 14.6、14.7 节）"；图21 为真机新增截图（勾选双语开关后，已缓存译文在书页内嵌排的实拍）；全册图注重编号 图1~图24 连续，目录刷新（42 页）。

**如何验证**
- 打开 store/manual/好好读Pro电子书阅读软件用户使用手册.docx / 同名 .pdf：第 14 章 14.1~14.8 八节；35~36 页 图20（卡片模式）/图21（页内双语嵌排）分别位于 14.6/14.7 节内。

**内部佐证（辅助）**
- tmp/scripts/split_translate_modes.py（两模式拆分+新图插入+图号重编）；截图 tmp/build/ai_nas_shots/bilingual_embedded.png（真机勾选双语对照后实拍）。

---

### [2026-09-25（三）] 手册 AI 翻译章拆出"AI 页内双语翻译"专节（14.6）

**做了什么（用户视角）**
- 应要求在 AI 翻译部分为页内双语翻译单独立节：原 14.5"AI 翻译与页内双语对照"拆为 **14.5 AI 翻译**（翻译面板、语言对、开始翻译、结果缓存、整书翻译格式 EPUB/TXT/MOBI/AZW3）与 **14.6 AI 页内双语翻译** 专节——对照阅读模式定位（译文直接排进原书页面、原文下方插译文浅色底纹区分）、使用过程（翻译面板勾选"在页面内显示译文（双语对照）"→开始翻译→页顶横幅实时显示"AI翻译(Pro) → 目标语言 · 第 N 页"进度，译到哪页哪页出译文）、效果与状态（双语模式按书记忆、重开自动延续、再次发起自动停旧任务、关闭开关恢复纯原文）、格式限制（仅 EPUB/TXT，非 UTF-8 书籍建议整书翻译）。原"使用注意事项"顺延为 14.7，目录随 Word 域刷新（41 页）。
- 修复过程中发现并纠正的一处段落顺序错误（14.6 内四段因编辑脚本插入次序颠倒），已按"定位→开启→效果→限制"重排并复验渲染。

**如何验证**
- 打开 store/manual/好好读Pro电子书阅读软件用户使用手册.docx / 同名 .pdf：目录第 14 章为 14.1~14.7 七节；34~35 页为 14.5/14.6/14.7 依次衔接，图20（双语对照截图）位于 14.6 节内。

**内部佐证（辅助）**
- tmp/scripts/add_bilingual_section.py、fix_146_order.py（段落顺序修正）、render_manual_pro.py（目录刷新与 PDF 导出）。

---

### [2026-09-25（二）] 手册增补：AI 功能与 NAS 网盘"使用过程与效果"专节（真机实测取证，新增 5 图）

**做了什么（用户视角）**
- 在 50.23 三协议测试环境与智谱 AI 配置下，用小米 9 真机（HowRead Pro v1.3.11）把要写进手册的功能逐项实测并截图，再按实测结果重写手册：
  - **第 14 章（AI 智能辅助）整章重写**为 6 节：14.1 功能概述、14.2 AI 服务配置（含 OpenAI 兼容接入智谱示例）、**14.3 AI 简介书籍**（入口→"AI 思考中…"→数秒生成百余字中文简介，英文书同样输出中文，可保存为笔记）、**14.4 AI 阅读笔记**（阅读中选中文字→"发送给 AI"→可附加问题→解读结果"保存到笔记"，笔记按书聚合置顶、带时间戳、随同步跨设备）、**14.5 AI 翻译与页内双语对照**（翻译面板选源/目标语言→勾"在页面内显示译文（双语对照）"→开始翻译→页顶横幅实时显示进度，原文/译文同屏对照，结果按书缓存不重复计费；页内双语仅 EPUB/TXT，整书翻译支持 EPUB/TXT/MOBI/AZW3）、14.6 使用注意事项。
  - **第 12 章（NAS 网盘）新增 2 图**：SMB 协议浏览个人书库（真实文件大小）、256 MB 扫描版 PDF 网盘点击即开（1/500 页）——用测试服 SMB 共享里的真实大书验证"大书点击即进阅读器"效果。
  - 全册图注重编号为 图1~图23 连续；目录随 Word 域刷新（41 页）。
- 实测过程说明：MI9 的 AI 大模型配置经后台方式重灌（app-State.json + app-AI.json 官方恢复路径注入，key 由用户手工修正中文句号笔误后保存生效）；测试书走 50.23 的 SMB 共享（testbooks），256MB 大 PDF 点击后 1~2 秒进入阅读器。

**如何验证**
- 打开 store/manual/好好读Pro电子书阅读软件用户使用手册.docx / 同名 .pdf（41 页）：第 12 章图13~图15、第 14 章图18~图20 为本轮新增；目录 14.1~14.6 归位第 32~35 页。
- 渲染验收 40/40 页初验后修复 2 项结构问题（14 章文字误落附录后、图18 含系统状态栏）并复验通过。

**内部佐证（辅助）**
- tmp/scripts/：ai_intro_flow.py（状态驱动真机取证）、shot2.py/crop_ai_shots.py（截图裁剪）、reinject_ai.py（AI 配置后台注入）、edit_ai_nas.py + fix_ch14_order.py + fix_appendix.py + renumber_caps*.py（docx 结构编辑与图注重编号）。
- 截图素材：tmp/build/ai_nas_shots/（ai_intro_dialog / ai_note / bilingual_page / smb_browse / nas_bigbook）。

---

### [2026-09-25] 用户手册应用名全称更新为"好好读Pro电子书阅读软件"

**做了什么（用户视角）**
- 用户手册（软著登记版）应用名全称统一为**好好读Pro电子书阅读软件**：文档定名《好好读Pro电子书阅读软件用户使用手册》（docx + 同名 pdf，store/manual/）；标题页软件名、页眉"好好读Pro电子书阅读软件V1.3.11用户使用手册"、正文 1.1 软件简介为应用全称（英文标识 HowRead Pro）；3.1 应用市场搜索关键词同步为"好好读Pro"或"HowReadPro"（与当前 APP 显示名 HowReadPro 一致）；文档属性标题同步。
- 用户本轮手工改动一并保留：删除 1.1"软件基本信息"表格（软件名/版本等信息由标题页与页眉承载）。
- 与 APP 当前状态核对一致：显示名 HowReadPro、版本 V1.3.11（appCode 7313）。
- **品牌写法去空格（同日补充）**：中文"好好读Pro"、英文"HowReadPro"，Pro 前一律不带空格——修正 1.1 英文标识"HowRead Pro"→"HowReadPro"，并对全册做带空格写法清零校验（页眉/正文/表格/页眉均无"HowRead Pro"或"好好读 Pro"）。
- 重导 PDF（38 页）并逐页程序检查：低行数页均为标题页/目录尾/末页或"图与图注同页"策略造成的适度留白（均超半页内容），页眉品牌正确，目录已刷新。

**如何验证**
- 打开 store/manual/好好读Pro电子书阅读软件用户使用手册.docx / 同名 .pdf：标题页、页眉、1.1、3.1 的应用名均为"好好读Pro"口径；搜索关键词与商店实际可搜名称一致。

**内部佐证（辅助）**
- tmp/scripts/scan_brand.py（品牌出现位置扫描）、fix_brand.py（3.1 关键词 + 文档属性）、render_manual_pro.py（Word COM 目录刷新 + PDF 导出）、check_manual_pro.py（逐页行数）。

---

### [2026-09-24] 官网版本口径统一为 V1.3.11（更高版本号的日志并入）+ 三张素材截图

**做了什么（用户视角）**
- 官网（docs 站）版本口径统一为当前版本 **V1.3.11**：大于该版本的"1.3.13"版本号全部清除，其修改描述原文并入 V1.3.11 条目之下——what-is-new 中英文页合并为一个"1.3.11 · 同步、书库与在线阅读更可靠"小节（描述一条不少）；首页"更新日志"手风琴首条改为合并后的 v1.3.11（日期 2026-09-17）；首页中英文的"当前版本"与两张下载卡的"版本 1.3.13"同步改为 1.3.11。全站已无 1.3.13 残留；应用内"更新日志"入口打开的就是官网 what-is-new 页，随之生效。
- 新增三张素材截图（华为 P30 真机、HowRead Pro v1.3.11 实拍，均已裁掉系统状态栏）：**APP 首页**、**关于页面**（对话框 8 行信息齐全，"This is a fork of Librera…"一行已抹除）、**隐私政策首页**（官网隐私政策页顶部，含通用/华为平台中英文四个政策入口；底部"旧版 Librera 应用（存档）"区域已裁掉）。截图存工作区 tmp/build/site_shots/。
- 为拍关于页，P30 上的应用由旧版 1.3.13 升级安装为当前 1.3.11（pro debug arm64，当日构建）。

**如何验证**
- 查看 docs/what-is-new/zh.md、what-is-new/index.md、_data/changelog.yml、index.md、en.md：版本号均为 1.3.11，搜索不到 1.3.13；
- 打开 tmp/build/site_shots/ 三张 PNG 核对内容。

**内部佐证（辅助）**
- tmp/scripts/site_sync_1311.py（docs 5 文件锚定替换）、make_site_shots.py / fix_site_shots.py（P30 截图裁状态栏、品牌行底色遮盖、存档区裁剪）；安装包 android/app/build/outputs/apk/pro/debug/HowRead-Pro-v1.3.11-arm64.apk（09-24 构建）。

---

### [2026-09-18（晚）] 软著手册升级：真机截图插图版（19 图）+ 文档去版本号 + 改名

**做了什么（用户视角）**
- 按要求文档名去掉版本号，定名为《好好读电子书阅读软件用户使用手册》（docx + 同名 pdf，存 store/manual/）；标题页"文档版本：V1.3.11"行删除；页眉与封面中的 V1.3.11 属软件版本，按要求保留；正文衔接用户的手工修改——用户已删除原 14.4 节，本轮把"14.5 使用注意事项"顺延改号为 14.4，第 14 章编号恢复连续。
- 为每个主要功能界面补充**华为 P20 真机截图 19 张**（pro V1.3.11 实拍；P30 未连接故改用 P20），覆盖：主界面/侧边栏/书库/阅读界面/阅读菜单/阅读器更多菜单/目录面板/阅读设置/我的文件/书库文件夹/添加 WebDAV 服务器/服务器浏览页/网盘书籍在线阅读/在线阅读设置/WebDAV 同步设置/AI 大模型接入/阅读统计图/偏好设置/书库设置；每张配加粗居中图注"图1~图19"。
- 截图处理：统一裁掉手机系统状态栏（信号/电量行，满足"不要真机信号栏"要求）；首页图抹除"Librera Reader"演示封面、阅读设置图抹除"Google Translate"词条等第三方品牌痕迹；同步对话框密码为掩码显示，无敏感信息。
- 版式保障：截图段落设"与下段同页"（图与图注不再跨页分离）；全部表格行禁止跨页断裂、跨页续表自动重复表头；目录域已刷新。
- 全册 38 页；有图页面按软著规则豁免每页 30 行要求，纯文字页仍满足 ≥30 行。

**如何验证**
- 打开 store/manual/好好读电子书阅读软件用户使用手册.docx 或同名 pdf（38 页）。
- 已逐页渲染验收 38/38 通过：图注编号连续、图与图注同页、截图完整无系统状态栏、目录页码抽查与正文一致、无第三方品牌与包名残留。

**内部佐证（辅助）**
- 工作区 tmp/scripts/：cap20.py（adb 真机截图/点击/控件树）、crop_shots.py 与 mask_shots*.py（裁状态栏、暗像素扫描定位品牌文字遮盖）、insert_shots.py（python-docx 按章节标题锚定插图）、fix_issues.py（keepNext/表格行属性/编号顺延）、render_cr_manual.py（Word COM 刷新目录导出 PDF）。

---

### [2026-09-18] 新增软著登记版《用户使用手册》（V1.3.11，文档鉴别材料）

**做了什么（用户视角）**
- 面向软件著作权登记申请，新增一份符合登记格式要求的《好好读电子书阅读软件V1.3.11用户使用手册（软著登记版）》，存放在 store/manual/，与面向用户的商店版手册（HowRead用户手册.docx）相互独立、互不影响。
- 手册基于当前最新安卓版本 V1.3.11（appCode 7311）的真实功能编写：18 章 + 附录速查表，覆盖获取安装、书库管理、阅读与排版、TTS 朗读/笔记/双语翻译、NAS 网盘访问与网盘书籍在线阅读（WebDAV/SMB/SFTP，含流式边下边读、缓存管理、变体识别等 1.3.7~1.3.11 的新能力）、数据同步、AI 智能辅助、阅读统计、偏好设置、Pro 说明与常见问题。
- 格式按软著文档鉴别材料通行要求执行：除标题页外每页页眉标注"软件名称+V1.3.11+用户使用手册"、页脚连续页码；正文宋体小四、固定行距 15.6 磅，正文页每页 30 行以上（满页约 43~46 行）；纯文字不夹图；目录由 Word 域生成、页码真实可刷新；全文不含应用包名与第三方品牌/版权信息；版本号口径统一为 V1.3.11。
- 全册 27 页。按登记规则"文档不足 60 页的全部提交"，可直接整册提交；如需凑足 60 页（前 30 后 30 提交规则）可再扩写。
- 软件全名采用"好好读电子书阅读软件"（英文标识 HowRead）；若著作权登记申请表使用了其他软件全称，告知名称即可重新生成。

**如何验证**
- 打开 store/manual/好好读电子书阅读软件V1.3.11用户使用手册.docx：目录可右键"更新域"刷新页码；除标题页外逐页有页眉页脚。
- 文档名已按用户要求去掉"（软著登记版）"后缀；用户手工修改后重新导出 PDF：store/manual/好好读电子书阅读软件V1.3.11用户使用手册.pdf（27 页，导出前已用 Word 刷新目录页码；程序逐页校验正文页均 ≥30 行，末页为附录表格自然收尾）。

**内部佐证（辅助）**
- 生成脚本（工作区 tmp/scripts/）：gen_cr_manual.py（python-docx 生成：固定行距/页眉页脚/目录域）+ cr_content_a.py/cr_content_b.py（章节内容）+ render_cr_manual.py（Word COM 更新目录、导出 PDF）。
- 事实来源：版本号取自 android/app/gradle.properties（1.3.11/7311）；运行环境取自 android/gradle/libs.versions.toml（minSdk 24，Android 7.0+）与 AndroidManifest 权限清单；格式支持清单与 BookType/格式设置一致。

### [2026-09-18] 鸿蒙 0.9.8：侧边栏日/夜背景图、我的文件精简（远程书进书库）、服务器专属浏览页、偏好页三大项改版

**做了什么（用户视角）**
1. 侧边栏顶部背景图与安卓一致：白天是"窗边读书的猫"暖色图、夜间是"月夜台灯"图，跟随夜间模式自动切换；"值得读，好好读"标语叠在图上。
2. 侧边栏格言下移：从导航菜单正下方移到底部按钮条上方（对齐安卓位置），打开抽屉即可在底栏上方看到每日格言。
3. 「我的文件」更清爽：去掉"远程书籍"一节和"已导入的书"入口；在服务器行点 ⟳ 扫描到的远程书、以及浏览时在线打开过的远程书，统一出现在「书库」标签里（带协议小图标），点开即流式阅读。书箱菜单的"删除"对远程书只移除记录、绝不误删本地文件；扫描提示改为"扫描完成：N 本已加入书库"。
4. 点击 WebDAV/SMB/SFTP 服务器行，进入该服务器的"专属浏览页"：顶部为"‹ 返回 + 服务器名 + ✕"，不再出现四协议切换按钮；WebDAV 不再显示手输地址表单（直接连接已保存的服务器）；目录/文件列表、在线打开、下载一切照旧。点书库文件夹仍是页内完整本地浏览（向上/路径/列表网格）。
5. 偏好页三大项（书库设置/常规设置/UI配置）内的配置子项改为安卓样式"配置按钮行"：左侧滑块图标 + 下划线链接文字 + 右侧展开箭头；"屏幕方向/语言/WebDAV 同步/AI 大模型"等名称+值行的值统一右对齐；"软件说明"移入 UI 配置组内（与安卓一致）。

**如何验证**
- Pura 90 模拟器 x86_64 debug（v0.9.8 / versionCode 46）：白天/夜间两张 banner 截图比对无误；格言位于底栏上方；我的文件无"远程书籍/已导入的书"；添加 WebDAV 测试服务器（192.168.50.23:8765）后扫描，书库出现全部远程书且可点开流式阅读，菜单删除纯扫描书后彻底消失；专属浏览页"返回/服务器名/无切换器"三要素 + Connected 列表 + 打开文件 + 返回均正常；偏好三大项行样式/右对齐/软件说明归位；重启后服务器条目保留。

**内部佐证（辅助）**
- Index.ets：buildDrawerPanel banner 改 Stack（drawer_banner_day/night.png 自安卓 drawable-xxhdpi 移植）；getLibraryBooks 合并 remoteBooks（recentBooks 优先去重，规避 recent_books 20 条上限）；buildNetworkOverlay 头部重写并删除协议切换器（新增 netBrowseTitle）；buildWebdavPanel 去手输表单；settingsSubBar 改滑块图标配置行；libMenuDeleteItem/libMenuBook 增加 remote:// 分支。

### [2026-09-19] 清理误入版本库的构建中间文件（约 4.9GB），并补 .gitignore 防再犯

**做了什么（用户视角）**
- 之前 git 状态里刷屏的 `Builder/jni/obj/...*.o` "修改"，是当年把 NDK 编译的**中间产物**（目标文件 .o、依赖文件 .d，共约 7000 个、2.7GB）误提交进了版本库——每次重新编译引擎它们都会变，git 状态永远不干净。本次把它们从工作区删除（git 里会显示为删除，提交一次即可永久清掉），并补了 .gitignore 规则（*.o、*.d、Builder/jni/obj、Builder/jni/libs、Builder/openssl/build-*），以后编译不会再污染版本库。
- 一并清掉的可再生产物（均不影响出包，Android 打包取的是 prebuilt/native 里的正式引擎库）：
  - `Builder/jni/libs/`（82MB）：ndk-build 的最终 so 输出，与 prebuilt/native 里的逐字节相同，纯冗余副本；
  - `Builder/openssl/build-ohos-*/`（90MB）：OpenSSL 鸿蒙构建树（该目录本就被忽略，属本地残留）;
  - `Builder/mupdf-1.23.7/build/`（308MB）与 platform 目录内约 3530 个 .o/.a：mupdf 源码树内的构建产物；
  - `harmony/native/build-arm64-v8a/`、`build-x86_64/`（各 651 个文件）：鸿蒙引擎 CMake 构建目录，下次构建自动重建；
  - `Builder/libsmb2`、`libssh2` 源码树内约 160 个编译残留。
- Builder 目录从 6.6GB 降到 1.7GB。清理后 Android 增量构建验证 BUILD SUCCESSFUL，出包链路无影响。

**保留项（勿当作垃圾）**
- `prebuilt/native/`、`prebuilt/harmony/`：已提交的正式引擎产物（打包种子）；
- `harmony/entry/libs/*/libmupdf.so`：HAP 打包用；
- `Builder/jni/~mupdf-1.23.7/`：有意的引擎补丁（AI 翻译段落标记等）；
- `Builder/jni/libmupdf-librera.c.bak_annot`：人工备份的源码文件。
...
**崩溃类（致命，已修）**
- 打开在线书时的网络探测若持续数秒，期间退出页面不再崩溃（原会对已关闭的窗口弹对话框，直接闪退）。
- 阅读设置中选择"连字语言"时，若当前书没有书库记录不再闪退（原空指针必崩）。

**数据正确与安全类（严重，已修）**
- 在线阅读块缓存：修复两本书同时超过单书缓存上限时互相读到对方数据的静默错乱（内存缓存改为每本书独立一份）。
- WebDAV 同步不再"假成功"：断网/密码错误导致的整轮失败现在明确报"同步失败"且不再刷新"上次同步时间"；部分失败时状态注明"部分未同步"。此前用户会误以为进度已备份。
- 同步变更日志不再泄露密码：书目密码、OPDS 登录等敏感值在日志（logcat 与可查看的 app-SyncLog.json）中整体打码。
- 删除的 OPDS 书源、书库文件夹不再被同步"复活"（删除写入本地墓碑，同步合入时过滤）；"我的文件"页删除 WebDAV 服务器也补齐了与 OPDS 页相同的"防复活+书架清理"。
- AI 命名配置的 API Key 不再明文保存/上传：本地改用加密存储，同步文件彻底剥离密钥；旧明文配置在下次保存时自动转为加密。
- 双语翻译三处并发缺陷：内部"翻译中"集合改为并发容器（原可能损坏导致整段永远不译）；重新打开书时残留翻译会话必须通过"页内双语"开关检查（原会绕过开关静默恢复翻译且无任何界面可关闭）；会话释放时解除对阅读页面的持有（原泄漏整个界面）。
- AI 翻译任务管理：再次发起翻译会自动停掉旧翻译；关闭翻译面板只停自己的翻译（原会误杀新翻译、旧翻译失控继续消耗 API 配额）。
- 同步三方合并：旧版本设备上传的配置缺少新字段时不再被当作"远端已删除"而清掉本地新设置（跨版本同步不再重置）。

**稳定与体验类（一般，已修）**
- 打开大书时点"取消"真正立即停止：原取消后仍会继续打开文档，每取消一次泄漏一份文档，还可能拖死后台配置写盘队列。
- AI 设置/问答对话框：关闭即取消未完成的网络请求，所有回调增加页面存活检查（原"拉取模型列表"在页面销毁后弹菜单可崩溃）；"拉取模型列表"的失败原因改为按次返回（原与双语翻译并发时提示类别随机错）。
- OPDS 下载：完成回调增加页面存活检查；下载立即显示进度条并可防重复点击（原进度条不出现、连点会并行下载互相覆盖损坏文件）。
- SFTP：文件打开阶段的失败会正确断开连接（原每次重试泄漏一条 SSH 连接，长时间弱网阅读可累积到"Too many open files"）。
- "清空在线缓存"改为后台执行（原主线程递归删除数百 MB 会卡顿甚至 ANR），且保留正在阅读书籍的缓存；缓存上限输入增加上限（防误输天文数字）。
- 书签的增删改全部加锁（原与同步清理并发可能静默丢书签）；最近/收藏记录写入同样加锁。
- "我的文件"整目录删除改为后台执行（原主线程可 ANR）；删除 OPDS 书源/文件夹的两处入口与 WebDAV 服务器删除统一接入防复活。
- DOCX 转换缓存改为"临时文件+改名"原子写入（原写一半的坏缓存会被永久采用导致书打不开）；FB2 打开探测失败重开时先回收旧文档（原泄漏）。
- PDF 导出 reflow：异常路径不再泄漏文档与文件句柄；HTML 编码声明不带引号时不再误判编码。
- 全文导出（documentToHtml）在文档被关闭时安全中止（原空指针）；封面/文本抽取改用"自有页"并自回收（消除一处共享页被回收的隐患）。
- 远程扫描对话框"取消"按钮真正取消扫描；ZIP 探测增加压缩大小上限（防畸形文件内存尖峰）；整书下载被取消时不再误开旧副本；重格式（mobi/djvu/cbr/doc）下载保留书签定位；"文件已更新"提示确认后不再每开一次弹一次。
- WebDAV 目录选择器：快速连续进入两层目录不再"列表与路径错位导致选错同步目录"；目录名含空格/?/#/% 时正确编码（新建目录同样处理）。
- 后台预热：超过 5 本按 LRU 淘汰（原进程内最多预热 5 本后该功能静默失效）；开始阅读时未预热书目归还队列稍后再试。
- 「我的文件」根页恢复区块高度上限：配置多个网络源（WebDAV/SMB/SFTP/OPDS）后，网络+书库文件夹区块会把搜索入口、列表/网格切换等挤出屏幕且整页不可滚动（即全量测试中 FN-04/33 记录的"搜索入口不可达"已知缺陷，属核心导航被破坏）。现给该区块恢复 220dp 滚动上限（其包裹类本为此设计，配置缺失），下方各入口恢复可达；浏览子目录视图不受影响。
- 配置保存的变更检测现在遵守 @IgnoreHashCode：亮度滑杆、选中文本等运行时字段不再触发无谓的写盘与同步轮询。
- 同步改用独立线程（原与配置落盘共用单线程池，弱网长同步会拖延所有设置保存）；手动恢复备份后完整生效（原可能被旧快照跳过部分项）；AI 厂商配置删除立即落盘（原可能被同步加回）；旧版书签导入遇坏数据跳过该条而不是整体失败；AdMob 激励视频无广告时不再静默吞掉回调（记录日志）。

**已知问题（本轮评估后暂不修，留档）**
- WebDAV 服务器不支持 Range 时，整书下载仍按块逐次重发前缀（大书流量放大），需重构为单流顺序下载；会话工厂在全局锁内建连（弱网首连可短暂阻塞其它书打开）。
- 同步完成提示里的"↑N 本"计数口径偏大（本地总数减下载数，非真实上传数）。
- 双语模式在主线程做全书段落匹配（大书翻页可能掉帧）；非 UTF-8（如 GBK）EPUB 的双语版会转码（此类书建议暂不用页内双语）。
- 若干轻微项：编辑 WebDAV 服务器改地址后旧凭据残留、少量 static 对话框引用、"清空最近阅读"按钮为空操作等。
- 自动化测试侧（非应用缺陷）："我的文件"根页改版后（网络源+书库文件夹结构），测试脚本仍按旧结构从根页直接找 Download 目录——当设备书库文件夹未包含 Download（MI9 当前为 DSfile/Books 与存储根）时，依赖"浏览到 Download 开书"的用例（FN-07/14/16/18/34/40/41/49/51）会失败。本轮已为 tc_function 增加两版导航兜底（netSection 区内滚动 + 存储根卡片"0"点击），未完全收敛；建议后续专项：①适配根页新导航（可结合 editPath 直达）②测试设备统一把 Download 加入书库文件夹。另 P20 新增同类失败 FN-17（目录跳转后页码指示器未变化，与 FN-49/51"读不到当前页码"同属底栏指示器显示家族，截图证据已存 results）。

**验证**：编译通过；`testProDebugUnitTest` 全绿；pro/fdroid × debug/release 四包构建成功。建议真机重点回归：①断网点"立即同步"应报失败且"上次同步"不变 ②删除 OPDS 书源/书库文件夹后等自动同步一轮不再复活 ③AI 设置里保存命名配置后检查 app-State.json 中 aiConfigs 无明文 key ④打开大书点取消后反复多次无内存增长 ⑤双语翻译中途再开一次翻译并关旧面板，配额消耗停止。

**顺手清理**：删除 `harmony/.variantbak/`（pro 构建脚本某次中断遗留的孤儿备份，已核实正主文件完好）。`Builder/jni/~mupdf-1.23.7/` 三个文件的改动是引擎有意补丁（非临时文件），请随本轮代码一并提交。

---

## [2026-09-18] 鸿蒙界面补齐 11 项：统计卡直达、更多改整页、偏好命名统一、书库/我的文件/偏好精简（0.9.7）

**做了什么（用户视角）**——按问题清单逐项修复鸿蒙版界面：

1. **首页阅读统计**：「阅读统计」区右上角的"更多"去掉；点「总阅读时间」卡直接弹出阅读统计图（近1周/近1月/近1年 三档切换，与安卓一致）；点「已读书籍」卡跳到书库并自动选中"已读"过滤；点「书籍总数」卡跳到书库看全部（后两点与安卓行为一致）。
2. **首页各区的"更多"改为整页切换**（不再是弹出小窗）：「最近阅读」「书签笔记」的更多（含书签卡片点击、抽屉里的"书签笔记"入口）切到对应整页子页，点左上 ‹ 或系统返回键回到原页；「网上书库」的更多直接切到「我的文件」页。
3. **侧边栏命名与图标统一**：抽屉底部的「设置选项」改名「偏好」，与底栏标签一致；"偏好"页签图标从扳手换成与抽屉相同的齿轮。
4. **书库页精简**：去掉「播放列表」「标签」「分组」三个按钮（书库固定为常规排列；书籍长按菜单里的标签/播放列表功能不受影响）；标题栏右侧的"+"按钮删除（全局生效，书库/偏好右上角随之干净）。
5. **我的文件·服务器区重构**：SMB、SFTP 区标题右侧补上「+ 添加」（与 WebDAV 一致）；删除原来的三条「添加 / 编辑 XX 服务器」行；服务器条目上的 扫描⟳ / 编辑✎ / 删除✕ 三个操作保持不变；「书库文件夹」区标题右侧新增「+ 添加」（弹本地目录浏览器选择），每个文件夹行尾新增 ✕ 删除，加入的文件夹**重启后保留**（新增持久化，此前只在内存里）。
6. **我的文件底部**「新建文件夹」「导入书籍」两个按钮删除。
7. **格式设置显示书库计数**：偏好 → 书库设置 → 格式设置，每种格式行显示书库中该格式的书籍数量（含 0）。
8. **四个配置移动位置**：常规设置里的「阅读提醒」「渲染质量」「翻页动画」「顶栏时钟电量」移到「阅读配置」分组（紧跟"退出确认对话框"）。
9. **目录配置升级**：备份配置的存储根目录默认改为 APP 相对路径「HowRead」（旧空值自动迁移），字体目录默认「fonts」、下载目录默认「Download/HowRead」（相对根目录前置）；三项由手输文本框改为可点击行——点击弹本地目录浏览器（逐级进入 / 上一级 / 选择此处），选中即写回并即时刷新。
10. **偏好分组归并**：「备份配置」「在线阅读与缓存」两个分组移入「常规设置」分组内部（作为子分组）。

**怎么验证**（Pura 90 模拟器，v0.9.7 x86_64 debug 实测）：
- 统计弹窗三档页签齐全、已读书籍卡跳书库后"已读"chip 为选中蓝底；统计区无"更多"；
- 最近阅读/书签笔记整页子页（标题+返回键），抽屉入口同效；网上书库更多落在我文件页；
- 抽屉底部显示「偏好」（无"设置选项"）；偏好页顶栏右侧无"+"；
- 书库 chips 仅剩 全部/未读/在读/已读/星标；我的文件页无"新建文件夹/导入书籍"；
- SMB/SFTP/书库文件夹区头"+"均弹出对应对话框；文件夹行 ✕ 删除生效；加入-强杀重启后行保留（持久化生效）；
- 格式行计数与书库实际一致（PDF 1、XPS 1、CBZ 1、HTML 1、其余 0）；
- 阅读配置组内四行齐全；常规设置组内嵌 备份配置/在线阅读与缓存；根目录默认值迁移为 HowRead，点行选目录后回写 `/data/storage/.../files` 并刷新。

**已知影响/提醒**：
- 标题栏"+"删除后，"从文件选择器手动导入书籍"不再有入口（文件可在我的文件里浏览直接打开；批量获取仍可用书库页的设备扫描）。如需保留手动导入，后续可在书库文件夹区提供双选项；
- 发现既有问题（非本次引入）：首页右下"继续阅读"悬浮按钮遮挡「网上书库」区"更多"链接的点击区，建议后续调整悬浮按钮位置或让首页滚到底部时隐藏。

**版本号** 0.9.6 → 0.9.7（versionCode 44 → 45）；鸿蒙只出 Pro 四件套（debug/release × arm64/x86_64），Android 本轮无改动。

**内部佐证（辅助）**：Index.ets（statCard 点击、sectionHeader 动作改向、subPage 整页子页 buildSubPage/buildRecentList/buildBookmarkList、TAB_ICONS、顶栏与书库 chips 删减、browseSectionHeader addKind 4/5/6、目录选择器 buildDirPickDialog/resolveFolderPath/folderPrefRow、fmtCount 计数、偏好四行移动与两组内嵌、initBrowseDir 持久化加载）；Servers.ets（LibFolder 持久化 getLibFolders/saveLibFolders）；Settings.ets（目录默认值与迁移）；LibrarySearch.ets（FORMAT_EXTS 导出）；app.json5（0.9.7/45）。

---

## [2026-09-18] 鸿蒙同步安卓「在线阅读设置」精简 + 修复鸿蒙引擎启动崩溃（0.9.6）

**做了什么（用户视角）**——把安卓端 2026-09-17「在线阅读设置调整」移植到鸿蒙版：

1. **在线阅读设置不再有重复开关**：原「仅 WiFi 预取」「计费网络允许整本缓存」两个语义重叠的开关合并为一个「仅 WiFi 下预取与整本缓存」：
   - 开（默认）：移动流量下不预取、不整本缓存，只随读随取（安卓侧"小书在流量下仍整本缓存"的特例一并取消）；
   - 关：移动网络与 WiFi 同等对待。开关下方附一句说明文字。
   - **鸿蒙版更进一步**：此前该开关只保存设置、不真正生效；本次把它真正接入在线阅读引擎——流量网络（蜂窝）下按开关拦截后台预取与整本缓存（探测失败一律按非流量处理，不影响正常使用）。
   - 旧设置自动迁移：之前开过「移动网络允许整本缓存」的，升级后合并开关自动为"关"（与安卓建议的手动操作等效）。
2. **重试更快**：弱网重连的基础等待间隔默认值从 1000ms 缩短为 100ms（指数退避不变）；已保存旧默认值 1000 的设备自动跟随新默认，手动改过其它数值的不受影响；重试间隔下限放开到 0（与安卓一致）。
3. **去掉「缓存过期天数」**：缓存不再按天过期（鸿蒙此前也未实现按天清理，界面移除该项即可）；设置页不再出现这两项旧配置。
4. **修复鸿蒙引擎启动崩溃（重要）**：近期共享引擎（MuPDF）为在线阅读做的优化更新到鸿蒙后，应用一启动就在打开 EPUB 等重排格式时闪退（EPUB/HTML/TXT 全部打不开）。定位为引擎两处问题：CSS 处理在"无用户样式"时未判空即解析（空指针崩溃）、一处安卓专用日志宏未加平台守卫导致鸿蒙编译不过。已修复源码并用**最新引擎源码**重新编译双 ABI——引擎保持最新优化版本、未回退。

**怎么验证**（Pura 90 模拟器，x86_64 debug 包实测，服务器 50.23 造场景）：
- 偏好 → 在线阅读与缓存：只剩一个「仅 WiFi 下预取与整本缓存」开关（带说明），无「计费网络」「缓存过期」两项，重试间隔显示 100；
- 迁移：注入旧版设置（移动整本=开、重试=1000）后升级安装 → 合并开关自动"关"、重试自动 100；开关切换后重启保持；新写入的设置不再含旧键；
- 在线阅读回归：TestNAS 打开 book_epub（整本缓存秒开）、book_bigepub.epub（92MB 流式 64 页、翻页零失败）、book_html/docx（分块拉取 1-2% 进度留存）；日志 `[RemoteBook] session open: metered=false, wifiOnly=true`；
- 引擎回归：启动自检 EPUB 105 页 + 全格式通过（demo.mobi FAILED 为既有已知现象）。

**版本号** 0.9.5 → 0.9.6（versionCode 43 → 44）；dist 双变体 8 件套。

**内部佐证（辅助）**：Settings.ets 合并迁移（LegacyRemoteSettings 读旧键、wifiOnly=旧值&&非旧计费开、retry 1000→100）；RemoteBook.ets isMeteredNetwork（connection bearerTypes 探测、fail-open）+ open() 每会话探测 + maybeStartFiller/prefetchAfter 门控；Index.ets 设置行与处理器清理；引擎 Builder/mupdf-1.23.7 html-parse.c 两处修复（user_css 判空守卫恢复 + __android_log_print 平台守卫），双 ABI 以修复后源码重建。

---

## [2026-09-17] 新增品牌信息速查文档 BRAND.md；确定鸿蒙平台只保留 HowRead Pro

**做了什么（用户视角）**：把两平台（Android / 鸿蒙）的包名、应用显示名、版本号、签名、产物命名等品牌信息，集中成仓库根目录的 `BRAND.md` 速查文档，作为开发、测试、发布时的统一参考——以后查"这个渠道包名是什么、装到设备叫什么名字、产物文件长什么样"只需看这一个文件。同时记录品牌决策：**鸿蒙平台正式渠道只保留 HowRead Pro（com.leestudio.howread.pro.reader.hmos）**；原 default 变体（HowRead，com.leestudio.howread.reader.hmos）的代码与构建能力保留不删除，但不再出包、不再纳入自动测试。Android 侧 pro / fdroid 双渠道维持不变。

**怎么验证**：打开仓库根 `BRAND.md`，逐项核对其中包名 / 显示名 / 版本与各平台配置一致（鸿蒙 `harmony/AppScope/app.json5`、Android `android/app/build.gradle` 与 `android/app/gradle.properties`）。

## [2026-09-17] 调整：在线阅读设置——两个重叠开关合一、重试默认提速、缓存不再按天过期

**做了什么 / 为什么**：

1. **合并重复开关**：原先在线阅读配置里"仅 WiFi 下预取与整本缓存"和"移动网络下也整本缓存"两个选项语义重叠、组合含义混乱，现合并为一个"仅 WiFi 下预取与整本缓存"开关：
   - 开（默认）：移动流量下不预取、不整本缓存，只随读随取当前需要的部分（原先小于 5MB 的小书在流量下仍会整本缓存的特例也一并取消，规则更清晰）；
   - 关：移动网络与 WiFi 同等对待，预取加速和整本缓存照常进行。
   - 之前开启过"移动网络下也整本缓存"的用户，现在把这个合并后的开关关掉即可得到同等效果。
2. **重试基础间隔默认 100ms**：弱网/断网重连的基础等待间隔默认值从 1000ms 缩短为 100ms（指数退避机制不变），网络抖动后恢复更快。已保存旧默认值 1000 的设备会自动跟随新默认；手动改过其他数值的保持用户设置不变。
3. **去掉"缓存过期天数"**：在线阅读缓存不再有时间限制，书籍缓存只要没超出容量上限就一直保留；空间不足时自动淘汰最久未使用的书（存新删旧），正在阅读的书不会被淘汰。

**怎么验证**：设置 → 偏好 → "在线阅读(Pro)"：对话框中只剩一个"仅 WiFi 下预取与整本缓存"开关，不再出现"移动网络下也整本缓存"和"缓存过期天数"两项，"重试基础间隔"显示 100。已在真机（MI9）实测：对话框项符合上述预期、旧值自动迁移为 100，且 WebDAV 在线打开书籍阅读正常。

## [2026-09-16] 鸿蒙同步安卓 WebDAV 空白修复：中文/空格路径打开列表空白（50.100 NAS 场景）+ 空目录提示（0.9.5）

**做了什么（用户视角）**——把安卓端 2026-09-14 同名修复移植到鸿蒙版：

1. **起始目录含中文或空格的 WebDAV 服务器能正常打开了**：此前这类服务器（如群晖 NAS 的 "Documents/05 其它"）点开后页面一片空白且无提示——原因是路径里的空格/中文被原样拼进请求地址，部分服务器对这种地址返回"成功但空"的列表。现在目录路径会自动按标准 URL 编码后再请求（逐段编码、保留斜杠），修复后文件正常列出、中文书名可直接在线打开；同样覆盖远程书扫描与浏览目录选起始目录两个入口。
2. **打开目录为空时不再无声空白**：云盘面板居中显示「目录为空或无访问权限」提示。
3. **WebDAV 请求留诊断日志**：每次列目录都在应用日志留一行（地址/状态码/条数），再遇"打开空白"可直接查日志定位。
4. **版本号** 0.9.4 → 0.9.5。

**如何验证**（Pura 90 模拟器，x86_64 debug 包实测，服务器 50.23 造场景）：
- 服务器建 "docs 中文 测试" 目录（含 book_txt.txt）并设为起始目录：服务器行点开 → 面板列出 book_txt.txt（修复前 0 项空白）；点开在线阅读正常；
- 起始目录指向空目录 "empty 空目录" → 显示「目录为空或无访问权限」；
- 起始目录清空回根目录 → 正常列出 4 目录 18 文件，中文目录名显示正确（回归）；
- 从中文起始目录扫描（行尾刷新按钮）→ 远程书籍出现 book_txt.txt（100% 缓存）；
- 日志验证：hilog 可见 `[WEBDAV] PROPFIND .../docs%20%E4%B8%AD.../ -> HTTP 207` 与条数记录。

**安卓侧不移植项**：抢占式 Basic 认证（鸿蒙每个请求本就携带凭据）；凭据失效自愈弹窗（鸿蒙服务器密码明文存储，无加密密钥失效场景）；尾斜杠重试（鸿蒙目录请求本就恒带尾斜杠）。

**内部佐证（辅助）**：Index.ets davEncodePath（webdavDirUrl/remoteScanServer 起点/davBrowseRefresh 四处应用）、davClassifyEntry 先容错解码 href 再做前缀匹配（apache 返回小写十六进制 %xx，与 encodeURIComponent 大写不一致，必须解码后比较）、云面板空态 Text；RemoteBook.ets webdavListDir 自引用过滤两侧统一解码 + 日志；strings dav_empty_hint。

---

## [2026-09-16] 手册定位修订：NAS 网盘访问 + 多平台定位 + 网盘书籍在线阅读 + 格式表纠偏 + 去包名 + 应用商店渠道

**做了什么（用户视角）**——按用户验收意见对《HowRead 用户使用手册》（v1.3.6，37 页）做第二轮修订：

1. **NAS 网盘访问**：功能简介新增「NAS 网盘访问」特色条目，第 8 章更名「网盘书籍在线阅读(NAS,Pro 特色)」，8.1 改「添加 NAS 网盘服务器」、8.2 改「网盘书籍直接在线打开」，全文强调 WebDAV / SMB / SFTP 多协议连接个人私有 NAS 网盘、书籍无需下载直接在线打开阅读。
2. **多平台定位**：去除全文的"Android 平台"限定表述——封面副标题与 §1.1 改为"开源多平台电子书阅读软件"，§1.3 系统要求表述为"安卓设备:Android 7.0+(API 24);更多平台(鸿蒙 / iOS / 桌面)规划中，陆续推出"，附录 A.4 改"多平台规划:安卓 / 鸿蒙 / iOS / 桌面全平台布局"。
3. **支持格式纠偏**（依据当前代码 BookType.java + 格式配置分组核实）：删除不支持的 PPT/PPTX、ODP、CHM、LIT、LRF、TCR、TEX、WPD；整行删除"压缩包直读(ZIP/RAR/7Z/TAR/GZ/ISO)"（RAR/7Z/ISO 均不支持）；格式表更新为 4 类——常用电子书（含 AZW4）、办公文档（DOC/DOCX、ODT、XPS）、漫画与图像（CBZ、CBR、TIFF、ZIP）、扫描文档（DJVU）。
4. **去除包名**：全册不再出现 com.leestudio.howread.* 包名；同步日志截图（图 12）中残留的旧包名路径以"com.howread.app"覆盖脱敏；删除"应用包名变更"提示段；下载表商店链接改为"play.google.com(搜索 HowRead)"形式。
5. **应用商店渠道**：下载渠道表新增首行"手机应用商店 | 小米应用商店、华为 AppGallery 等(陆续上架) | 在商店内搜索 HowRead 直接安装"，安装引导语同步更新。

**如何验证**：打开手册核对封面（无 Android 字样）、目录第 8 章新标题、§1.2 表 1 四行格式、§2.1 表 2 首行应用商店与无包名链接、第 8 章 NAS 表述；渲染验收 37/37 页通过，目录页码抽查全部与正文一致。

**内部佐证（辅助）**：格式清单核实自 `android/app/src/main/java/org/ebookdroid/BookType.java` 与 `PrefFragment2.formatEntries`；图 12 脱敏后重嵌入 word/media/image12.png；TOC 由 Word 刷新域重算。

---

## [2026-09-15] 用户手册更新至 v1.3.6：新增「在线阅读」一章 + 全册过时信息修正 + 6 张新功能真机截图

**做了什么（用户视角）**——《HowRead 用户使用手册》（`store/manual/HowRead用户手册.docx`）从 v1.0.0（9 月 6 日版）全面更新到当前 v1.3.6，手册与新版本功能对齐：

1. **新增第 8 章「在线阅读(三协议,Pro 特色)」**：完整讲解 WebDAV / SMB / SFTP 三类服务器的添加方法（主机+端口、共享名、私钥认证、起始目录、测试连接/浏览目录、信任主机勾选），在线打开的格式分级策略（PDF/EPUB 直开流式、MOBI 等高成本格式弹"整本取回"确认、DRM 书提示下载后打开），在线阅读设置（缓存上限、仅 WiFi 预取、重试、缓存过期、一键清空），网络书架（远程目录扫描入架、云朵角标+缓存百分比、断网离线阅读）与远程书管理。原第 8/9/10 章顺延为 9/10/11 章。
2. **各章增补新功能**：功能简介"四大特性"扩为"五大"；书库管理章补网络书云朵角标/长按远程书菜单/文件信息真实地址；阅读章补左缘亮度手势与侧边栏昼夜横幅；AI 配置章补 Key 明文回显；备份章补网络配置与 AI 配置的同步备份（逐条目合并、删除需两台设备分别删）；常见问题表新增"远程书打不开/断网读书/测试连接失败"三行；附录对比表新增"三协议在线阅读/同步备份补齐/阅读与界面细节"三行。
3. **修正过时信息**：封面版本 1.0.0→1.3.6、日期改为 2026-09-15；渠道由"三渠道"更正为双渠道（Pro 版 + F-Droid 版，标注包名 com.leestudio.howread.*）；下载表链接同步新包名；新增"旧包升级不迁移数据"提示；格式配置说明对齐新分组（MOBI/AZW/PRC 合并组、压缩包组默认不扫描）。
4. **6 张新功能真机截图**（华为 P30 实拍，图 15-20）：三协议服务器管理页、添加 SMB 对话框、SMB 远程目录、在线阅读设置、书库文件夹配置（勾选远程目录）、网络书云朵角标（100% 缓存）。涉及内网 IP 的截图已脱敏为 192.168.x.x。
5. **排版修缮**：42 个章节标题补"与下段同页"属性（消除两处标题孤悬页底）；目录由 Word 重新刷新域，页码与正文完全一致；表 1 类别"扫描与 Other"改为"扫描与其他格式"。

**如何验证**：打开 `store/manual/HowRead用户手册.docx`（38 页），核对目录第 8 章及图 15-20；PDF 预览副本在 `tmp/build/HowRead用户手册_v1.3.6.pdf`；上一版备份在 `tmp/build/HowRead用户手册_v1.0.0_backup.docx`。

**内部佐证（辅助）**：新增图 media 与关系 rId26-rId31；新章标题书签 _Toc100036-100041；渲染验收 38/38 页通过（图 15-20 无变形、脱敏到位、目录页码抽查一致）。

---

## [2026-09-15] 鸿蒙同步安卓 v1.3.4：三协议专属图标 + 远程地址可见 + 左缘亮度手势 + 格式组对齐（0.9.4）

**做了什么（用户视角）**——把安卓端 2026-09-14（v1.3.4）一轮体验优化同步到鸿蒙版：

1. **WebDAV / SMB / SFTP 有了各自的专属彩色图标**（此前统一用同一个云图标无法区分）：WebDAV = NAS 主机 + 蓝色地球角标、SFTP = NAS 主机 + 金色盾牌、SMB = Windows 风格共享文件夹。应用到「我的文件」服务器列表行与远程书籍列表行——远程书按所属协议显示对应图标，彩色原样不被主题色染色。
2. **远程书「文件信息」显示真实地址**：阅读中长按→文件信息对远程书显示完整真实地址（如 `http://192.168.50.23:8765/book_epub.epub`、`smb://主机/共享/路径`），不再是内部 `remote://…` 编码串。
3. **新增阅读时左缘亮度手势**（安卓同款）：阅读界面把手指按在屏幕左边缘竖直拖动即可调节亮度，拖动时屏幕上显示「亮度 NN%」气泡；工具行亮度条下方新增提示文字「划动屏幕左边缘调节亮度」。
4. **格式配置对齐**：MOBI 组更名「MOBI/AZW/PRC」并把 pdb 纳入该组开关；azw/azw3/prc/pdb 纳入设备扫描范围。安卓侧移除的「压缩包/其它」格式组鸿蒙本就不存在，无需改动。
5. **书库文件夹设置去掉「添加文件」入口**（书库按文件夹管理；顶栏「+」与「我的文件」底部「导入书籍」保留）。
6. **WebDAV 测试连接失败提示对齐中性文案**：「连接失败，请检查服务器地址与网络」。
7. **版本号** 0.9.3 → 0.9.4。

**如何验证**（Pura 90 模拟器，x86_64 debug 包实测）：
- 「我的文件」添加 WebDAV 服务器 → 列表行显示彩色 NAS+地球图标；对话框表单/测试连接/浏览目录/起始目录回归正常；
- 书库文件夹区标题行无「+」按钮；OPDS→SFTP→远程书籍→书库文件夹→搜索整页单滚动；
- 打开书 → 左缘竖直拖动：设备亮度实测变化（10→255）、拖动中「亮度 NN%」气泡完整显示、工具行亮度滑块同步；工具行下方提示文字灰色清晰可读。

**安卓侧不移植项**（鸿蒙形态不同/本就满足）：「我的文件」页两轮布局改造（鸿蒙本就是单一滚动区）；SMB/SFTP 测试连接文案中性化（鸿蒙文案本就中性）；书库设置「远程扫描目录」列表（鸿蒙无此对话框，扫描由服务器行按钮触发）；「我的文件」浏览列表隐藏不可打开格式（鸿蒙保留文件管理器全量显示）。

**内部佐证（辅助）**：media/my_nas_{webdav,smb,sftp}.png；Index.ets remoteProtoIcon/davRowSubtitle、browseSectionHeader(书库文件夹,0)；Reader.ets prettyRemoteUri/openFileInfo、左缘亮度条（PanGesture 竖直+气泡 1.5s）；LibrarySearch.ets FORMAT_EXTS mobi+pdb、BOOK_EXTS+azw/azw3/prc/pdb；FORMAT_ROWS mobi 行名。

**遗留（真机复验）**：上滑降亮在模拟器上 setValue 后系统回读值不跟随（模拟器亮度同步不精确，App 内状态与滑块已同步）；垂直滚动模式的左缘条与翻页模式同构未单独实测；SMB/SFTP 行图标同机制未单独实测。

---
## [2026-09-14] 修复：我的文件里部分 WebDAV 服务器打开后列表空白（50.100 NAS 场景）

**现象与原因（用户视角）**：
在"我的文件"里添加的 WebDAV 服务器，有的能正常打开浏览（如 50.23 测试服务器），有的（如群晖 NAS 50.100，起始目录为 Documents/05 其它 这类含中文和空格的路径）点击打开后页面一片空白，不报错也无内容。定位确认是三个问题叠加：
1. 起始目录里的空格和中文被原样拼进请求地址，群晖对这种地址返回"成功但空"的列表（而非报错），页面于是无声空白；
2. 服务器的账号密码是加密保存的，读取不到时（如更换锁屏密码导致密钥失效）应用会按匿名继续访问，按权限过滤内容的服务器同样返回空列表，用户完全无感知；
3. 服务器返回的空目录与"出错"在界面上无法区分，一律静默。

**改进（用起来有什么变化）**：
1. 含空格/中文的路径会自动按标准编码后再请求：50.100 NAS 的起始目录现在能正常列出全部文件，中文书名也能直接在线打开阅读；
2. 打开服务器时若"已配置账号但读取不到凭据"或"认证被拒"，会弹窗询问是否重新输入账号密码，确认后直接打开预填好的服务器编辑框，保存后自动重新加载——不再静默失败；
3. 打开的目录确实为空（或无权限）时，页面居中显示"目录为空或无访问权限"提示，不再无声空白；
4. 对不返回认证质询、按权限过滤内容的宽松型 WebDAV 服务器，现在第一个请求就携带账号密码（抢占式 Basic 认证），已保存的凭据不再"发不出去"；严格要求认证的服务器行为不变；
5. 个别服务器对无尾斜杠的目录地址不返回子项，现在会自动补斜杠重试一次；
6. 每次 WebDAV 请求都在后台日志留有一行记录（地址/是否携带凭据/结果条数或错误类型，adb logcat -s WEBDAV 可见），列表为空时另附一次原始响应抽样，以后再遇到"打开空白"一眼可查。

**验证**：MI9 自动化用例 13/13 全过（斜杠兜底、凭据自愈闭环、空目录提示、真实 50.100 NAS 列书 + 中文书在线打开、50.23 回归打开书籍）；P20 通过 V1/V3/V4 与 V2 的弹窗、对话框预填、保存、带凭据重载各环节；两台真机均无崩溃。

内部佐证：WebDavClient.java（encodeIfNeeded 路径编码 / 抢占式认证 / 诊断日志 / 空列表原始探针）、OpdsFragment2.java（凭据缺失检测、重新登录弹窗、空态提示、尾斜杠重试）、AddWebDavDialog.java（URL 保存前规范化）、WebDavStore.java（读取规范化）、RemoteSessionFactory.java（在线阅读地址编码）、fragment_opds2.xml 与三语言字符串。


## [2026-09-14] 「我的文件」页搜索区并入同一滚动 + 三协议测试连接失败提示中性化

**做了什么 / 为什么**（继续上一条布局反馈的进一步优化）：

1. **整页一体化滚动**：上一轮改造后页面上半部分（OPDS→书库文件夹）与底部搜索区
   仍是"两个区域、上面滑下面固定"。本轮把搜索区（搜索标题 + "在多个文档中搜索"行）
   直接渲染进同一个滚动容器，紧排在书库文件夹之后——**从 OPDS 到最底部的搜索项
   合成一个滚动区，不再分成上下两部分滑动**；条目少时一屏自然放下，多时整页滚动。
   原底部固定的搜索小容器已移除。
2. **测试连接失败提示中性化**：SMB / SFTP 添加对话框的"测试连接"失败时，
   此前复用了 WebDAV 的提示文案（显示"无法连接 WebDAV 服务器"，有协议残留）。
   现将该提示改为中性文案"连接失败，请检查服务器地址与网络"，三种协议统一；
   英文同步改为不含协议名，繁中文案补齐。认证失败的提示本就中性，保持不变。

**如何验证**（MI9 + P20 真机实测通过）：
- 「我的文件」根页：OPDS → WebDAV → SMB → SFTP → 书库文件夹 → 搜索区
  连成一页，轻扫即整页滚动（P20 条目多、内容溢出，滚到底可见搜索区，
  滚回顶可见 OPDS）；搜索区不再固定在底部；
- SMB / SFTP 添加对话框中填入不可达主机点"测试连接"：失败提示显示
  "连接失败，请检查服务器地址与网络"，无 WebDAV 字样；
- WebDAV 对话框认证失败提示"认证失败，请检查服务器地址和账号密码"，中性无残留；
- 回归：FN-30 远程书在线打开 PASS（73 秒，一次通过）。

**版本说明**：版本号未递增（仍为 1.3.4），已重新出齐 pro/fdroid × debug/release 全量包。


## [2026-09-14] 「我的文件」页 OPDS→书库文件夹整体自适应改造 + 自动化页码解析修正

**做了什么 / 为什么**（针对"我的文件页布局割裂"的使用体验反馈）：

1. **页面布局重排**：此前上半部分（网上书库 OPDS、WebDAV、SMB、SFTP、书库文件夹标题）
   装在一个限高 300dp 的小滚动窗里，添加的书库文件夹列表却在中间另一个独立容器——
   标题在上面滑、文件夹在下面固定，条目一多标题还会被折叠隐藏。
   现在书库文件夹列表行直接渲染在"书库文件夹"标题正下方，与网络区块共用同一个滚动容器，
   整体占满工具栏与底部搜索区之间的空间：**条目少时一屏自然放下（固定不滚），
   条目多时整块上下滑动**，搜索区始终钉在底部。
2. **操作行为保持不变**：点击文件夹行打开该文件夹浏览页、长按弹出"移出书库"确认框
   （只移出列表，不删磁盘文件）、"+"添加文件夹——与原卡片列表完全一致。
3. **子目录浏览不受影响**：进入文件夹后网络区块整块隐藏，文件列表恢复全高独立滚动，
   大目录浏览的性能与滚动体验和原来相同。
4. **自动化测试侧修正**：测试脚本读页码的"百分比格式换算"存在 +1 错位
   （应用中 4/5 页显示 80.0%，旧公式却算成第 5 页），导致 FN-30 打开即停在末页时
   "末页回翻兜底"永远检测不到页码变化而误报"翻页无效"。已修正换算。

**如何验证**（MI9 + P20 真机实测通过）：
- 「我的文件」根页：OPDS/WebDAV/SMB/SFTP/书库文件夹连成一屏自适应区块，
  文件夹行紧贴"书库文件夹"标题，搜索区钉在底部；
- 条目多时（P20 实测）整块上下滚动：顶部区块随滚动移出视区、下滑复位，
  不再出现"两个滚动区各滑各的"；
- 点击文件夹行打开浏览页、返回后根页结构恢复；长按文件夹行弹出移出确认（取消不删）；
  进入子目录浏览正常、返回正常；
- 回归：FN-30 远程书在线打开 PASS（53 秒，一次通过）。

**版本说明**：本轮为体验修正与测试修正，版本号未递增（仍为 1.3.4），
已重新出齐 pro/fdroid × debug/release 全量安装包。


## [2026-09-14] 三协议图标焕新 + 书库文件夹设置整理 + 远程路径可见 + 格式配置瘦身 + 阅读亮度操作提示（v1.3.4）

**做了什么 / 为什么**（围绕"网络书目更好认、书库设置更清爽、格式配置不忽悠"的一轮体验优化）：

1. **WebDAV / SMB / SFTP 有了各自的专属图标**（此前三者共用同一个云图标，无法区分）：
   WebDAV = NAS 主机 + 蓝色地球角标（跑在 HTTP/Web 之上）、SFTP = NAS 主机 + 金色盾牌角标（SSH 安全）、
   SMB = Windows 风格共享文件夹 + 下方连接节点。图标取自 docs/ 下的三张 logo 源图，
   已裁掉右下角水印（元宝 AI生成 / 昵图网 / ID 条）并去除白底转为透明。
   统一替换到四处：首页"我的文件"圆钮、"我的文件"页服务器列表、书库文件夹设置的远程目录行、WebDAV 浏览页。
2. **书库文件夹设置更清爽**（书库页右上刷新按钮弹出）：
   - 去掉"添加文件"入口（书库按文件夹管理；书库页"书库文件夹"菜单里的"添加文件"同步移除）；
   - "本地文件夹路径"标题与"+ 添加文件夹"合并为同一行，添加动作就在列表旁边。
3. **远程目录一眼可辨**：设置里的远程目录条目改为"IP/网址 · 起始目录"显示（不再显示别名和冗长 URL），
   行首配协议图标；书籍"文件信息"对远程书显示完整真实地址
   （如 `http://192.168.50.23:8765/book_epub.epub`、`smb://主机/共享/路径`），
   不再是内部 `remote://…` 编码路径。
4. **格式配置瘦身**（核查真实打开能力后移除"名不副实"的组）：
   - "压缩包"组整组移除（iso/gz/tar/7z/rar/dmg 等 22 个，应用实际都不能打开，点开只能转交外部应用）；
   - "其它"组整组移除（chm/lit/odp/ppt 等同样无打开实现；其中真实可开的 prc/pdb 并入 MOBI/AZW 组，
     组名相应改为 MOBI/AZW/PRC）；
   - 这些格式不再进入书库扫描与"我的文件"文件夹浏览；已入库的存量条目不受影响；
   - DJVU 维持现状（保留开关）。
5. **阅读亮度操作提示**：阅读界面偏好面板中，亮度调节条下方新增一行提示"划动屏幕左边缘调节亮度"，
   告知阅读时的左缘亮度手势。
6. **版本号**：1.3.2 → 1.3.4（小版本 +2；appCodeNumber 7302 → 7304 保持单调递增）。

**如何验证**（P20 装 pro 包实测通过，P30 装 fdroid 包冒烟通过）：
- 首页"我的文件"与"我的文件"页：三种协议条目各自显示新图标、彩色不被染色，夜间主题下正常；
- 书库页刷新 → 书库文件夹配置：本地标题与"+ 添加文件夹"同行、无"添加文件"、
  远程行显示"地址 · 起始目录"+协议图标；
- 长按远程书 → 文件信息：显示完整远程地址；
- 打开书 → 偏好：亮度条下方显示提示行；
- 偏好 → 格式配置：只剩 PDF/DJVU/FB2/MOBI/AZW/PRC/EPUB/XPS/DOC/RTF/ODT/CBZ/ZIP/HTML-TXT 等真实组。

## [2026-09-14] 鸿蒙同步安卓 2026-09-13 修复：远程缓存与文件名鲁棒性 + AI 模型列表

**做了什么（用户视角）**——把安卓端 2026-09-13 代码检视修复中鸿蒙侧同样存在的问题一并修掉：
1. **弱网下远程书本地副本不再被损坏**：重新组装（落地）远程书时原来直接清空旧文件再逐块写——中途断网，半截文件会被当好书打开。现在先写临时文件、完整后原子改名，旧副本永不被破坏；"整本已缓存"标记也改为落地成功后才置位。
2. **书名含 % 的文件不再导致列表失败**：WebDAV 目录解析对文件名做 URL 解码时原来一次不容错 + 一次重复解码——名字里带 %（如"进度50%.pdf"）会让整个目录列不出来。现在解码容错（解不动就原名显示）且只解一次。
3. **远程书缓存自愈**：重新打开缓存书时校验数据文件是否真的装得下位图声称的块，文件被截断/损坏时自动作废旧位图重新下载，不再渲染出空白页。
4. **AI"拉取模型列表"兼容 Gemini 风格**：模型名带 "models/" 前缀的会自动去掉，从列表选择的模型可直接使用。
5. **修掉删不掉最后一个标签的小 bug**：删除书籍最后一枚标签时会被"空值=保留旧值"的合并逻辑复活，改为直写列表。

**对使用者的影响**：远程阅读的弱网可靠性与离线缓存完整性提升；AI 模型列表对更多厂商可用。需重新安装 HAP 生效。

**内部佐证（辅助）**：RemoteBook.ets materializeBook 临时文件+renameSync、fetchWholeBookToCache fullyCached 后置、parseDavList davDecodeSafe 单次解码、loadOrCreate data.bin 长度校验；AiClient.ets listModels 前缀剥离；Tags.ets removeTagFromBook 改 replaceRecentBooks。

---

## [2026-09-14] 鸿蒙批次3补齐：书库标签管理 + 作者/系列/分类分组 + WebDAV 起始目录（0.9.3）

**做了什么（用户视角）**：
1. **书籍标签管理**：书库每一行新增"⋮"菜单按钮（网格视图封面上早有"⋮"，列表视图此前没有入口），菜单里新增「标签」项——打开标签编辑对话框：点已建标签即勾选/取消挂到当前书，底部输入框可新建标签。书库筛选区新增「标签」chip，点开对话框选某个标签即可只看带该标签的书（点"全部"恢复）。
2. **分组浏览**：筛选区新增「分组」chip，循环切换 分组关 → 作者 → 系列 → 分类；分组模式下书按所选字段归组，组头显示"组名 (本数)"，空值统一归入"未分类"，可与搜索/筛选叠加。
3. **WebDAV 服务器配置补齐 测试连接 / 浏览目录 / 起始目录**（对齐安卓 2026-09-13 同名功能）：添加/编辑 WebDAV 服务器对话框新增"起始目录（可选）"输入框与"测试连接""浏览目录"两个按钮——测试连接即时反馈连接成功/认证失败/无法连接；浏览目录可在服务器上逐级进文件夹，点"选此目录"自动回填。设置了起始目录的服务器，点击该服务器浏览与扫描都从该目录开始（留空=服务器根目录，老配置不受影响）。

**对使用者的影响**：书库多书管理更清晰（标签归类/筛选、按作者系列分类分组浏览）；WebDAV 配置从"只能盲填 URL"变为可测试、可浏览、可指定起始目录。

**验证**（模拟器 x86_64 debug，2026-09-14）：⋮ 菜单→标签→创建"novel"→自动勾选→关闭；标签 chip→筛:novel→列表 11→1→"全部"恢复；分组 chip→作者→组头"未分类 (10)"；WebDAV 服务器行点击连接浏览 50.23（Connected! 1 dirs, 16 files）回归正常。双变体 8 件套（HowRead/Pro × debug/release × arm64/x86_64）出包。表单类交互（起始目录回填/测试连接 toast 提示）因模拟器自动化输入法限制未 UI 实测，留真机手动验收。

**内部佐证（辅助）**：Index.ets LibRow ⋮ 按钮、libMenuTagsItem、buildTagDialog/buildTagFilterDialog、libGroupBy + getGroupedLibrary 分组渲染、davTestConnection/davBrowse 系列 + buildDavBrowseDialog、DavServer.startPath（getDavServers 字段级合并）；Tags.ets removeTagFromBook 改直写；Servers.ets/AiClient.ets 同步修改。
...
### 1. 修复：小屏手机「我的文件」里书库文件夹入口会整体消失（Android）
- **现象**：在小屏设备上添加若干 OPDS/WebDAV/SMB/SFTP 条目后，「我的文件」根页只剩网络条目，
  下方的书库文件夹卡片被挤出屏幕且无法滚动找回（真机实测卡片区域被挤成 0 高度），
  本地书籍目录再也进不去；屏幕越矮越容易触发。
- **原因**：根页布局给远程区块声明了高度上限，但系统 ScrollView 并不支持该属性（静默忽略），
  区块实际不限高，把下方文件夹列表挤没了。
- **修复**：新增 MaxHeightScrollView 让高度上限真正生效——远程区块最多占约三分之一屏且可滚动，
  书库文件夹列表始终保留可见空间。已实测小屏机上文件夹卡片高度从 0 恢复正常，浏览全部可用。
- **验证**：MI9 + 小屏华为真机全量回归；受影响的收藏/隐藏文件/网络添加等用例全部通过。

### 2. 排查辅助：侧边栏「夜间模式」切换增加日志埋点（Android）
- 针对反馈的「侧边栏与阅读器日夜切换几次后，侧边栏切换失败一直停在夜间」：
  在侧边栏切换逻辑处增加 logcat 埋点（tag=DayNight，记录点击判向与切换结果）。
- 本次在 MI9/P20/KSA 三台真机做了 11 次交替切换均未复现卡死（状态机审计也未发现卡死路径）。
  若再次遇到，请抓取 `adb logcat -s DayNight` 一并提供，可一次性定位是判向、保存还是同步回灌问题。
- 说明：阅读器内切换日夜只影响书页颜色，退出后外壳仍是侧边栏设置的主题，属设计行为。

### 3. 自动化测试：修复一批用例误报与环境脆性问题（ci/autotest）
- 真机回归从 91 通过/5 失败/12 跳过 的基线推进到当前状态，本轮修复覆盖：
  翻页模式/字号/笔记（工具条竞态改轮询）、阅读页锁定（按真实语义重写为开关冒烟）、
  页码格式/进度条位置（偏好弹窗改控件级定位，不再被弹窗拖拽/标签无监听坑）、
  设置备份导出（先展开「备份配置」折叠区；zip 按精确文件名查找，规避 Android find 缺陷）、
  SMB/SFTP 添加浏览（远程区块限高后改为滚动查找）、页码格式还原闭环
  （防止百分比格式泄漏给后续用例）等。
- 另修正测试自身的两类问题：浏览列表记住滚动位导致漏找书（先滚回顶部再找）、
  上一用例残留阅读器导致下一用例开错书（用例结束确保退出）。
- 已知保留项：FN-13 小屏标签入口（旧布局待勘探）、FN-47 门禁探针仅 fdroid 渠道执行（设计内）、
  FN-46 断网离线阅读归由在线阅读专项跟进。

## 2026-09-15

### 1. 修复：从「我的文件」打开远程目录后的返回与首页按钮行为（Android）
- **现象**：在「我的文件」里点开 WebDAV/SMB/SFTP 服务器后，页面左上角的返回箭头
  在服务器根目录点击没有任何反应（要进了子文件夹才起作用）；右上角的 HOME 图标
  点击会落到「网络」页面，而不是回到出发的「我的文件」页。
- **原因**：该远程页是覆盖在「我的文件」之上的独立页面，两个按钮没有适配
  "从我的文件打开"这一场景——返回箭头在"已到入口目录"时被设计为交给系统返回键
  关闭页面，但页内箭头丢弃了这个结果变成空操作；HOME 则被硬编码为跳转网络根。
- **修复**：从「我的文件」打开的远程页里——
  1) 返回箭头：在服务器入口目录点击 = 关闭本页回到「我的文件」；
     在子文件夹点击 = 逐级向上退，退到入口后再点即关闭；
  2) HOME：任何层级点击都直接关闭本页回到「我的文件」；
  3) 「网络」页本身的按钮行为保持不变（HOME 仍是回到网络根，供正常浏览网络书源）。
- **验证**：MI9 + P20 真机实测——入口层箭头关闭、子层箭头上退、任意层 HOME 关闭、
  系统返回键不受影响、「网络」页原有行为不变，全部通过。

### 2. 优化：打开远程目录不再先闪一下「网络页」样式的空页面（Android）
- **现象**：点开 WebDAV/SMB/SFTP 后，目录列表出来之前会短暂显示一个像「网络」页的
  空架子（标题是"目录"，列表是空的 OPDS/WebDAV 分区）；服务器慢时这个空页面停留更久，
  看起来像"先跳到网络页再进去"。
- **修复**：页面打开瞬间就直接显示目标服务器名称与加载进度，数据返回后无缝替换为
  目录内容——观感上就是"直接打开了该服务器的目录"，不再闪现网络页骨架。
- **验证**：MI9 高频连拍取证，打开过程全程只出现目标服务器标题，网络页骨架帧消失。

### 3. 测试：安卓模拟器(AVD)首次全量回归打通，并发现一个 Android 11+ 兼容性现象
- **做了什么**：在 MedicineAVD（Android 14 模拟器）上跑通了 54 例全量回归
  （31 过 / 15 失败 / 8 跳过）。为让模拟器能正常执行测试，修复了 4 处测试框架问题：
  新版安卓命令输出字段改名导致"书已打开却判定失败"的误报（14 处兼容修正）、
  开书等待窗口放宽（模拟器渲染慢，真机不受影响）、azw3 测试书本地文件名映射
  写错（间隔号·被写成星号）导致推书失败、模拟器系统语言切为中文（用例界面选择器
  以中文为主）。
- **值得关注的发现**：在 Android 14 模拟器上，应用「我的文件」页看不到本机文件树、
  书库扫不到 Download 里的书——系统安全策略（SELinux）禁止应用读取 /sdcard 链接，
  logcat 有明确证据。现有真机（Android 9~11）均不受影响，但 Android 11+ 新用户
  全新安装场景可能遇到同样问题，建议后续对浏览根路径做一次应用侧勘探适配。
- **如何验证**：结果报告 `ci/autotest/results/20260915-010245_L1_ui-avd/report.md`；
  模拟器上其余失败项均为模拟器环境怪癖（音量键/长按选词/分区点击/主题像素对比——
  同用例在 MI9/KSA 真机全部通过），三协议网络用例在模拟器上首次全过。
  测试坑点记入 ci/autotest/docs/PITFALLS.md 第 42~47 条。

## 2026-09-15（二）

### 1. 修复：WebDAV 同步的备份/恢复补齐——AI 大模型配置按厂商逐项备份，"我的文件"网络配置独立成文件逐项恢复（Android）
- **做了什么**：
  1. **AI 大模型配置**：同步备份文件 app-AI.json 升级为按模型厂商维度——每个添加的厂商
     （名称/协议/服务器地址/ApiKey/模型/最大Token/思考开关）作为一份独立配置项备份，
     并记录当前激活厂商；恢复时按厂商逐一恢复：某台设备上新增的厂商，同步后自动出现在
     其他设备，各厂商的 ApiKey 一并跟随，激活配置也随备份恢复（仅在设备尚未配置时采纳，
     不覆盖本地已选）。
  2. **"我的文件"网络配置**：新增专属备份文件 app-NetworkSources.json，把 网上书库(OPDS)、
     WebDAV、SMB、SFTP 与 书库文件夹 全部纳入；每个添加的条目是一个独立子项，且携带它的
     全部配置——例如一条 SFTP 配置含 主机/端口/用户名/名称/起始目录/信任主机开关 及其密码
     （SFTP 私钥口令一并备份）。添加了 N 条 SFTP，恢复时就原样恢复这 N 条（含原条目标识，
     与书内远程链接保持对应）。
  3. 同步/恢复均为**逐条目合并**：本地没有的条目补进来，已有的按较新内容更新；此前
     SMB/SFTP 完全不参与同步、AI 厂商列表只能整串覆盖的问题由此解决。app-State.json 中的
     AI 厂商列表改为设备本地保留（由 app-AI.json 按厂商接管），避免整串互相覆盖。
- **注意**：恢复按"条目并集"进行——在一台设备上删除的条目，若另一台设备的备份里仍有，
  下次同步会被补回来（如需彻底删除请在两台设备上分别删除）。
- **验证**（MI9 真机 + 50.23 测试服务器全闭环）：
  备份方向：配置 2 个 AI 厂商 + 2 条 SFTP（一条含密码）+ 2 条 WebDAV/SMB 后手动同步，
  服务器上的两个备份文件结构与条目全部符合预期（含密码字段）；
  恢复方向：清空本地的 SFTP 条目、AI 厂商并删除 SFTP 密码凭证后同步，2 条 SFTP（原条目
  标识与密码）与 2 个厂商（含 ApiKey）全部逐项恢复；恢复出的 SFTP 条目实际打开浏览成功
  （密码可用性实证）。FN-30 远程书在线打开回归通过。
- **版本**：适配验证完成后版本号 1.3.4 → 1.3.6（appCodeNumber 7304 → 7306）。


---

## [2026-09-15（三）] 三项修复：WebDAV 浏览列表混入应用同步目录 / 侧边栏夜间切换偶发"没生效" / AI Key 显示核实

**一、"我的文件 → WEBDAV"列表不再显示应用自己的同步目录（Android）**

- 用户视角：进入 WebDAV 服务器浏览时，列表里会混进应用同步功能生成的目录（如配置的同步目录 HowReadTest，以及它里面的 books、global 子目录），看起来就是"列表包含了它的子目录路径"，和自己的书混在一起。现在浏览 WebDAV 时会自动隐藏**当前配置的同步根目录**这一行，进入同步目录内部时也会隐藏其中的 books、global 两个内部目录；书和普通文件夹照常显示。
- 边界：只对"WebDAV 同步"配置的那台服务器生效，其他服务器上的同名文件夹不受影响；"同步远程目录"选择对话框不受影响（那里本来就要看到它）；其他服务器（SMB/SFTP/OPDS）不涉及。
- 佐证：`OpdsFragment2.withoutAppSyncDirs()`（列表过滤）。

**二、侧边栏"夜间模式"切换偶发不生效——修复（Android）**

- 用户视角：点抽屉底部的夜间/白天切换，偶尔界面毫无变化。两个根因都修了：
  1. **落盘竞态**（主因，与 9 月 4 日修过的"字号切不回去"同一机制）：切换后配置是异步写盘的，而界面立刻重启并重新读配置，重启偶尔抢在写盘完成之前，把**旧主题**又读了回来——表现为"点了没反应"。现在重启前先把所有排队中的配置写入**强制刷盘**（新增 `IO.awaitWrites()`，挂在所有"保存配置并重启主界面"的统一入口上，主题/语言/字号等路径一并受益）。
  2. **连点回翻**：重启需要数秒，期间再点一下会把刚保存的新主题**又翻回去**。现在 1.2 秒内的连点自动忽略，且正在关闭的旧界面上点击不再生效。
  3. 另加了兜底：个别机型上系统把重启意图投递给未销毁的旧实例时（singleTop 特性），旧实例现在会自动重建界面，保证新主题一定应用（设置页的主题切换同样受益）。
- 验证（MI9 真机）：连续切换 6 次，6/6 每次都成功换肤（截图像素校验）；快速双击只切换一次、不会翻回；logcat 确认第二次点击被忽略。
- 佐证：`IO.awaitWrites()`、`MainTabs2.applyDayNight()/startActivity()/onNewIntent()`。

**三、AI 大模型 KEY 显示明文？——核实结论：当前版本已是密文，为旧包现象**

- 逐台核实：KEY 输入框自 2026-09-05 起就是密码掩码显示（布局 `textPassword` + 代码双重设置），MI9 最新包实测显示为密文圆点（截图为证）；鸿蒙侧同样已用 `InputType.Password`。发现仍在跑旧包的 P30（1.3.2）后已随本次一并升级——现在三台真机（MI9/P20/P30）均为最新包，KEY 均为密文显示。若您在其他设备上仍看到明文，把该设备的应用升级到本次的包即可。

**构建与回归**：pro/fdroid × debug/release 四变体 BUILD SUCCESSFUL（版本保持 1.3.6，本轮未调版本号）；三台真机升级后无崩溃；FN-30（WebDAV 在线打开）回归 PASS。

**改动文件**：`android/app/src/main/java/com/foobnix/android/utils/IO.java`、`com/foobnix/ui2/MainTabs2.java`、`com/foobnix/ui2/fragment/OpdsFragment2.java`。鸿蒙工程零改动。


---

## [2026-09-15（四）] AI 大模型 KEY 改为明文显示（按需求）

- 用户视角：偏好设置 → AI 大模型接入 对话框里，KEY 输入框从圆点掩码改回**明文直接显示**，打开即可核对、修改，不用再逐位猜；输入过程中同样明文。其余字段与保存/测试连接逻辑不变。
- 说明：这是应用内"显示方式"的调整，Key 的存储与同步机制不变（本地仍是 AndroidKeyStore 加密存储，不进系统备份）。
- 佐证：`AiConfigDialog.java`（去掉 setInputType 密码类型）、`res/layout/dialog_ai_config.xml`（inputType 改 text）。
- 验证（MI9 真机）：打开对话框 KEY 完整明文回显（截屏 + 控件文本回读均为真实 Key 字符）；保存、测试连接正常。pro/fdroid × debug/release 四变体重新构建成功，MI9/P20/P30 已装最新包。鸿蒙工程零改动（鸿蒙侧 KEY 当前仍为掩码显示，如也需明文请告知）。


---

## [2026-09-15（五）] 远程网络书籍三项改进：文件信息显示真实大小 / 大书打开更稳 / 打开过程显示加载进度（Android）

**一、"书籍信息 → 文件信息"不再显示 0 B**

- 用户视角：书库里远程网络书籍的"文件信息"里，文件大小显示 0 B。原因是"大小"这一栏只在书籍成功打开过一次之后才回填，从没打开（或旧版本打开失败）的书就一直是 0。
- 现在：信息页直接显示真实大小；数据库里没有时，自动从该书的在线缓存元数据（meta.json）里取；拿到真实大小后还会回写数据库，书架各处显示保持一致。完全没有任何大小信息时不再显示误导性的 0 B。
- 验证：服务器新放一本 256MB 的测试书，浏览器列表/信息大小显示正确。

**二、两三百 MB 的大书打开"白屏一直加载中"——打开更稳、有出口**

- 根因分析：这类书是边下边读的流式打开。PDF 的关键索引（xref）在文件**末尾**，大书读到 80% 多时末尾恰恰还没缓存，每次打开都要临时从网络取末尾；网络慢或抖动时，这些读取串行叠加，就表现为白屏"加载中"不动（数据源本身有 15–30 秒超时与重试，属"很慢"而非死锁，但对用户来说就是打不开）。
- 改进：① 点击打开时，先在后台**预热文件末尾**（最后约 2MB），正文打开不再等末尾网络读取；② 网络读取的重试/超时机制保持不变，打开失败会弹"下载并打开"的兜底对话框（此前已具备）。
- 验证（MI9，清空缓存后打开 256MB 测试 PDF，500 页）：打开流畅、正常渲染首页，无崩溃。

**三、打开远程书全程显示加载进度（新增）**

- 在线流式打开：阅读器"加载中"对话框现在实时显示**已缓存百分比和字节数**（如"正在加载远程书籍 42%（105.3 MB / 256.0 MB）"），与书架看到的是同一个数字；超过 30 秒会提示可点 ✕ 取消后改用下载。
- 整本获取/下载路径（MOBI/DOC/DJVU 等需整本的格式，以及"下载并打开"）：新增进度对话框，实时显示"正在获取整本 x%（已下载 / 总大小）"，并带**取消**按钮——中途取消不残留半截文件，已缓存的字节保留，下次重试从断点继续。
- 验证（MI9，256MB 测试书）：进度对话框实时跳百分比；点取消立即中止、无残留、无崩溃；再次点击从已缓存字节续传，完整下载后正常打开。

**构建与回归**：pro/fdroid × debug/release 四变体 BUILD SUCCESSFUL（版本保持 1.3.6）；MI9/P20/P30 已装最新包；FN-30（WebDAV 在线打开）回归 PASS；全程无崩溃。鸿蒙工程零改动。

**改动文件**：`remote/BlockCacheStore.java`（+peekSize）、`remote/RemoteBookSession.java`（+warmTail 尾部预热）、`remote/RemoteBookOpener.java`（整本获取进度+取消、fmtMB）、`org/ebookdroid/ui/viewer/ViewerActivityController.java`（加载对话框实时百分比）、`pdf/info/widget/FileInformationDialog.java`（远程书真实大小）、strings（中/英/繁，4 条新文案）。


---

## [2026-09-15（六）] 侧边栏夜间按钮随日夜变色换图标 / 偏好页"在线阅读"与"在线阅读缓存"合并为一项

**一、侧边栏夜间模式按钮的图标和文字随模式联动**

- 用户视角：以前不管白天黑夜，抽屉底部那个按钮永远是"🌙 夜间模式"，切到夜间后也不知道当前状态。现在和阅读界面里的日夜切换一样：**白天模式显示月亮图标 + "夜间模式"**（点它进夜间），**夜间模式显示太阳图标 + "白天模式"**（点它回白天），切换后整个应用重建时按钮随之更新，当前处于哪个模式一眼可见。
- 验证（MI9）：白天态显示 🌙"夜间模式"，点击切到夜间后按钮变为 ☀"白天模式"；再点切回，图标文案随之还原。
- 佐证：`MainTabs2.buildDrawerBottomBar()`（按当前主题选 glyphicons_231_moon / 232_sun 与对应文案），新增字符串 `sun_day_mode`（白天模式 / Day mode）。

**二、偏好设置："在线阅读"与"在线阅读缓存"合并为一项"在线阅读"**

- 用户视角：偏好页"常规设置"里原来有两行——"在线阅读(Pro)"（显示已开启/已关闭）和"在线阅读缓存(Pro)"（显示缓存占用），点开的又是同一个设置窗口，显得重复。现在合并为一行 **"在线阅读(Pro)"**，值里同时显示开关状态和当前缓存占用（如"已开启 · 269MB"），点击打开原来的在线阅读设置窗口（开关、缓存上限、预取策略、清空缓存等都在里面，功能不变）。
- 验证（MI9）：偏好页只保留一行"在线阅读(Pro)"，值显示"已开启 · 269MB"，"在线阅读缓存"行已移除；点击打开设置窗口正常。
- 佐证：`PrefFragment2`（合并两行逻辑、refreshRemoteRow 显示状态+缓存占用）、`fragment_preferences.xml`（删除缓存行）。

**构建与回归**：pro/fdroid × debug/release 四变体 BUILD SUCCESSFUL（版本保持 1.3.6）；MI9/P20/P30 已装最新包，无崩溃。鸿蒙工程零改动。

**改动文件**：`ui2/MainTabs2.java`、`ui2/fragment/PrefFragment2.java`、`res/layout/fragment_preferences.xml`、`res/layout/drawer_bottom_bar.xml`（未改，图标在代码中动态切换）、strings（新增 sun_day_mode）。


---

## [2026-09-16] 书库一致性两项修复：删除远程服务器后配置/书库联动清理 + 搜索时清掉已不存在的书

**一、"我的文件"里删掉的远程服务器，不会再出现在"书库文件夹配置"里（Android）**

- 用户视角：以前在"我的文件"里删除了某个 WebDAV/SMB/SFTP 服务器后，书库页的"书库文件夹配置"里仍可能看到它（以及一些无法删除的乱码目录）。原因有二：① 配置同步会把删除的条目从备份里恢复回来；② 部分设备上存在历史遗留的**损坏目录数据**（某次旧版本把目录信息写成了 JSON 串，之后每次同步还会层层嵌套，越滚越多），这些条目在"我的文件"里看不到、在配置对话框里却一直显示。
- 现在：
  1. 删除服务器时留下**本地删除标记**，之后无论怎么同步，被删的服务器都不会在本机复活（在别的设备重新添加同一服务器不受影响；本机重新添加同一服务器会自动清掉标记）；
  2. 打开目录列表时**自动清洗损坏数据**：能从 JSON 串里恢复出真实服务器的直接修复（去重、落盘），恢复不出来的直接丢弃，且同步导入/合并时同样拦截，损坏数据从此不会再进配置；
  3. 删除服务器的同时，**书库里属于该服务器的远程书籍一并移除**（连同其在线缓存），不再留下打不开的死条目。
- 验证（KSA 真机）：升级后配置数据自动清洗为一条干净记录（`192.168.50.23:8765,HowRead-Test`），乱码目录消失；MI9 删除服务器后立即从列表消失，执行一次 WebDAV 同步后**未复活**，服务器端的同步备份数据也被清理干净。

**二、"书库文件夹配置 → 搜索"时，同步删除已不存在的书籍（Android）**

- 用户视角：书库里会残留已经不存在的书——本地文件删掉了、远程服务器删掉了、或者服务器上的文件被移走了——之前它们一直占着书架。现在点"搜索"后：
  1. 本地文件已删除的书不再重新入库，收藏里指向不存在文件的条目也不再回填；
  2. 已配置服务器的**整本扫描完成后**，服务器上已删除的文件会从书架中同步移除（扫描中途取消/失败不会误删）；
  3. 服务器已被删除的，其全部书籍随删除动作立即清掉（见上）。
- 验证（MI9 真机）：把服务器上的 book_azw.azw 移走后重新扫描，书库中该书条目被删除（10 行 → 9 行）；FN-30（WebDAV 在线打开）回归 PASS；P20 上的 8 条已删服务器残留书籍经一次"搜索"后全部清除。
- 说明：删除的书连同其在线缓存一起清理，不占用存储。

**构建与回归**：pro/fdroid × debug/release 四变体 BUILD SUCCESSFUL（版本保持 1.3.6）；MI9 / P20 / KSA 三台真机全部升级并验证，无崩溃（KSA 为 32 位 arm 包）。鸿蒙工程零改动。

**改动文件**：`remote/RemoteTombstones.java`（新增，删除标记）、`remote/RemoteLibraryCleaner.java`（新增，书库联动清理）、`webdav/WebDavStore.java`（损坏数据自清洗+去重）、`remote/RemoteStore.java`、`ui2/fragment/OpdsFragment2.java`、`ui2/fragment/BrowseFragment2.java`、`pdf/info/widget/PrefDialogs.java`、`remote/RemoteScanner.java`（扫描见集+修剪+完成后刷新书架）、`work/SearchAllBooksWorker.java`（收藏存在性过滤）、`webdav/WebDavSyncer.java`（同步后应用删除标记并重清洗）、`model/ProfileStateIO.java`（同步合并/导入拦截损坏与已删条目）。

---

## [2026-09-16（二）] 两项修复：WEBDAV 打开连接后列表混入起始目录本身 / 未打开过的远程书"文件信息"显示 0 B

**一、"我的文件 → WEBDAV"：打开连接后直接进入起始目录，列表不再混入起始目录本身（Android）**

- 用户视角：WEBDAV 里点开配置了起始目录的连接（如 NAS 上的 Documents/05 其它）后，列表里会出现起始目录自己（"05 其它"）这一行，点进去还是同样的列表，看起来像套娃。原因是这类服务器对含中文/空格的路径返回的条目地址是"未编码"的原始形态，而应用请求时用的是编码后的地址——两者做"隐藏当前目录自身"比对时对不上，起始目录那一行就漏进了列表。现在比对前先把地址解码成同一形态，起始目录（以及任意层级的当前目录）自身条目都会被正确隐藏。
- 验证（MI9 真机 + 您的 HowRead NAS 连接实测）：打开连接直接进入 Documents/05 其它，列表只显示里面的 14 本书（中文名、大小均正常），不再出现"05 其它"条目；日志确认列表请求成功、无异常。
- 佐证：`WebDavClient.isSelf()/decodedUrl()`。

**二、书库：没有打开过的远程书"文件信息"不再显示 0 B（Android）**

- 用户视角：书库里从没打开过的远程书，查看"文件信息"显示 0 B。三个环节都修了：
  1. **堵住清零源头**：书库本地全量扫描会把收藏的远程书也当本地文件跑一遍元数据更新，远程书没有本地文件，大小就被写成 0——现在扫描对远程书跳过这一步，扫描得来的大小不再被清掉；
  2. **扫描即回显**：远程目录扫描本来就能从服务器拿到每个文件的大小，现在连同大小文本一起写入书库记录，书架与文件信息直接显示真实大小（如 44.2 MB）；
  3. **查看时兜底**：仍查不到大小的老数据，打开"文件信息"时会自动向服务器查询一次真实大小（WebDAV/SMB/SFTP 三协议都支持），当场显示并写回书库，之后书架也不再是 0 B。
- 验证（MI9 真机）：远程 book_docx.docx 文件信息显示 96.51kB；原本 0 B 的 test_rtf.rtf 打开文件信息后自动向服务器查得 318 B 并显示，本地记录同步从 0/0B 修正为 318/318B；浏览、在线打开无回归、无崩溃。
- 佐证：`SearchAllBooksWorker`（扫描跳过远程）、`FileMetaCore.upadteBasicMeta()/reUpdateIfNeed()`（远程守卫）、`RemoteScanner.upsert()`（尺寸回显）、`RemoteStat`（新增三协议查大小）、`FileInformationDialog`（尺寸三级来源+回写）、`WebDavClient.fileSize()`。

**构建**：pro/fdroid × debug/release 四变体 BUILD SUCCESSFUL（版本保持 1.3.6）；MI9（arm64）安装验证。鸿蒙工程零改动。

## [2026-09-16（三）] 修复：书库里打开 WebDAV 书提示"文件已不可用"；大部头电子书现在秒开（流式阅读）

### 问题 1：书库里的 WebDAV 书，打开提示"文件已不可用"，实际文件还在
**现象**：在书库点一本 WebDAV 远程书，弹出"文件已不可用：'xxx.epub' 已被删除或移动"，但到服务器上看文件明明存在；SFTP 的书则一切正常。

**原因**：给 WebDAV 服务器配置了"起始目录"（比如 NAS 上的 `Documents/05 其它`）后，扫描书库和浏览点击在记录这本书时，把起始目录这段前缀丢掉了——记录的地址变成了"服务器根目录下的 xxx.epub"，而文件实际在起始目录里。打开时按错误地址去服务器取文件，服务器自然回答 404，应用就误判成"文件已被删除或移动"。SFTP 没有这个起始目录剥离逻辑，所以不受影响。

**修复后**：
- 书库扫描、浏览点击记录的地址都改为"相对服务器根目录的完整路径"，打开时能对上号；
- 书库里**已有的旧记录**不用重新扫描也能直接打开：打开时若发现按老地址找不到（404），会自动换成"起始目录 + 文件名"再试一次；
- 下次"更新书库"重新扫描后，旧的错误记录会被自动清掉，只保留正确的（收藏夹里的旧条目会保留，且同样能打开）；
- 书籍的"文件信息"里补查文件大小时也走同样的自动重试。

### 问题 2：打开 200-300MB 的大部头，缓存了几十 MB 也没进阅读器
**现象**：点开一本 245MB 的 epub（如《网络传播概论》），只看到一直在缓存下载，缓存了几十 MB 也没有打开文件。

**修复后**：epub/PDF 等支持在线阅读的格式，打开走"分块流式"路线——只取当前需要渲染的部分，**点击后不到 1 秒即可进入阅读器**（实测 MI9 经 NAS WebDAV 打开 245.7MB 的 epub：约 0.5 秒文档打开，后续翻页按需加载，不存在"先缓存几十 MB"的过程）。此前打不开的根源正是问题 1 的地址错误；地址修好后流式链路即恢复。

**附带加固**：如果某台服务器的 WebDAV 不支持分段下载（HTTP Range，物理上无法流式），打开超过 50MB 的书时会先弹出提示"该服务器不支持分段下载，需先完整缓存 xxx 才能打开"，由您决定是否继续，不再静默开始几百 MB 的下载。

### 验证（MI9 真机，NAS WebDAV 192.168.50.100:5005）
- 书库旧条目点击：自动重试后正常打开（日志可见 404 → 起始目录重试成功 → range 支持 → 阅读器启动）；
- "更新书库"重扫后：12 本书全部记录为完整路径（含起始目录），大小显示齐全，旧错误条目自动清理；
- 245.7MB epub：点击 → 0.5 秒文档打开 → 阅读器正常显示，后续按需读取（无整本下载、无下载进度框）；
- 全程无崩溃。

### 构建
- Android 四个变体（pro/fdroid × debug/release）v1.3.6 全部构建通过；鸿蒙端零改动。

## [2026-09-16（三）] 优化：打开大部头在线书时的等待体验（实时进度 + 加载中可返回）

### 现象
打开 2-3 百 MB、支持在线阅读的电子书（如 245MB 的教材 epub）时：
1. 加载框只有"请稍候..."，看不到任何缓存/加载进度；
2. 加载过程中按返回（或手势返回）退不出去，感觉卡死在加载页。

### 原因
这类大书首次打开时，排版引擎要读取大量内容才能画出第一屏（这本 245MB 的书首屏排版要读约 100-200MB 数据，局域网下要十几秒到几十秒）。等待期间：
1. 加载框是"通用加载框"，对远程书没有接入进度刷新——只显示静态的"请稍候..."；
2. 该加载框被设成不可取消，返回键被它吞掉，无法退出（只能点右上角小小的 ✕）。

### 修复后
1. **实时进度**：远程书加载时，加载框实时显示"正在加载远程书籍 已缓存 xx%（a / b）"（进度按已缓存字节/总大小算，每 0.6 秒刷新）；若暂读不到缓存信息则显示"已等待 N 秒"；等待超 30 秒追加"网络较慢，可点 ✕ 取消后改用下载后打开"提示。
2. **加载中可返回**：加载框现在响应返回键——按返回（含手势返回）等同于点 ✕：取消本次打开并退出阅读器，不会卡死。

### 验证（MI9 真机，NAS WebDAV 245.7MB epub）
- 清空缓存后冷打开：加载框从"0%（0 KB / 245.7 MB）"开始实时爬升到"59%（145 MB / 245.7 MB）…"；
- 加载过程中按返回：立即退回主界面，无崩溃、无残留；
- 二次打开（已有缓存）：进度秒级到位、快速进入阅读器。

### 构建
- Android 四个变体（pro/fdroid × debug/release）v1.3.6 全部构建通过；鸿蒙端零改动。

## [2026-09-16（三）] 优化：首次打开 2-3 百 MB 在线书——阅读器立即出现、排版转后台、剩余内容后台缓存

### 现象
第一次打开 2-3 百 MB、支持在线阅读的电子书（如 245MB 教材 epub）时，缓冲了几十 MB 也没显示出内容——因为排版引擎要把整本书读完才能画出第一屏，此前这个过程是"模态等待"（看不到阅读器、进度有限、期间返回也受影响）。

### 优化后
1. **阅读器立即出现**：点击打开后 ~1-2 秒内进入阅读器界面（标题栏/页码/进度条都在），不再被全屏加载框挡住；
2. **排版转后台**：屏幕上覆盖一层"正在排版远程书籍 已缓存 x%（a / b）"的实时进度（每 0.6 秒刷新，超 30 秒提示可点 ✕ 改用"下载后打开"）；排版在后台线程完成，**排版一结束就显示内容并自动回到上次读到的地方**；
3. **加载中可返回**：排版过程中按返回（含手势返回）立即退出，不会卡死（后台排版任务会在结束时自行清理资源）；
4. **剩余内容后台缓存**：打开的书会在后台把整本继续缓存（受总缓存 500MB 与单书 512MB 上限约束），这次 245MB 的书排版完成后已是"整本已缓存"，**下次打开秒开、断网也能读**；
5. **首次打开更快**：文本格式的下载分块由 256KB 提到 1MB（排版大多是顺序读取，网络请求数降约 4 倍）；并修复了"超过单书缓存上限后零散小读取会反复整块重下"导致的排版尾段极慢问题。

### 验证（MI9 真机，NAS WebDAV 245.7MB epub《网络传播概论》）
- 冷开（清空缓存）：点击 → 阅读器界面立即出现 + 进度浮层从 0% 实时爬升 → 排版完成（真实页数 266 页）→ 自动回到上次位置（155/266）→ 内容显示，全程约 20-25 秒（此前 60 秒以上且无反馈）；
- 排版过程中按返回：立即退回主界面，无崩溃、无残留；
- 排版完成后：整本已缓存（fullyCached），再开秒进、可离线。

### 构建
- Android 四个变体（pro/fdroid × debug/release）v1.3.6 全部构建通过；鸿蒙端零改动。
- 说明：epub/fb2 等流式文本的"第一屏"由排版引擎决定——引擎不支持"排一部分就出页"，所以首次打开仍需读完本书（现在全程有进度、可取消、阅读器可见）；再次打开因整本已缓存而秒开。

## [2026-09-16（三）] 优化：远程大部头 epub 在线阅读——缓存一小部分就开读，边读边后台补齐

**做了什么、为什么**
- 第一次打开大部头远程 epub（几十上百 MB）不再"缓存完/排完才出内容"：阅读器秒开，开头几 MB 数据到位就立刻显示正文、马上可以开始读；剩余章节在后台继续排版和缓存，页脚总页数逐步增长到真实值，期间翻页、返回都不受影响。之前要等整本缓存（245MB 的书要等 20 多秒的整书排版）才能看到第一个字，和"下载回来再打开"没有区别。
- 中途退出后再打开：进度遮罩只盖到后台排版"追上"你上次读到的位置为止（按上次记录的总页数估算落点，提前撤遮罩提前开读），剩余章节继续后台排。
- 修复本轮引入并当场发现的问题：延迟排版打开后阅读器"翻不了页"——翻页适配器的总页数在创建时被冻结为临时值 1，导致无论点击还是滑动都无法翻页；现已改为动态页数，点击翻页、解锁后滑动翻页均正常。
- 缓存"存新删旧"补全：正在读的书自身占用上限不再固定 512MB，改为不超过你设置的缓存总预算（默认 500MB）——在读书受保护不会被清理，只能靠自身上限兜底，防止一本大书把总预算顶穿；预算快满时依旧按"最久未读先删"清理旧书缓存，给新书腾地方。
- fb2/mobi/txt 等格式引擎必须读入整个文件才能解析，维持上一轮"秒开外壳 + 后台排版 + 实时进度"的体验；epub 因为是分章容器才具备本次的"边缓存边读"能力。

**用起来的变化**
- 大 epub 在线打开：从"等 20 多秒全缓存"变成"几秒出正文、后台静默补齐"；二次打开秒开且可离线。
- 阅读中的总页数、进度百分比会先显示部分值再长到真实值，属正常现象。

**如何验证（MI9 真机，88MB / 64 章合成教材 epub，WebDAV）**
- 冷缓存首开：点击后正文数秒内显示、全程无遮罩；页脚 10 → 70 → 128 页渐进长到真实值；冷缓存全书排版 5.3 秒完成。
- 翻页：点击翻页正常；解锁"页面拖动锁"后滑动翻页正常（文本书默认锁定拖动是既有产品行为，点底部锁图标解锁）。
- 后台缓存：打开十几秒后缓存元数据显示 fullyCached（88MB 全部落盘）；重开 0.6 秒排完全书并恢复到上次位置。
- 排版中按返回：约 1 秒退回书库，无卡死无崩溃。
- 版本 v1.3.7（appCode 7307），pro/fdroid × debug/release 四个安装包均产出。

---

## [2026-09-16] 大书远程在线阅读提速：单章扫描书“边下边读”（引擎级延迟图片加载）+ 打开过程体验优化（v1.3.8）

**做了什么（用户视角）**——解决“远程大书打开时一直显示缓存、迟迟看不到内容”的问题（以《网络传播概论》246MB 扫描版 epub 为例，此前冷开要先把整本 246MB 缓存完才开始排版，4-5 分钟甚至更久才出第一屏）：

1. **单章扫描书改为“边下边读”**：这类书整本只有一个章节、每页是一张扫描图。以前排版要把全部 372 张图下载下来才能算出有多少页、画出第一页；现在排版阶段只读取书的文字结构（几十 KB），**几秒内就能打开并显示内容**；之后翻到哪一页，就按需下载那一页的图（首次约 1 秒/页，之后秒开），后台继续把整本缓存完。打开大书再也不用等整本下完。
2. **打开中的“缓存中”遮罩更聪明**：遮罩只保留到“第一个能画出内容的分块”出现为止——多章节的书几秒内就进入阅读；有保存进度的书不再被遮罩挡住，而是**顶部一条小横幅提示“正在定位上次阅读位置”**，定位完成后自动跳回原位置（此时你已经在读了，横幅不挡任何操作）。
3. **回到上次阅读位置更准**：改用真实页数计算位置，字体/字号改过之后重新打开也能回到正确位置；定位过程中如果你自己翻了页，应用会尊重你的操作、不再强行跳转。
4. **遮罩增加“预计剩余时间”**：确实需要等待时（个别整本单章且无图结构的书）能直接看到还剩多久，而不是干等。
5. **修复打开大书偶发“打开后阅读器自动退出”**（引擎在渲染图片时回调 Java 被系统判定违规而终止进程，封面加载时触发）——现在稳定。
6. **版本号** v1.3.7 → v1.3.8（code 7308）；Android pro/fdroid × debug/release 四变体全部出包，鸿蒙侧 MuPDF 原生库同步重编（行为不变，仅随源码更新）。

**如何验证**（MI9 真机，WebDAV 远程书）：
- 清缓存后冷开《网络传播概论》（246MB 单章扫描书）：约 2 秒完成排版进入阅读器，首屏内容秒级出现（修复前需缓存完整本 246MB，4-5 分钟以上）；翻页时按需拉取对应页图片，后台持续缓存整本；
- 关书后重开：回到上次阅读位置（横幅提示后自动跳转，若在定位期间手动翻页则不打扰）；
- 多章节远程书（book_bigepub 88MB）冷开：首个分块 0.4 秒、逐章渐进排版、内容即时可见（回归通过）；
...
## 2026-09-17 v1.3.10（Round 13）：在线打开可靠性加固 + Phase 2（DOCX 文字版在线 / 扫描书图片嗅探 / RTF 评估）

### 做了什么（用户视角）
- **在线打开"静默无响应"修复（FN-30/FN-46 加固）**：此前极少数情况下点开网络书籍会没有任何弹窗、没有进度、也没有报错，一直停在列表页。排查发现四个互相叠加的"静默失败"点并全部修复：多个打开请求会排队互相卡住（改为并行执行，不再排队）；后台内存回收标记可能在打开瞬间把加载静默掐断（现在会留下日志并可恢复）；网络读块失败会被当成"文件结尾"喂给排版引擎导致报"文件损坏"（现在明确报错并走"下载整本"兜底）；进程被杀后残留的缓存块文件长度不校验可能读到空洞数据（现在打开前校验，不符自动重建缓存）。同时给整条打开链路加了设备可查的 REMOTE 日志，再出问题可凭日志直接定位。
- **DOCX 支持受限在线阅读（文字版）**：.docx 文件体积大头通常是图片，文字其实只有几十 KB。现在点开远程 docx 会先弹"在线打开（文字版）"，只提取文档文字（正文/样式/页眉页脚，图片跳过）并快速进入阅读器，不再先下载整本书；本地已留有副本时秒开，断网也能打开。文档超大（正文超 32MB）、加密（DRM）或结构特殊时自动转回原来的"整本下载"链路，体验与旧版一致。
- **扫描版 epub 图片"按需加载"扩展到无属性图片**：此前只有写了 width/height 属性的图片才能"排版不等图、翻到才取图"；大量扫描工具导出的 epub 图片不带属性，排版时仍会把所有图片下载完。现在引擎会读取图片文件头部（PNG/JPEG/GIF/BMP）解析出真实尺寸来占位排版，图片字节仍推迟到翻到该页才拉取。识别不出的格式保持原行为，排版结果与原来完全一致。本地大扫描 epub（≥100MB）同样受益。
- **修复一个潜伏 bug：ZIP 压缩条目解压失败**（上轮 1.3.9 引入的变体探测在使用真书时必现）：epub 的"变体体检"（DRM 识别、巨型单文件识别）读取压缩条目时用了错误的解压模式，导致对真实 epub（条目为压缩存储）探测必然失败并静默跳过——DRM 书会照常进阅读器然后打不开，而不是提前提示。本轮真机测试用真实压缩条目构造的书暴露了该问题，已修复并用"压缩条目 + DRM"的书复验：现在正确弹出"该书受 DRM 加密保护"提示。
- **测试框架证据链补齐**：自动测试失败时现在必定保存 logcat/UI 树/截图并检查崩溃记录（此前失败分支会静默跳过，现场证据缺失，正是 FN-30/46 一度无法定位的原因）。
- 版本 1.3.10（appCode 7310），pro/fdroid × debug/release 全量出包。RTF 流式解析经评估暂不实施（解析器本身已是单遍流式，瓶颈在排版引擎需整书就绪；详见工作区《RTF流式解析评估-v1.3.10.md》）。鸿蒙侧本轮引擎源码有更新，按约定暂不重编鸿蒙原生库。

### 如何验证（MI9 真机，50.23 测试服）
- FN-30（远程书在线打开）/ FN-46（断网离线阅读）自动用例：首轮与重启后复跑均 3 PASS / 0 FAIL；
- var2_docx_media.docx（25MB、其中 24MB 为图片 blob）：点开约 1 秒进入阅读器显示正文，日志显示只提取 1.4KB 文字部件、无整本下载；二次打开/断网打开走本地副本秒开；
- var2_epub_noattr.epub（12 章全部为无 width/height 属性的 800x600 PNG）：流式打开秒进阅读器，12 张图片全部从文件头嗅探出 800x600 并转为延迟加载（日志 img sniff ×12），翻页时图片正常渲染；
- var2_epub_drm_def.epub（压缩条目 + DRM）：正确弹出"加密书籍"提示（同时验证了 ZIP 解压修复）；
- 既有 var_* 变体书与本地书回归正常。

### 内部佐证（辅助）
- RemoteBookOpener 两个 AsyncTask 改 executeOnExecutor(THREAD_POOL_EXECUTOR) + 全链路 REMOTE 日志；RemoteBookSession.readAt 块读取失败抛 IOException；BlockCacheStore.open 校验 data.bin 长度；ViewerActivityController 取消门命中留日志；ExtUtils.showDocumentInner 启动前后留日志。
- 新增 RemoteDocxLite（远程 ZIP 中央目录定位 + 文字部件提取 + 绘图块剥离 + 轻量 docx 组装，32MB/48MB 上限，ZIP64/加密/异常一律回退整本下载）；openOrDownload 的 docx 分支；DocxContext 按源文件 mtime 失效转换缓存。
...
## 2026-09-19 v1.3.13 (appCode 7313) 在线阅读：小量缓存秒开 + 全链路可观测 + 稳定性

### 用起来有什么变化
1. **打开大书不再"缓冲几十 M 还不开"**：
   - 大 PDF 横屏打开不再等待整本解析，点开约 0.4 秒出现阅读器，内容边下边显（实测 531MB PDF，WebDAV）；
   - 竖屏与横屏节奏一致：加载框半秒即放行，顶部横幅持续显示"已缓存 X MB"进度，并常驻"整本下载"按钮，页面数据到齐自动渲染、横幅自动消失；
   - 远程 TXT 引擎级分块：253MB 的 TXT 只需先取头部约 256KB 就能开始阅读（此前必须整本下载完才能打开）。
2. **翻页更省流量**：PDF 类书的缓存块从 1MB 调整为 64KB——此前为读取散落在全文件的结构数据，每 8KB 的读取都要拖下整个 1MB 块（打开一本大书仅"数页数"就要多下 134MB）；现在散读只取实际需要的部分，顺序阅读由预取窗口补足，整书打开所需流量下降约 5-16 倍。
3. **断网/弱网可预期**：
   - 已缓存部分的书断网后也能打开阅读（打开时提示"离线模式：可读已缓存部分"），未缓存的页面留白；
   - 损坏的数据块会自动校验并重新下载（每块 CRC32 校验），书打不开的"永久坏缓存"问题根治；
   - 升级到本版本后，旧的远程缓存会在下次打开对应书时自动重建一次。
4. **退出/取消秒级生效**：关闭阅读器、取消下载时立即中断在途网络请求（此前一次卡住的网络读取会让界面死到只能强制停止）；整本下载进度框支持返回键取消。
5. **缓存管理更透明**：偏好里的"在线阅读(Pro)"对话框新增当前网络状态（WiFi/流量，流量下后台整本缓存自动停用）与容量水位显示；"清空缓存"修复了此前读过的书清不掉的问题，清理后数值即时刷新；大书可配置"阅读位置前/后各缓存多少比例"（默认前 20%、后 30%，流量下固定 10%/20%）。
6. **全链路时间线日志**：`logcat -s REMOTE` 现在输出统一的 [T+毫秒] 时间线（打开→书籍信息→收到缓存块→MuPDF 打开→首屏渲染），配合周期性的"块数/流量/耗时"统计，性能与功能问题可凭日志直接定位。

### 实测数据（小米12S / 小米9，WebDAV）
- 531MB PDF 冷缓存横向打开：T+0.4s 阅读器外壳、T+0.7s 内容展示；竖屏外壳 0.6s；打开全程仅取头部结构 + 当前页数据；
- 253MB TXT 冷缓存打开：1.3 秒进入阅读器（分块引擎）；
- 268MB PDF 冷缓存：T+97ms 书籍信息、T+123ms 首块到货、T+567ms MuPDF 打开完成、T+699ms 首屏渲染。

### 已知限制
- FB2 仍为整本解析（按 section 分块列二期）；MOBI/AZW/DJVU/CBR/DOC/RTF 属转换格式仍需整本下载；DOCX 在线为纯文本受限模式；
- 远程 TXT 暂无章节目录（与此前一致）；
- 自动化 FN-30/FN-46 回归通过（3 PASS / 0 FAIL）：阅读器翻页时在 REMOTE 标签打印 "page now X/N" 页码日志，测试脚手架改从该日志读取页码，MIUI 打断 uiautomator 导致 dump 无文本的场景不再影响用例判定。


### 追加（2026-09-19）：PDF 页树映射持久化——重开大 PDF 免走页树网络遍历
- **用起来有什么变化**：远程 PDF 第一次打开后，应用会把"页码 → 文件内部对象"的页树映射作为关键书籍信息保存到该书缓存旁（pagemap.bin）；再次打开同一本书时直接注入该映射，MuPDF 不再为定位页面在网络上做逐页树遍历。此前竖屏打开大 PDF 卡在"数页数"阶段（12S 上 531MB 书曾需约 28 秒才出首屏），现在重开时这一步零联网、秒级出内容。文件变更保护：缓存旁的映射带文件大小与版本标识双重校验，远端文件更新后旧映射自动作废重建，不会渲染错页。
- **修复过程中发现并解决的打包问题**：页树映射依赖的原生库函数此前一直没真正打进安装包——Gradle 的原生库来源配置为 `prebuilt/native/mupdf-1.23.7/`（app/build.gradle 的 jniLibs.srcDirs 指定，替代默认 jniLibs 目录），而重编后的 libMuPDF.so 只更新到了 `android/app/src/main/jniLibs/`（Gradle 不读这里）与 `Builder/jni/libs/`，导致运行时报 UnsatisfiedLinkError、映射保存静默失败。现已把 4 个 ABI 的新库同步发布到 prebuilt 目录并重出 pro/fdroid × debug/release 四个包，逐包校验新函数已包含在包内。
- **如何验证**：小米9 真机连跑两轮 FN-30（WebDAV 打开 book_pdf.pdf）：第一轮退出时日志出现 `page tree map saved: 5 pages`，第二轮打开时出现 `page tree map injected: 5 pages (tree walk skipped)` 且紧接着出页——保存→注入闭环成立；FN-30/FN-46 回归全部 PASS。大书（12S 531MB PDF）的注入收益待 12S 回连后复测。

### 追加（2026-09-19）：竖屏打开大 PDF 秒出首屏——每页宽高一次获取、随缓存持久化
- **用起来有什么变化**：竖屏（上下滚动）模式打开远程大 PDF，此前即使"页树免建"已生效也照样卡住——竖屏滚动布局需要每一页的宽高，旧逻辑对全书每页逐个发网络请求（995 页实测约 4 页/秒，要等数分钟），首屏渲染被压在最后，用户看到的就是壳出来了、内容一直等不到。现在每页宽高在建页树那一刻顺手一并读出（页对象当时已在手上，零额外流量），与页树映射一起存进该书缓存旁的 pagemap.bin（升级为 v2：映射+尺寸）；再次打开时两者一并注入，每页宽高从内存直读——竖屏打开全程（书籍信息→打开→布局→首屏渲染）不再有任何逐页网络请求。旧格式侧车（只有映射没有尺寸）自动作废，下一次打开重建（页面块通常已在缓存，秒级完成），之后永久秒开。
- **如何验证**：小米9 真机、NAS 上 133MB/995 页《中国消防手册》竖屏打开：修复前 5.5 秒仍无首屏（加载任务被取消）；修复后两次打开布局任务分别 T+387ms / T+371ms 完成，首屏位图 T+498ms / T+481ms 渲染，日志可见 "page tree map injected: 995 pages (tree walk skipped, sizes 995)"。PDF/CBZ/XPS 等逐页尺寸格式全部受益；epub/txt 等文本书无需逐页取尺寸，行为不变。FN-30、FN-46 回归通过。

### 追加（2026-09-19 下午）：修复缓存"存新删旧"误删刚建好的书（含页树侧车）
- **用起来有什么变化**：此前淘汰器按磁盘文件大小统计缓存占用，而缓存的数据文件是按整本大小预分配的——一本 609MB 的书即使只缓存了 8MB 也按 609MB 计账，导致总账瞬间"超限"，刚看完/刚建好页树信息的书几分钟内就被当"最久未用"整目录删掉（缓存块与页树侧车一起丢），用户再打开时一切重来。现在改为按**实际缓存的字节**统计（按缓存位图计数×块大小，忽略预分配占位），容量对话框的水位显示同步修正。小米12S 实测：清空缓存后冷开《中国大百科全书土木工程》(796页) 建树约 21 秒出首屏（首次打开的固定成本），退出即保存页树+尺寸；再次打开布局任务 T+260ms、首屏 T+347ms、零联网，缓存与侧车完好保留不再被误删。
- 另：页树侧车加载的各静默分支补充了诊断日志（文件缺失/魔法数/数量异常时在 REMOTE 标签写明原因），便于现场排查。

### 追加（2026-09-19 下午）：建树顺序预读——首次打开也进入秒开区间；定位日志完善
- **用起来有什么变化**：新缓存的书第一次打开时，页树建树此前要为每个页对象串行等一次网络往返（页数越多越慢，995 页实测约 21 秒，期间横屏白屏/竖屏一直显示缓存加载横幅）。现在打开后即以 4 条并行通道、每条 1MB 批量、4MB 窗口向前预读，且预读本身不再给前台让步（它就在关键路径上），建树从"逐页串行等待"变成"批量传输"：995 页、页对象散布全书的《中国消防手册》清空缓存后冷开首屏 **T+2034ms**（此前约 21 秒），重开 T+359ms；回归 FN-30/FN-46 全部 PASS。流量侧：建树期间拉取的块正是后续阅读要用的内容块，一次付出、翻页直接命中。
- **定位日志完善**（本轮排障中缺口逐项补齐）：① 建树本身此前完全不可见（只有海量 seek 行）——现在记录建树耗时与页数，随侧车保存日志输出（`walk 482ms`）；② 页尺寸缓存（PageCacheFile）命中/未命中/保存可见（此前竖屏布局为何 10ms 完成成谜）；③ 淘汰器删书从静默改为逐条日志（键名+释放量）——此前"缓存目录消失"排查了一小时；④ 预读窗口触发日志（`prefetch window [a,b) x4 lanes`）便于确认预读覆盖；⑤ 每次读/seek 的逐条日志默认关闭（此前一次冷开刷出数十万行），需要时 `adb shell setprop log.tag.REMOTE VERBOSE` 运行时打开，异常/慢读日志不受影响。
- **如何验证**：小米9 清空缓存 → WebDAV 目录冷开《中国消防手册 第十二卷》(133MB/995页)：首屏 T+2034ms、`walk 482ms`；重开注入 `sizes 995`、首屏 T+359ms。

### 追加（2026-09-19 傍晚）：封面探测不再拖慢打开；取消/返回秒级生效；远程书封面读后生成
- **用起来有什么变化**：此前在 WebDAV 目录里翻列表时，每一行都要"完整打开一次远程书"来生成封面缩略图，这些探测串行持有全局锁——实测打开下一本书时被占锁 43.3 秒，期间点取消/返回完全无响应（取消无法打断等锁），只能等锁释放后才退出；且列表滚得越多、书越大，锁占用越久。现在：
  1. **列表封面不再打开远程书**——未读过的远程书显示"书名/作者"占位封面；书第一次打开后自动把当前页渲染成封面并持久化保存（本地操作、零联网），此后列表直接显示真实封面；
  2. **取消/返回立即生效**——打开过程的全局锁等待改为可中断轮询，检测到取消即刻终止，不再出现"取消后白屏卡住几十秒"；
  3. **用户打开优先**——即使仍有后台探测在跑，用户的打开也被标记为优先，探测自动让路；
  4. 探测路径不再触发 4MB 预读窗口（上一轮预读改造的连带影响一并消除）。
- **如何验证**（小米9，清空缓存）：浏览 WebDAV 目录期间远程书打开次数为 0（此前 10+ 次）；冷开《中国消防手册》全局锁等待 0ms、首屏 T+2876ms、封面自动保存（RemoteCovers/xxx.jpg）；冷开《大百科全书》加载中按返回，阅读器 1.0 秒内关闭；FN-30/FN-46 回归 3 PASS。

### 追加（2026-09-19 晚）：换书空白页三连修复 + 预读窗口去重纠正 + 散布页对象书的量化结论
- **问题一（换书空白页）**：快速连续打开多本大书时，后一本的页尺寸布局会被"上一本书退出/广告页关闭时置位的取消标志"掐死（布局静默中止→空白→静默关闭）。修复：加载任务启动时清除上一次残留的取消标志；关闭路径在"有新书正在打开"时不再置位该标志（新增来源日志便于追溯）。
- **问题二（侧车被零填充数据污染）**：建树中途退出（或被打断）时，保存的页树映射/尺寸数组大部分是零——之后每次打开注入这份数据，MuPDF 报 "Page tree load failed. Falling back to slow lookup"，退回逐页慢速路径。修复：C 侧新增"建树完成"标志，只有建树完整结束（或映射完整注入）才允许导出；不完整的建树自动按"无侧车"处理，下次打开重建。
- **问题三（其他书的后台缓存拖慢新书建树）**：用户打开新书时，其他存活会话的整本填充/预读通道仍在抢占带宽（实测单流仅约 0.77MB/s）。修复：用户打开期间自动暂停其他会话的后台下载，新书建树独占带宽。
- **预读窗口去重纠正**：打开时读文件尾（xref）触发的预读窗口，把去重标记推到了文件尾部，导致建树在文件头部的所有预读触发被误判"已覆盖"——整个建树全程零预读、纯串行（796 页实测 55.7s）。已改为"仅当触发点落在当前窗口内才跳过"。
- **量化结论（小米9，《中国大百科全书土木工程》201MB/796页，页对象散布全书）**：这类书的建树要把散布全文件的页对象连同 64KB 块一起拉回，实测下载 188MB（约整本书的 93%），在约 3MB/s 的 WiFi→NAS 链路上冷开首屏约 60 秒；重开（有侧车+缓存）数百毫秒。页对象聚集的书（如消防手册）不受影响，冷开 2~3 秒。**要把"页对象散布"类书籍的首开也压到 2~3 秒，需要下一步做 MuPDF 惰性页树（按需只走当前页的树路径，全量建树转后台），这是引擎级改动，待确认后实施。**

### 追加（2026-09-19 晚）：MuPDF 惰性页树 + 竖屏渐进尺寸布局——散布型大书冷开首屏进入 3 秒级
- **用起来有什么变化**：页对象散布全文件的远程大 PDF（如 796 页的《中国大百科全书土木工程》），此前竖屏冷开要把全书 93%（188MB）经建树拉回才能出首屏（约 60 秒白屏）。现在打开后**只解析当前阅读页的树路径**（O(树高) 次取数），布局只取阅读窗口的真实页尺寸、其余页用阅读页的宽高比作占位——冷开首屏 **3.2 秒**（16 倍提升）；重开 0.39 秒（页尺寸缓存命中，零联网）。均匀尺寸的扫描书占位即真实，零校正；混合尺寸书在各页解码时就地修正（复用既有增量重排，修正范围仅该页及之后）。
- **实现要点**：① C 侧新增惰性模式标志——远程书打开且无完整映射时，`pdf_lookup_page_obj`/`pdf_lookup_page_number` 跳过全量建树，按需走"当前页树路径"解析与"/Parent 链"慢速页号查找（链接跳转低频场景可接受）；② Java 布局新增惰性尺寸模式：页数从 trailer 立即可得（1 次取数），阅读窗口（前 2 后 4 页）取真实尺寸并与页尺寸缓存文件融合保存（未知槽位容错续补）；③ 既有 initPages 对空布局结果未判空的问题一并修复。
- **如何验证**（小米9，清空缓存）：冷开《中国大百科全书土木工程》首屏 T+3239ms（`lazy page sizes: 796 pages (window 0..6)`）；重开 T+388ms（`page-size cache hit: 796 pages`）；FN-30/FN-46 回归 3 PASS。四变体（pro/fdroid × debug/release）已重出并逐包校验。
- **遗留**：页尺寸后台渐进补全（混合尺寸书的占位页随阅读逐步真实化）列为后续可选优化；12S 回连后装最新 release 包复测。

### 追加（2026-09-19 深夜）：MuPDF 惰性页树 + 竖屏渐进尺寸布局 + 页尺寸后台补全——散布型大书冷开首屏进入 3 秒级
- **用起来有什么变化**：页对象散布全文件的远程大 PDF（如 796 页的《中国大百科全书土木工程》），此前竖屏冷开要经建树拉回全书 93%（188MB）才能出首屏（约 60 秒白屏）。现在打开后只解析当前阅读页的树路径（MuPDF 新增惰性模式），竖屏布局只取阅读窗口（前 2 后 4 页）的真实页尺寸、其余页以阅读页宽高比作占位——冷开首屏 **3.2 秒**（16 倍提升）；首屏稳定后，后台低优先级任务按"先当前页之后、再当前页之前"的顺序逐页把占位尺寸替换成真实尺寸（视口下方页的增量重排不会引起滚动跳变；越视口页只更新数据不动布局），完成后页尺寸缓存存为全真实数据，**重开 0.4 秒**。均匀尺寸的扫描书占位即真实、零校正；混合尺寸书在各页解码时就地修正（复用既有 checkCropping 增量重排）。
- **实现要点**：① C 侧惰性模式：`pdf_lookup_page_obj`/`pdf_lookup_page_number` 在惰性标志下跳过全量建树，改走既有的单页树路径查找（pdf_lookup_page_loc，按 /Count 提示 O(树高)）与 /Parent 链慢速页号查找——上游本就内建这两条回退路径，稀疏映射天然安全；② Java 布局惰性尺寸模式：页数从 trailer 一次取数立即可得，阅读窗口真实尺寸 + PageCacheFile 部分内容融合（未知槽位容错续补）；③ 后台补全线程：分块让位全局锁、低优先级、进度日志（每 64 页）、阅读器关闭即取消；④ 顺带修复 initPages 对空布局结果未判空的问题。
- **如何验证**（小米9，清空缓存）：冷开《中国大百科全书土木工程》(796页) 首屏 T+3239ms；后台补全 `size completion done: fetched 796/796, layout fixed 752 in 35655ms`；重开首屏 T+406ms；FN-30/FN-46 回归 3 PASS；四变体重出并逐包校验。

### 追加（2026-09-19 深夜）：新远程 PDF 也能存出页树侧车——"重开零网络快开"链路真正打通

**用起来有什么变化**：远程 PDF 的"重开零网络快开"能力补齐了最后一块。此前一本**新**的远程 PDF 以惰性模式打开时，应用只解析当前阅读页的树路径、不做全量页树，导致"页码→文件内部对象"映射（侧车）永远存不出来——读完一本书，重开仍要联网走树定位页面，"重开零网络"的快开收益只对过去已完整建过树的老书生效。现在后台页尺寸补全任务完成后，若整本书的数据块已全部缓存，应用会在**全局锁内**自动补全整棵页树并落盘侧车（v2：映射+尺寸）；下一次打开起映射与尺寸一并注入，竖屏打开全程（书籍信息→打开→布局→首屏渲染）零逐页网络请求。
- **顺带修复的三个缺陷**（均经真机测试发现并修复）：① 关闭时并发崩溃——最初在阅读器关闭时做全量建树，建树与尺寸补全/渲染线程共用同一 MuPDF 上下文（非线程安全），真机触发原生崩溃；改为在尺寸补全完成后、全局锁内执行；② JNI 签名错配——Java 侧声明漏了文档句柄参数，原生侧从垃圾寄存器读句柄而崩溃，改为传入文档句柄；③ 收尾守卫误判——最初把"映射为空"当作"已建过"跳过收尾，而惰性模式下映射本就按设计为空，守卫改为仅在页树损坏时跳过。
- **如何验证**（小米9，清空缓存）：冷开 300 页测试书，后台补全约 5 秒完成，日志出现 `page tree map saved: 300 pages (sizes 300)`；重开日志出现 `page tree map injected: 300 pages (tree walk skipped, sizes 300)` + `page-size cache hit: 300 pages`，BENCH load-end 60ms（零联网）。大书（268MB/500 页）未整本缓存时侧车按设计延后（等整本缓存齐），页尺寸缓存文件兜底尺寸。
- **内部佐证**：MuPDF C 侧新增 `pdf_finish_lazy_page_tree`（惰性模式补全整树，仅页树损坏时跳过）；JNI `finishLazyPageTree(long handle)`；Java `MuPdfDocument.finishLazyTreeAndSaveSidecar`（全局锁内、整本缓存齐才落盘）；触发点为尺寸补全线程收尾处。

### 追加（2026-09-19 深夜）：在线阅读优化专项自动化测试用例（FN-55~FN-60 + PF-04）

**用起来有什么变化**：测试体系新增 7 条自动化用例，覆盖本轮在线阅读优化的完整链路——惰性页树、侧车保存/注入、后台尺寸补全、打开取消秒退、封面持久化、惰性布局页码正确性、首屏性能基线——今后每轮引擎/布局改动都能自动回归验证，不必再人工开书看日志。
- **新增用例**：FN-55 冷开惰性页树链路（断言日志链 sidecar not found → lazy page sizes → page-size cache saved，冷开 ≤3.5s）；FN-56 重开零网络快开（断言注入链路 + 零网络 + load-end ≤1s）；FN-57 后台尺寸补全+断点持久化（断言进度递增 + 补全完成 + 持久化命中）；FN-58 打开取消秒退（断言 ≤3s 退出 + 无误触发取消门）；FN-59 封面持久化（断言浏览不开书 + 开书后封面恰好生成 1 次 + 重开不再生成）；FN-60 惰性布局页码正确性（断言页码分母一致 + 滚到底可达 + 退出后补全线程停止）；PF-04 远程首屏性能基线（冷开 + 3 次重开各段耗时，取中位数）。
- **公共 helper**：把 FN-30/FN-46 共用的"打开远程书"流程抽成 `_open_remote_book`（两个既有用例改调用它，行为不变）；新增日志清空/日志抓取/缓存清理/BENCH 解析工具。
- **测试书**：测试服务器（192.168.50.23）新增 `book_lazy.pdf`（300 页/84KB，侧车保存-注入闭环用）与 `big_pdf.pdf`（268MB/500 页，惰性树/后台补全/性能基线用），投放 WebDAV/SMB/SFTP 三服务。
- **如何验证**（小米9，v1.3.13 debug pro）：FN-55~60 + FN-30/FN-46 合并跑 9 PASS（FN-16 为既有状态脆弱——"我的文件"列表滚动位未复位、Download 行在屏外找不到书，与本轮改动无关，有截图证据）；PF-04 PASS（冷开 load-end 190ms，3 次重开中位 56ms）。

## 2026-09-19 官网（docs）四项更新：更新日志刷新 / 渠道调整 / 品牌升级为 HowRead Pro

**做了什么**：官网做了四项面向访客的更新——
1. **更新日志页（更新日志/What is new）按真实版本号完整刷新**：从 0.9.0 到当前 1.3.13，每个实际发布过的版本单独一条（含日期与一句话主题），内容提炼成普通用户能看懂的要点（大书秒开、边下边读、断网可读、同步更可靠等），不再只写一句"当前版本 1.0.x"；从未发布过的版本号（1.3.1/1.3.3/1.3.5/1.3.12）不出现。旧版 Librera 的历史更新日志存档保持原样。
2. **下载区先不体现 Google Play 渠道**：首页、下载页（全部语言）、FAQ 页里的 Google Play 卡片/链接/文字已移除，F-Droid 与 GitHub Releases 渠道保留。
3. **品牌统一升级为 HowRead Pro（好好读）**：全站所有展示品牌名的地方（logo、页面标题、页脚、各语言首页、FAQ、隐私政策、贡献者页、在线阅读器页面等）由 "HowRead 好好读" 改为 "HowRead Pro 好好读"，英文站点同步为 "HowRead Pro"。
4. **版本号描述同步**：首页"当前版本"与下载卡版本号由 1.0.0 更新为 1.3.13；首页"更新日志"手风琴刷新为最近三条（v1.3.13 在线阅读提速、v1.3.7–1.3.9 大书边下边读、v1.3.0 SMB/SFTP）。

**影响**：访客看到的产品名称、版本号与各版本新功能介绍与安卓端实际发布情况一致；下载入口暂时只剩 F-Droid、GitHub Releases 和在线阅读器。

**验证**：脚本化全站复查——无残留 Google Play 链接、无遗漏加 Pro 的品牌串、首页/下载页版本号均为 1.3.13、更新日志页版本列表完整且跳号版本未出现；首页与手风琴数据 YAML 校验通过，Legacy 存档区未被改动。

## 2026-09-19 官网隐私政策改版：去包名显示 + 新增"华为平台"隐私声明

**做了什么**：官网隐私政策做了两项调整——
1. **不再显示包名**：隐私政策各处原来标注的"(com.howread.reader)"（旧包名）全部移除，页面标题只保留产品名"隐私政策 — HowRead Pro 好好读"。
2. **政策按"通用 / 华为平台"两部分重写**，并新增华为平台专属隐私声明：
   - **通用隐私声明（适用于所有版本）**：说明阅读数据本地处理、不上传；WebDAV 同步与 SMB/SFTP 在线阅读均直连用户自己的服务器；AI 功能仅向用户自行配置的服务地址发送所选文本；开发者不运营任何收集数据的服务器。另附渠道差异说明（F-Droid 版纯净无广告无 IAP）。
   - **华为平台隐私声明（华为应用市场 / AppGallery 渠道版本）**：基于对华为渠道实际集成的分析撰写——① 华为应用内支付（HMS IAP）：支付由华为账号/华为支付完成，支付信息不经过应用；应用仅接收购买结果（商品 ID、订单号、购买时间与状态），且只存本机不上传。② 华为广告（HMS Ads Kit）：横幅/插屏/激励视频由华为投放，SDK 会按其政策处理 OAID/AAID、设备与网络信息、广告交互数据，并附华为隐私声明链接与用户侧控制方式（系统设置限制个性化广告/重置 OAID）。③ 明确声明未接入华为分析（Analytics Kit）、推送与 AGConnect 云服务。
3. **页面结构简化**：原来"隐私政策"目录页 + 以旧包名命名的正文页两跳结构，合并为"隐私政策"单页直接呈现（中英文各一份），删除了两个以旧包名命名的页面文件；旧版 Librera 应用隐私政策存档链接保留。

**影响**：访客打开导航栏"隐私政策"即直达完整政策正文，看不到任何包名；华为应用市场渠道用户可以清楚了解华为支付与华为广告分别处理哪些数据、如何控制；政策口径与华为渠道的实际集成（IAP + 广告 SDK，无分析云服务）一致。

**验证**：脚本化复查——全站无"(com.howread.reader)"包名显示残留；隐私政策内链接目标全部存在；删除页面后无站内死链；中英文两份政策均含"通用"与"华为平台"两部分。

## 2026-09-20 官网隐私政策改为两份独立文档（通用 / 华为平台分开成文）

**做了什么**：按用户要求，把昨天合并在一页里的隐私声明拆成**两份独立文档**，隐私政策入口页只给出两个链接：
1. **《通用隐私政策》**（general / general.zh）：离线阅读本地处理、WebDAV 同步与 SMB/SFTP 在线阅读直连自建服务器、AI 功能仅发送到用户配置的服务地址、渠道差异（F-Droid 纯净）、日志数据/Cookie/儿童隐私等通用条款；页面顶部显著链接到华为平台声明。
2. **《华为平台隐私声明》**（huawei / huawei.zh）：单独成文，开头声明是对通用政策的补充、两者一并适用；正文三节——华为应用内支付（支付由华为完成、应用只接收购买结果且仅存本机）、华为广告（华为 SDK 按其政策处理 OAID/设备/网络/广告交互数据，附华为隐私声明链接与用户控制方式）、未接入的华为服务（无 Analytics Kit/推送/AGConnect 云服务）。
3. **隐私政策入口页**（导航栏"隐私政策"指向的页面）恢复为目录式：列出新两份文档的中英文链接 + 旧版 Librera 应用隐私政策存档；两份文档中英文各一份（共 4 个正文页），互相交叉引用，均标注更新日期 2026-09-20。

**影响**：华为应用市场上架时可以直接提交华为平台声明页的独立链接；通用政策页与华为声明页各自独立维护，互不掺杂。

**验证**：脚本化复查——入口页两个链接及全部站内链接目标文件存在；通用与华为页互相交叉引用正确；无包名显示残留。

## 2026-09-20 自动化测试按「易败用例分析」落地优化：「我的文件」导航自愈 + 阅读器页码读取兜底，9 个历史常败用例全部消除失败

**做了什么 / 为什么（用户视角）**

依据《自动化测试易败用例分析-20260919》的结论，对自动化测试脚本做了三处优化（**只改 ci/autotest 测试代码，不动应用**）：

1. **「我的文件」导航自愈**：9-18 根页改版后书库文件夹里没有 Download，一批"从文件列表开书"的用例（跳页/字号调节/阅读页锁定/页码格式/进度条位置/TTS 朗读等）反复"浏览找不到书"。现在脚本发现 Download 缺失时会**自动把它添加进书库文件夹**——走应用真实的「书库文件夹 → + 添加 → 添加文件夹 → 选中 Download → 选择」流程，配置落盘、一次添加长期有效，同时与测试框架"测试书放在 Download"的原始设计对齐（书库扫描也能收录测试书）。
2. **阅读器页码读取兜底**：intent 直开的书默认垂直滚动模式，页码控件不在屏幕上，音量键翻页/点按分区手势用例因此"读不到页码"。现在读不到时脚本会**单击屏幕中央调出阅读菜单栏**，从底栏读取"当前进度 / 总页数"（进度为百分比时自动换算回页码），任何阅读模式下都能取到进度。
3. **跳页用例改点 ✓ 确认**：MIUI 上"收起键盘的返回键"会把跳页对话框一起关掉，导致输入的页码落空、跳转不生效。改为填完页码后**直接点对话框右上角的 ✓ 按钮**，不再依赖输入法回车链路。

**顺带发现一个产品确认项**：「我的文件」9-18 改版后，文件夹子页工具栏有意精简（仅 返回/路径/排序/视图切换，无新建按钮），**"新建 txt 文件"入口在当前界面已不可达**。文件浏览用例的"新建文件"步骤改为 SKIP 并注明原因，待产品决定是否恢复入口或迁移位置；zip 直读验证不受影响。

**如何验证**（小米9，v1.3.13 debug pro，2026-09-20 00:24~00:56 共四轮）：

- FN-16 跳页（117.7s）、FN-18 字号调节（80.3s）、FN-34 阅读页锁定（68.8s）、FN-40 页码格式切换（144.0s）、FN-41 进度条位置（133.5s）、FN-07 TTS 朗读（148.1s）、FN-49 音量键翻页（38.6s，方向 上键回翻/下键前进 实测正确）、FN-51 点按分区手势（38.5s）——**8 个历史常败用例全部 PASS，且均为一次尝试通过**；
- FN-14 文件浏览：按预期 SKIP（新建入口缺失，注明产品确认项）；
- 报告与证据：`ci/autotest/results/20260920-00{2439,3738,3959,4135,5624}_L1_ui-device/`。

**内部佐证（辅助）**：`_ensure_library_folder`（书库文件夹幂等添加，含已存在检测与失败回落）、`_browse_dl_row` 插入第三级兜底、`_reader_page_no_back` 第⑤层 currentSeek/maxSeek 百分比换算、`_reader_page_ready` 读不到页码时强制中央 tap 翻转菜单栏、fn16 `goto_page` 改点 `onSearch` ✓ + 兜底直读对话框预填值、fn14 createFolder 缺失转 TestSkip。

## 2026-09-20 晚间 FN-14 文件浏览用例适配产品决策：新建 TXT 功能不再需要，用例改为"目录浏览 + zip 直读"

**做了什么 / 为什么（用户视角）**

- 按产品决策（新建 TXT 功能不再需要），把文件浏览自动化用例（FN-14）里"新建 txt 文件"的步骤**整体移除**，不再出现"入口不可达 → SKIP"的等待状态；
- 用例刷新为两步，覆盖保留：①**「我的文件」→ Download 目录浏览**（进目录后断言路径栏/列表加载，验证树形浏览能力，含书库文件夹自愈导航）；②**zip 压缩包直读**（原样保留：推送 zip → 打开出包内条目列表 → 点条目解压进阅读器 → 无崩溃退出）。

**如何验证**（小米9，v1.3.13 debug pro，2026-09-20 22:57）：FN-14 一次尝试 **PASS**（33.8s，此前为 SKIP）。报告：`ci/autotest/results/20260920-225758_L1_ui-device/`。

**内部佐证（辅助）**：fn14_browse_ops 的 create_txt 步骤替换为 browse_download（`recyclerView` 子项数 + 路径栏 `emulated` 面包屑断言，4 轮轮询）；open_zip 步骤未动。

## 2026-09-23 书签/笔记跳转落点修复（首页书签笔记 → 打开书位置不对）

- **现象**：在首页「书签笔记」分区点击某本书的书签/笔记条目，书打开后停留的是"上次阅读位置"，不是书签记录的位置；而在阅读页面内打开书签列表再点同一条目，可以准确跳到书签位置。
- **原因（两处叠加）**：
  1. 首页书签卡片点击打开**本地书**时，没有把书签记录的阅读位置（百分比）传给阅读器，阅读器只能按"上次阅读位置"打开（远程书分支原本就传了，行为正常）；
  2. 垂直（上下滚动）阅读模式下，"快速打开"优化只预先排版到上次阅读位置。带目标位置打开时目标页往往超出已排版范围，导致跳转落不准——而判断"本次打开是否带目标位置"的代码把浮点参数当字符串读取，永远读不到，这层保护从未生效过。横向（左右翻页）模式不受影响，这也是之前只有部分场景能复现的原因。
- **修复**：
  1. 首页书签卡片打开本地书时，按书签记录的位置打开（percent≤0 的首页书签仍按原逻辑，不改变行为）；
  2. 修正上述判断写法——带目标位置打开时先完成全文排版再精准落位。
- **影响范围**：首页「书签笔记」卡片、书签列表页、阅读笔记弹窗内的时间戳跳转、远程书按书签在线打开——所有"带位置开书"的入口在垂直模式下现在都能准确落到书签位置。
- **验证（MI9 真机）**：epub（219 页）以 50% 目标位置打开，精确落在第 110 页（预期 109.5→110）；mobi（273 页）以 95% 目标位置打开，精确落在第 259 页（预期 259.4→259）。两次均一次到位。
- 佐证：改动集中在 DashboardFragment2（首页书签卡片点击）与 ViewerActivityController（垂直模式开书的快速排版判断），共两处小改。


## 2026-09-23
### 自动化测试：历史常败用例根因修复，L1 全量迭代至 0 FAIL（AVD 先行 + 真机验证）

**做了什么**：针对 75 轮测试历史中反复失败的约 20 个用例（FN-03/07/14/16/17/22/28/32/33/34/39/40/41/42/43/55~60 等），逐一通过打桩日志、UI dump、截图取证定位根因并修复，全部修复落在自动化测试侧（ci/autotest/），应用功能代码零改动。

**为什么/根因**（三大类）：
1. **用例未跟上 v1.3.13 的产品交互与 UI 重构**：阅读器"单击翻页/双击菜单"重构后，用例的单击呼出菜单、菜单可见性判定全部失灵；书签添加改为对话框式（必须输入文字点确定才保存，旧流程等于丢弃）；顶栏页码/章节指示器换了控件 id（旧 chapter/pagesCountIndicator 已不存在）。
2. **用例前置条件不明确**：远程专项用例（FN-55~60）的日志链只在竖屏阅读模式存在，前置用例留下的横屏模式让它四台真机全误报；全新模拟器缺默认 TTS 引擎、缺网络路由、SELinux 拦 /sdcard 等环境项未声明。
3. **环境与状态依赖**：MIUI 的 ByteDownload 目录被 textContains("Download") 误中（整批用例进错文件夹）；浏览 Tab 停在子目录/列表惯性滚动/应用"继续阅读"晚到恢复等状态问题；模拟器 --reset 不清 BETA 配置导致旧远程条目凭据丢失后 401。

**怎么修的**（关键项）：菜单呼出改为多策略+按"菜单专属控件"判定可见性；FN-55~60 开书前显式钉竖屏模式、远程条目逐行尝试+凭据自愈；FN-03 补对话框文字输入与确定点击；FN-16/17 页码/章节判定切换到新 id 与全设备可见的 page now 日志通道；浏览导航只认精确"Download"、先归一回根页、列表滚动稳定后再点击、开书后校验书名开错自动重试；FN-42 等增加晚到阅读器恢复兜底；FN-39 多偏移点击；FN-56/60 按慢环境实测放宽等待窗/滑动上限。

**用起来有什么变化/影响**：
- L1 全量回归 60 例在 MI9 真机与 MedicineAVD 模拟器上双双达到 0 FAIL：MI9 全量确认轮 58 PASS + 1 SKIP（FN-47 设计性）；AVD 全量确认轮 57 PASS + 2 SKIP（FN-47 设计性 + FN-07 模拟器 TTS 引擎环境限制），两轮中各出现的 1~2 个末批时序波动（FN-16/40/43）经针对性加固后单点复验全部 PASS；对比历史基线 MI9 78.3%（09-19）/P20 68.3%（09-20），常败用例清零；
- 自动化现在可以在纯模拟器环境完成修复迭代（网络路由/SELinux/TTS/locale/模拟器串号漂移等环境项已脚本化自愈或在用例中声明前置），不再依赖真机排障；
- 用例文档（易败用例分析文档第六节）补全了每个用例的前置条件与测试参数（模式钉定、等待窗、滑动上限、服务器书位、TTS 引擎等）。

**如何验证**：
- `cd ci/autotest && python run_all.py --level L1 --serial 48fee174 --no-install`（真机 MI9）；
- `python run_all.py --level L1 --avd --serial emulator-5556 --reset`（模拟器全新态）；
- 预期：0 FAIL（FN-07 在引擎不可用的模拟器上按环境 SKIP、FN-47 在 pro 包按设计 SKIP，均不计失败）。
- 单用例复跑示例：`python run_all.py --level L1 --serial 48fee174 --cases FN-03,FN-16,FN-41,FN-55 --no-install`。

## 2026-09-23（晚）安卓自动化测试：虚拟机首次三段全量回归全绿；修复冷启动用例在安卓 14 模拟器上的误报

**做了什么**：在 MedicineAVD 模拟器（安卓 14/API 34）上首次完整跑完 L0+L1+L2 三段全量回归（共 73 例）。结果：L0 冒烟 8/8 全过；L1 功能 58 过 / 2 跳过 / 0 失败（跳过项为模拟器无 TTS 引擎的环境限制与仅限 fdroid 渠道的门禁探针，均属预期）；L2 性能 5/5 全过。三段合计 71 过 / 0 失败 / 2 跳过，满足门禁。

**为什么改**：首跑 L2 时「PF-01 冷启动耗时」误报失败（测得约 2.9 秒，超过 2 秒阈值）。经打桩取证，应用真实冷启动仅 1.0~1.4 秒——问题出在测试用例本身：安卓 14 系统打印的启动完成日志行比安卓 11 多了 "for user 0" 字样，用例的识别规则没有覆盖这种格式，识别不到时就退回到一种包含固定 2 秒等待的粗略计时法，必然超过阈值。此前模拟器门禁只跑 L1 功能段，这次跑全量才让这个隐藏缺口第一次暴露。

**用起来有什么变化**：对应用功能零影响（本轮未改任何应用代码，只修了测试用例的日志识别规则）。此后在安卓 12+ 的设备/模拟器上跑性能门禁，冷启动耗时会给出真实测量值，不再被误判为超阈值；自动化回归在虚拟机上的结果可与真机等同采信。

**如何验证**：`cd ci/autotest && python run_all.py --avd --level L2 --reset` → 5 过 / 0 失败（PF-01 冷启动中位数 1230ms，远低于 2000ms 阈值）；三段全量跑法为 L0、L1、L2 各执行一次 `run_all.py --avd --reset`。证据见 `ci/autotest/results/20260923-{194829,195213,205426,211217}_*`。

## 2026-09-24 首页/书库页"继续阅读"浮动图标支持拖动（仅安卓）

**做了什么**：首页和书库页右下角的"继续阅读"书本浮动图标（点击可打开最近在读的书）现在支持**按住拖动**：拖到屏幕任意位置松手即停在那里，拖到屏幕边缘会被自动收在边界内，不会拖丢或压到底部标签栏。默认位置不变（仍是右下角）；**位置不做记忆，每次重开应用都回到默认的右下角**；横竖屏切换后同样回到默认位置。单击打开最近这本书的原有功能完全不变（拖动与点击自动区分：手指位移小于约 10dp 判为点击）。

**为什么改**：用户需求——浮动图标默认在右下角，部分场景（如右手单手操作、遮挡列表内容）希望可以挪开，但又不需要长期记住位置，重开回到默认即可。

**用起来有什么变化**：对 pro / fdroid / 华为三个渠道同样生效（改动在共享代码里，无渠道差异）。应用的数据、设置均无任何变化；不拖动时行为与原来完全一致。仅安卓侧改动，鸿蒙侧保持原样。

**如何验证**：在首页或书库页按住右下角书本图标拖动 → 松手后停在松手位置；再拖到屏幕四角 → 图标完整留在屏幕内；轻点图标 → 正常打开最近在读的书；切换底部标签页 → 位置保持；退出并重开应用 → 图标回到右下角默认位置。已在安卓 14 模拟器与小米 9 真机（Pro 包 v1.3.13）按上述全链验证通过。版本号未升（保持 1.3.13/7311，随下次发版统一处理）。

## 2026-09-24 关于页五项调整；版本号锁定 1.3.11（仅安卓）

**做了什么**（「偏好 → 软件说明」关于对话框，五项全部按需求落地）：
1. **版本号锁定 1.3.11**：应用显示版本从 1.3.13 改回并锁定在 1.3.11（内部升级码保持只增不减，不影响已装用户的后续升级）；标题横幅同步显示 v1.3.11。
2. **支持和问题反馈**：原「支持邮箱」行改为「支持和问题反馈」，右侧显示 **HowRead Pro issues**，点击直接打开 GitHub 问题反馈页（https://github.com/380121850/howread/issues ），不再拉起邮件应用。
3. **开放源代码许可**：原「许可:GNU GPL v3.0」文案改为「开放源代码许可」，右侧「第三方库许可」链接保持不变。
4. **更新日志**：改为左标题「更新日志」、右侧显示 **好好读Pro v版本号**（如 好好读Pro v1.3.11），点击查看更新日志的行为不变。
5. **高级功能**：原「升级 Pro」改为左标题「高级功能」，未解锁时右侧显示 **解锁Pro高级功能**（点击进入 IAP 购买流程，华为渠道走华为 IAP、Play 渠道为购买入口、fdroid 跳官网），已解锁时显示「Pro 已激活」及渠道/订单信息，行为不变。

**为什么改**：用户需求——品牌化关于页（反馈走 GitHub issues、弱化 GPL 表述、突出版本与高级功能入口），并把展示版本先固定在 1.3.11。

**用起来有什么变化**：pro / fdroid / 华为三渠道同样生效；繁体中文界面同步更新（支援和問題回報/開放源代碼授權等）；其余语言关于区两行使用英文新文案。偏好页里的「软件说明」入口与旧版完全一致，只是内容按上面五项更新。

**如何验证**：偏好 → 软件说明：核对五行文案与右值；点「HowRead Pro issues」应打开浏览器进入 GitHub issues；点「解锁Pro高级功能」进入购买流程；点「好好读Pro v1.3.11」打开更新日志。已在安卓 14 模拟器（解锁态+锁定态两态截图实证）与小米 9 真机（Pro 包 v1.3.11）全部验证通过。

## 2026-09-24（续）关于页追加"开发者"行；标题列统一左对齐

**做了什么**：
1. **开发者行**：在「更新日志」上方新增一行，左标题「开发者」、右侧显示 **LeeStudio**（下划线链接），点击打开与「官网」相同的站点（https://380121850.github.io/howread/ ）；官网未配置时该行与官网行一起隐藏。繁体界面为「開發者」，英文为 Developer。
2. **标题列左对齐**：「高级功能」一行此前比其它行多缩进 5dip（Pro 卡片容器自带的左边距叠加导致），已去除，关于页全部六行标题（高级功能/开发者/更新日志/开放源代码许可/支持和问题反馈/官网）现在共用同一条左对齐基线。

**用起来有什么变化**：纯视觉与入口新增，功能行为不变；三渠道同样生效。

**如何验证**：偏好 → 软件说明：六行标题左边缘应完全对齐（实测模拟器与小米 9 真机逐行坐标极差 0 像素）；点「LeeStudio」应打开官网。注意：本轮修复过程中发现 NFS 编译机上 gradle 资源增量缓存可能漏检布局改动（改了不生效），已用"触碰源文件+清增量缓存"流程重出全部 6 组包，后续构建同样适用。

## 2026-09-24（三）关于页新增隐私政策与邮件行；官网文案与行序调整

**做了什么**（「偏好 → 软件说明」关于对话框，共 8 行，自上而下）：
1. **高级功能**
2. **开发者** → LeeStudio（官网链接）
3. **更新日志** → 好好读Pro v版本号
4. **隐私政策**（新增）→ 右侧显示 **好好读Pro 隐私政策**，点击打开官网隐私政策页（https://380121850.github.io/howread/PrivacyPolicy/zh.html ）
5. **开放源代码许可** → 第三方库许可（不变）
6. **支持和问题反馈** → HowRead Pro issues（不变）
7. **邮件**（新增）→ 右侧显示邮箱 **380121850@163.com**，点击唤起邮件应用（主题自动带应用名与版本）
8. **官网** → 右侧文案由「HowRead 好好读」改为 **好好读 Pro**，链接不变

**为什么改**：用户需求——上架合规需要关于页内可直达的隐私政策；补充邮件联系渠道；统一品牌文案与行序。

**用起来有什么变化**：pro / fdroid / 华为三渠道同样生效；繁体界面为「隱私政策／好好讀Pro 隱私政策／郵件」，英文为 Privacy Policy / HowRead Pro Privacy Policy / Email。标题列六行对齐保持不变。

**如何验证**：偏好 → 软件说明：按上面顺序核对 8 行与右值；点「好好读Pro 隐私政策」应打开浏览器进入隐私政策页；点邮箱地址应唤起邮件应用。已在安卓 14 模拟器与小米 9 真机（Pro 包 v1.3.11）逐项验证通过（行序坐标与文案全对、标题列极差 0 像素）。

## 2026-09-24（四）关于页「邮件」行标题改为「邮箱」

**做了什么**：关于页联系邮箱一行的标题由「邮件」改为「邮箱」（繁体「郵件」→「郵箱」，英文 Email 不变），右侧邮箱地址 380121850@163.com 与点击发邮件行为不变。

**如何验证**：偏好 → 软件说明，第七行标题显示「邮箱」。已在本号模拟器与小米 9 真机验证（8 行顺序与标题列对齐保持不变）。

## 2026-09-25 官网品牌名同步：去空格写法 HowReadPro / 好好读Pro

**做了什么**：安卓端应用显示名已改为去空格写法（HowReadPro / 好好读Pro，见 BRAND.md 2026-09-25 记录），官网全站同步：
1. 页面标题、logo、页脚版权、站点名（_config.yml）、各语言首页主标题、更新日志页、FAQ、下载页、在线阅读器页面、贡献者页等所有品牌展示处，由 "HowRead Pro 好好读" 统一改为 "HowReadPro 好好读Pro"；英文站点统一为 "HowReadPro"。共更新 32 个文件。
2. **隐私声明同步**：《通用隐私政策》与《华为平台隐私声明》（中英文共 4 份）的标题与正文中的产品名一并更新为去空格写法，条款内容不变。
3. 下载卡上的版本号无需改动——核实 gradle.properties 当前为版本锁 1.3.11（appCode 7313），网站各处已是 1.3.11，一致。

**影响**：访客在官网看到的品牌写法与应用桌面图标名称（HowReadPro / 好好读Pro）完全一致；APK 产物文件名（HowRead-Pro-v*.apk，带连字符）与仓库内代码标识不受影响。

**验证**：脚本化复查——全站无残留带空格的 "HowRead Pro" 写法；无遗漏加 Pro 的独立 "好好读" 品牌串；首页/隐私政策/导航栏等关键位置抽查确认；中英文首页 frontmatter YAML 校验通过。

## 2026-09-25 修复：删除当前 AI 厂商后其配置信息残留仍生效

**问题**：在「AI 大模型接入」配置里删除厂商后，该厂商的接入地址、模型、API Key 等信息仍然残留——更严重的是它们仍是**当前生效配置**（AI 笔记/翻译等功能会继续用已删除厂商的接口和密钥发请求），且配置表单里也还是旧值，此时若点「保存」会把已删除厂商的配置整套写回，等于删除失效。

**原因**：厂商列表条目与"当前生效配置"是两份数据，删除操作只清了列表条目和对话框选择状态，没有清全局生效字段和单独加密存储的 API Key。

**修复**：删除的厂商正是当前生效厂商时，同步清空全部生效配置（接入地址/模型/密钥/参数/激活名称）并复位对话框表单（与"添加新厂商"的空白状态一致）；删除的**不是**当前生效厂商时，其它厂商的配置不受任何影响。被删厂商的防同步复活墓碑机制保持不变。

**如何验证**：添加两个厂商 A、B 并激活 B → 删除 A：B 的配置原样保留；再删除 B：接入地址/模型/密钥全部清空、厂商标签显示"未添加"、AI 功能回到未配置状态，重开对话框为全新空白表单。已在安卓 14 模拟器与小米 9 真机（Pro 包）按完整链路自动化验证通过。

## 2026-09-26

### 修复：双语对照翻译翻到未翻译部分时闪退（小米 12S 实测定位）

**问题**：远程电子书开启"页内双语对照翻译"后，向前翻页进入尚未翻译的部分，应用随机闪退（真机崩溃日志：主线程 SIGSEGV，位于 MuPDF 页树导出函数）。同一本书时崩时不崩，双语重载刚发生时最容易触发。

**原因（用户视角）**：翻译完成后应用会"无感重载"书籍以显示新译文；重载瞬间应用会顺手保存一份"页码目录缓存"（用于下次秒开远程书）。这份缓存只对 PDF 有意义，但代码漏了判断：EPUB/MOBI/TXT 等文字版式的书也会走这段保存逻辑，导致把文字版式的内部数据当成 PDF 页目录去读，读到了无效内存——闪退。是否崩取决于当时内存里的恰好内容，所以表现为"随机"。

**修复**：
1. 文字版式（EPUB 等）一律跳过"页码目录缓存"的读取与保存（日志中出现 `page tree save skipped: text format` 即为该保护生效）；PDF 远程书的秒开缓存不受影响；
2. 顺带加固了书籍关闭/重载路径的并发保护（本次排查中先行落地的防护，保留）；
3. 原生库源码同步加固了 PDF 类型判别（下次原生库重编时生效，当前版本由第 1 条已完全覆盖）。

**影响范围**：仅安卓（鸿蒙侧未改动）。三渠道（pro/fdroid/huawei）debug+release 共 6 包已全部重新出包（15:2x），版本号保持 1.3.11。

**验证**：小米 9 真机（小米 12S 中途借出无法继续使用）：安装新包后打开同一本远程 EPUB（Hidden in Plain Sight），开启页内双语，连续向前翻页 18 轮穿越未翻译区域、期间多批段落翻译完成并触发无感重载——应用全程存活（崩溃缓冲 0 条），双语对照正常显示，保护日志命中 5 次（即此前必崩的 5 个保存点全部被安全拦截）。

## 2026-09-26 AI 配置存储重构：生效配置只认厂商指针；API KEY 改明文存储 + 界面打码

**做了什么**：
1. **生效配置只认指针**：AI 功能发请求时不再读取单独存放的"当前配置拷贝"，而是按「模型厂商」指针（aiConfigName）实时从厂商列表条目里取地址/密钥/模型——URL、API KEY、模型与厂商条目从此严格一一对应，不存在第二份会走样的拷贝。
2. **API KEY 改为明文存储**（在厂商条目内），配合同步导出时依旧整体剥离密钥的既有机制，密钥不出设备；同时移除了原来的 Keystore 加密全局密钥副本——旧结构里"厂商列表加密密文"与"全局密钥"两处不一致，正是此前"换设备/清数据后其它厂家 key 全部失效"的根源。
3. **界面打码**：配置对话框里 API Key 输入框默认以圆点打码显示，右侧新增「显示/隐藏」切换，防旁观泄漏。
4. **旧数据自动迁移**：升级后首次使用时，旧版加密密文（本机可解密的）自动解出为明文；更早期的"无厂商名"配置也会自动归入一个名为 default 的厂商条目，已有配置不丢失。
5. 同步兼容：旧版本同步文件顶层的密钥值会回填到激活厂商条目（仅当其没有密钥时）；旧扁平字段列为设备本地，不再参与同步。

**为什么改**：用户需求——URL/KEY 与厂商条目严格一一对应，删除"生效配置拷贝"这一容易和列表脱钩的冗余存储；同时 key 不再加密存储、改为界面打码。

**用起来有什么变化**：多厂商切换即切即用，某个厂商的 key 只存在于它自己的条目里；清除应用数据后厂商列表仍会保留（/sdcard），key 以明文随条目保留，**不会再出现"除了第一个厂家其它全部失效"**；新增/编辑/删除厂商的行为与之前一致。

**如何验证**：真机小米 9 完整链路——旧加密条目自动解出真实 key 并在保存后转为明文落盘；bigmodel 厂商「测试连接」真实网络成功；切到假地址厂商测连报网络错误（证明请求严格走激活条目自身配置）、再切回 bigmodel 测连恢复成功；偏好页 AI 行摘要显示激活厂商模型名；API Key 默认圆点打码、点「显示」可见。安卓 14 模拟器同步通过（新增/切换/明文落盘/删除清指针）。同步服务器侧 app-AI.json 仍不含任何密钥。

## 2026-09-26（二）修复：双语对照点「开始翻译」后无反应（远程书实测）

**问题**：缓存到 100% 的远程书籍开启双语对照翻译，点「开始翻译」后没有任何反应——功能完全没有启动。小米 9 真机复现实锤：开关没写入、界面不重建、日志为零。

**原因**：翻译面板的两个语言下拉框（源/目标）的默认选中项是在对话框首次布局**之前**设置的，触发安卓经典的 Spinner 回位缺陷——目标语言被静默重置回第一项「英文」，与源语言相同。于是每次点「开始翻译」都被"源语言与目标语言相同"的一闪而过的提示拦截后直接返回，看起来就是毫无反应。此前验证时人工点过语言下拉框，掩盖了这个默认值缺陷。

**修复**：语言选择改为在对话框布局完成后应用（目标语言默认中文、源语言按上次保存值），不再受该 Spinner 缺陷影响；不改变任何其它行为。

**用起来有什么变化**：远程书籍（缓存到 100%）和本地书籍一样：勾选「在页面内显示译文（双语对照）」→「开始翻译」即正常进入双语模式，页面出现对照译文；不再需要手动重新选择语言。小米 9 真机实测全链：离线基底自动组装（188KB）→ 双语版构建（778 段、已译 96 段）→ 双语版打开 → 会话挂载开始翻译。

## 2026-09-26（三）修复：远程书首次开启双语后会话不启动（HIDDEN IN PLAIN 实测）

**问题**：小米 9 真机上，一本 100% 缓存的远程英文书（Hidden in Plain Sight）开启页内双语对照后仍然"没有反应"——面板语言已正确（中文目标），书也重建打开了，但页面上始终不出现译文，翻页等两三分钟后依然如此，退出阅读后双语开关还会被自动关掉。

**原因**：首次开启时该书还没有任何已翻译段落，阅读器先以纯原文方式打开，双语会话在后台枚举全书段落准备翻译。这里存在一个时序竞态：会话挂载发生在书籍异步加载完成之前，拿到的页码/页数是无效的 (-1, 0)；等加载完成、真实的翻译窗口应该启动时，这次调用又被"枚举进行中"的并发保护丢掉了。两者叠加导致翻译队列永远是空的——会话饿死，界面自然毫无动静。退出阅读器时的安全逻辑再把双语模式关掉，进一步加重了"开了却没反应"的观感。

**修复**：段落枚举完成后，从当前阅读控制器重新同步真实的页码与页数再计算翻译窗口——无论挂载时序如何，可见页面立即进入后台翻译队列，译文完成后双语版自动换入显示。

**如何验证**：小米 9 真机实测该书：开启双语后日志立即出现翻译批次与段落完成记录，双语版构建并换入（741 页），页面上可见中文对照译文（截图确认）；偏好页/书内开关行为不变。另实测确认：该书的在线流式打开现在会被"该格式不支持在线阅读"对话框引导走下载打开，下载后双语功能正常。

## 2026-09-26（四）修复：双语会话找不到宿主导致部分书籍翻译不启动（小米 12S 实测）

**问题**：小米 12S 真机上，同一本 EPUB 远程书开启双语后翻译批次始终不启动（日志零记录）。此前已修的"翻译窗口饥饿"只解决了一半——真正的断点在更前面：双语版电子书打开后，阅读器的"当前书籍"路径变成了双语版临时文件，而双语会话注册在原始书籍路径下；阅读器每次刷新按当前书籍路径寻找会话时永远找不到，翻译队列因此永远是空的。

**修复**：双语会话的所有查找/注册入口统一做路径归一化（兼容 remote:// 与 remote:/ 两种形态），并让每个会话记住自己的宿主阅读器——按路径找不到时按宿主控制器兜底匹配。无论书籍以流式、下载副本还是双语版形式打开，会话都能被正确驱动。

**如何验证**：小米 12S 真机实测同一本 EPUB 远程书：开启双语后日志立即出现段落枚举（3133 段）→ 回同步到真实阅读位置（第 37 页/882 页双语版）→ 翻译批次按当前页窗口启动 → 页面渲染出英中对照译文（截图确认）。
