...
## [2026-09-13] 九项问题修复轮（远程阅读体验 + 首页/书架/统计，8 项落地 1 项按需搁置）

**做了什么（用户视角）**——本轮处理用户提出的 9 个问题（第 7 项按用户要求暂不动）：

1. **首页"我的文件"补齐 SMB/SFTP 条目**：此前首页快捷入口只显示 WebDAV 服务器和本地文件夹，配置过的 SMB/SFTP 服务器"看不见"。现在三类服务器都会按配置显示（云朵图标区分色），点击同样直达该服务器的浏览页。
2. **首页"今日阅读"不再把历史时间算进来**：根因是"今日"计数只在当天第一次阅读会话结束时才换日——午夜后到首次阅读结束前，卡片显示的是上一个有阅读日子的总时长；多设备同步还会把别的设备的"当日值"连同旧日期一起搬进来。现在换日发生在会话开始时、显示前校验日期键、同步合并只接受"今天"的数值。跨天的历史时长全部保留在"总阅读时间"的月/日柱状图里，不丢失。
3. **书库"刷新书库"支持勾选远程目录**：书库文件夹配置对话框顶部新增"远程目录 (Pro)"区块，把已配置的 WebDAV/SMB/SFTP 服务器逐行列出（服务器名 · 起始目录），由用户勾选，**默认全部不勾选**；勾选后点"搜索"，本地扫描照旧，被勾选的服务器在同一个进度框里依次扫描入架。勾选状态会记住，下次打开保持。一台服务器都没配置时该区块自动隐藏。
4. **远程书在阅读器里"属性"不再报"文件未找到"**：根因是远程路径放进 `File` 对象后 `remote://` 被系统塌缩成 `remote:/`，"…"菜单的入口校验认不出远程书，整个菜单（含属性）都被"文件未找到"挡住。现在入口校验兼容两种路径形态，远程书的属性改为展示服务器端信息：书名/作者/格式/大小/远程路径 + 删除按钮（本地书属性不变）。
5. **书架长按远程书与本地书菜单一致**：去掉"只弹删除"的旧捷径，远程书长按出现完整菜单——移出/加入书库、加入播放列表、标记已读/未读/在读、AI 简介、文件信息、删除直接可用；打开方式/发送文件/编辑/PDF 文本重排会先自动取回本地副本再执行（点一下→短暂下载→弹出分享/打开）；"上传到云盘"对远程书不适用已隐藏。删除仍只删书架记录和本地缓存，绝不动服务器原文件。
6. **WebDAV 配置补齐 测试连接 / 浏览目录 / 起始目录**（此前只有 SMB/SFTP 有）：配置对话框新增"起始目录（可选）"输入框和两个按钮——测试连接即时显示"连接成功/失败原因"；浏览目录在服务器上逐级选文件夹并自动回填。设置起始目录后，首页/网络页点击该服务器、书架扫描都从该目录开始（留空=服务器根目录，老配置完全不受影响）。
8. **格式设置的"压缩包/其它"列出全部具体格式**：两行开关的文字现在直接列出组内所有扩展名——"压缩包 (img/rar/7z/…/dmg 共23个)"、"其它 (abw/docm/…/prc/pdb 共28个)"，一眼看清范围；开关逻辑不变。
9. **远程书书签点击不再"文件未找到"**：书签保存时路径被系统塌缩、点击时又按本地文件校验，导致远程书签点不开（笔记因为是纯弹窗展示所以一直正常）。现在保存改存原始远程路径、点击兼容历史塌缩数据、远程书通过在线打开/取回副本直达书签位置（本地书签行为不变）；"仅显示可用的书"开关不再把远程书签藏掉；首页"书签笔记"卡片里的远程书签同样可点。

第 7 项（软件说明页 Pro 长按退款显示不回退）按用户要求**本轮不动**——现状是 v1.3.2 验证期的临时桩（pro 包恒视为已激活），退款开关写入的标志暂时无人读取，等真实计费接入时一并处理。

**对使用者的影响**：远程阅读（WebDAV/SMB/SFTP）的交互闭环补齐——从配置、浏览、扫描入架到属性/菜单/书签全部与本地书对齐；首页信息与阅读统计恢复可信。需重新安装 APK 生效。

**验证**（真机 P20=SNE-AL00 / P30=ELE-AL00，测试服务器 ubuntu22-test 50.23，2026-09-13 晚）：
- 构建：全量四包成功（pro/fdroid × debug/release，v1.3.2 全 ABI）；
- P20（配置了三协议服务器）逐项真机验证：①首页出现 HowRead-SMB/SFTP/HowRead-Test 三条目；②存储日期键为昨日时"今日阅读"显示 0 分钟、总时长 25 分钟不再串入；③刷新书库出现三行远程目录、默认不勾选、勾选 WebDAV 后扫描入架并记住勾选；④在线打开远程 PDF 后"…"菜单正常弹出、属性显示远程信息；⑤长按远程书菜单齐全、发送文件取回副本后弹出分享；⑥WebDAV 编辑对话框测试连接"连接成功"、浏览目录选中 /sub 自动回填、保存后浏览/扫描从 /sub 起始（验证后已还原配置并删除服务器测试目录），旧两字段配置兼容；⑧格式设置两行列出全部扩展名；⑨注入的远程书签在首页点击后在线打开并落点书签位置；
- 回归：本地书打开/长按菜单正常，logcat crash 缓冲区无本应用记录；
- P30 冒烟：安装启动正常、无崩溃、菜单与统计修复同样生效。

**内部佐证（辅助）**：DashboardFragment2（我的文件行/今日阅读/首页书签行）、ReadingStats+ProfileStateIO（换日与合并校验）、PrefDialogs+RemoteScanner.scanAll（远程目录勾选扫描）、ExtUtils（isValidFile/doifFileExists/openFile 兼容塌缩路径）、ShareDialog+FileInformationDialog（远程菜单与远程属性）、DefaultListeners+DocumentWrapperUI（长按与阅读器菜单入口）、BookmarksFragment2+BookmarksData+ListBoxHelper（书签存取与过滤）、WebDavServer/WebDavStore/AddWebDavDialog/RemoteDirPicker/OpdsFragment2/BrowseFragment2（WebDAV 三项能力与起始目录）、PrefFragment2（格式标签）、strings.xml 三语言（新增 moon_lib_remote_folders）。
...
**做了什么（用户视角）**：
- **回应「每轮测试前是否重装 APK、是否重置到默认状态」的检视**：结论是此前**没有**——L1 轮次从不装包（只有 L0 冒烟装），设备跑旧版本时整轮结果对的是旧代码且无任何告警；应用数据（书签/收藏/服务器条目/阅读进度/偏好）跨轮残留，用例靠各自的幂等逻辑兜底。本轮把前置条件做成框架能力：
  - **APK 版本一致性自动核对**：每轮执行前比对 APK 文件名版本号与设备已装版本，不一致自动升级安装 + 授权（此前靠人工核对时间戳才发现版本错位）。
  - **`--reset` 出厂态重置**：`pm clear` 清数据 + 重装 + 授权 + Pro 解锁自动写入 + 重推基准书，从干净状态起跑——**全量回归一律带 `--reset`**。代价是三协议凭据（WebDAV/SMB/SFTP）需一次性重新入库，网络用例首轮多约 3 分钟。
  - **`--no-install` 快速调试**：跳过安装仅告警，单用例调试用。
- **Pro 解锁自动化**（`--reset` 的关键配套）：清数据后 Pro 功能锁会挡住 FN-22/23/24/26~30/43/44 全部 Pro 用例，且该解锁键在 SDK 30 权限预授场景下应用首启永不落盘——框架现在自动预写解锁状态（debug 包 run-as 写 AppTemp.xml），无需手工解锁。
- **坑点 A 级 7 条规则写入 AGENTS.md**（按你「A 级全进」的裁决）：新增 3 条 gotcha（22 MIUI 密码框打崩 uiautomator→密码一律 ADB 键入；23 长跑息屏杀自动化→每用例入口唤醒解锁；24 冷启动丢内存态偏好→改设置后必须热态验证），另 4 条确认由既有 gotcha 16/19/21③/21⑤ 覆盖（adb push 中文乱码、resourceId 包名、set_text 回读、服务端日志第一现场）。详见 `ci/autotest/docs/PITFALLS.md` 第五节落地状态表。

**对使用者的影响**：以后全量回归直接 `python run_all.py --level L1 --reset` 即可保证「测的就是刚构建的包、且从出厂态起跑」，不再受上一轮残留状态或设备旧版本干扰；单用例调试用 `--no-install` 更快。AGENTS.md 规则扩充后，后续会话（含其它任务）做真机自动化时会自动遵守密码输入/息屏/内存态偏好这三条高危规避。

**验证**（MI9 真机 48fee174，Pro debug 1.3.2，2026-09-13）：
- `--no-install`：版本一致跳过安装，FN-48 PASS。
- `--reset`：pm clear → 重装 → Pro 解锁自动写入 → FN-48 PASS；**端到端**：`--reset` + FN-24（Pro 门控用例，阅读统计）26.1s PASS，证明解锁真实生效（未解锁时该用例会被 Pro 锁挡住）。
- 证据：`ci/autotest/results/20260913-134248_L1_ui-device/` 等轮次 report.md。

**内部佐证（辅助）**：
- `run_all.py` 新增 `prepare_device` / `ensure_pro_unlocked` / `apk_version`，worker 内 `run_level` 前调用；`--reset`/`--no-install` 互斥校验。
- Pro 解锁键落盘时机：`AppSP.save()` 仅两处调用（MainTabs2.onActivityResult 权限授予后 / AppProfile.save），SDK30 权限预授走 `Android6.checkPermissions` 已授权分支不触发 → pm clear 后键永不落盘，须预写（`Objects.loadFromSp` 通用反射加载，单键文件安全）。
- 设备侧写入转义坑：printf/sed 带引号写法在 adb→sh→run-as 多层转义下必坏，base64 解码写入 + 无引号 sed `/iapProUnlocked/s/false/true/` 为稳态写法（PITFALLS 24）。
- AGENTS.md：Known Gotchas 新增 22/23/24 三条。

---

## [2026-09-13] L1 自动测试覆盖扩展到 54 例：新增 6 个阅读手势/速读/更新日志用例 + 测试坑点清单文档 + FN-30/46 修复复验（纯测试侧，不改应用）

**做了什么（用户视角）**：
- **测试覆盖从 48 例扩展到 54 例**：新增 6 个用例，补齐此前未覆盖的阅读交互功能——
  - **FN-49 音量键翻页**：开 PDF 后按音量上/下键，页码应随之变化且两键方向相反（小屏设备方向反转也兼容）；
  - **FN-50 双击动作**：默认双击动作是"显示/隐藏界面"，双击屏幕中央后顶栏与页脚的组合状态应翻转；
  - **FN-51 点按分区手势**：点屏幕右分区翻下一页、左分区翻上一页，点中央切换界面显隐；
  - **FN-52 选词菜单**：长按正文应弹出选词菜单（书签/分享/复制/搜索/发送给AI/笔记/词典等项）；
  - **FN-53 速读 RSVP**：阅读器菜单→「快速阅读 (RSVP)」→词闪对话框（文字区+语速滑杆）应正常打开；
  - **FN-54 更新日志入口**：偏好→软件说明→更新日志，应能打开更新日志（本 fork 实现为外跳浏览器打开 CHANGES.md）。
- **新增测试坑点清单文档 `ci/autotest/docs/PITFALLS.md`**：把历轮真机测试遇到并解决的 23+ 个坑（MIUI 密码保险箱打崩自动化、EMUI 键盘遮按钮、屏幕息屏、u2 瞬断、书名位图无法定位、冷启动丢内存态偏好、adb push 中文乱码等）整理成"现象→根因→规避做法→是否建议升格为工作区规则"的表格，供后续维护者排障与规则沉淀参考。
- **FN-30/46 修复复验**：另一任务修复的"远程书在线打开无响应"问题，在本轮用 v1.3.2 构建复跑确认——**FN-30 连跑 3 次全过**（54.3s/55.1s/135.0s，第 3 次首试为 u2 瞬断自动重试通过，非应用问题），**FN-46 一次通过**（89.7s，在线打开→断网→最近阅读重开→翻页→恢复网络全链路）。两例由 FAIL 转 PASS。
- **测试框架加固**（长跑稳定性）：用例开头自动清理抢占前台的系统弹窗（华为"USB 连接方式"反复弹出）、阅读器残留任务抢前台时 back 兜底、MIUI 息屏重锁增加 MENU 键解锁、**设备侧加白名单防 MIUI 省电强杀**（`dumpsys deviceidle whitelist`，这是此前"阅读器静默退出"的根因——events 日志实锤 PowerKeeper 在前台时强杀应用）、阅读器存活校验+重开书重试。

**对使用者的影响**：回归测试更全面（54 例覆盖功能列表主要章节），阅读手势类功能（音量键/双击/分区点按/选词/速读）有了自动回归保障；测试侧发现并规避了 MIUI 省电强杀导致的"应用无故退出"误报（白名单后 3/3 稳定）。所有改动仅在 `ci/autotest/` 测试侧，**不涉及应用代码，无需重新构建 APP**。

**验证**（MI9 真机 48fee174 + KSA 真机 NETNU20617301956，Pro debug 1.3.2，2026-09-13）：
- **MI9 全 6 例新用例 PASS**（FN-53 31.8s）；FN-30/46 修复复验 PASS（见上）。
- **KSA（EMUI/Android9/32位）**：FN-50/52/53/54 PASS；**FN-49/51 FAIL**——音量键与分区 tap 在该机未触发翻页（页码指示器正常、配置项核对无误，属设备/ROM 差异，待 EMUI 低端机专项定位）。
- P20/P30 设备未在线，待补跑。
- 当前 MI9 基准：**49 PASS / 0 FAIL / 4 SKIP / 1 禁用**（通过率 90.7%，不含禁用项 92.5%）。
- 证据：`ci/autotest/results/20260913-124336_L1_ui-device/`（FN-30/46 复验）、`20260913-125033_L1_ui-device/`（FN-30 稳定性）等轮次 report.md + evidence。

**后续建议**：
- KSA 的 FN-49/51 输入未生效需 EMUI/Android9 低端机专项定位（音量键监听与分区点击在该 ROM 上的行为差异）。
- PITFALLS.md 末尾附了"建议升格为 AGENTS.md 规则"分级（A 强烈建议/B 可选/C 仅测试侧），由用户裁决哪些进规则文件。
- P20/P30 上线后补跑 FN-49~54 与 FN-30/46。

---

## [2026-09-13] 修复远程书在线打开无响应（书本模式下必现）+ 远程书纳入「最近阅读」（FN-30/FN-46 转绿）

**做了什么（用户视角）**：
- **修复「点远程书（WebDAV/SMB/SFTP 里的书）完全没反应」的应用缺陷**。真根因：应用判断"文件是否有效"时，远程书路径（remote://）会被误判为无效文件而**静默放弃**——只有竖排/滚动模式带了对远程书的豁免，**书本模式（左右翻页）下远程书 100% 打不开**（仅闪一下"找不到文件"提示）。这解释了此前 FN-30"时好时坏"：同一 APK 昨天能过、今天连挂，取决于当时的阅读模式与入口路径，并非代码回归。
- **远程书现在会进入首页「最近阅读」**：之前远程书读完不会出现在最近阅读里（内部把它当"不存在的本地文件"过滤掉了），断网后想从最近阅读重开根本没有入口。修复了读取记录与首页展示两处的过滤，远程书按最新时间正常显示、点击可用分块缓存离线重开。
- **配套健壮性修复（消除其它"点了没反应"的隐患）**：远程打开的网络任务改为并行执行，不再被卡住的下载任务静默排队阻塞；块读取失败不再被当成"读到文件末尾"静默吞掉（会走"打开失败→是否下载"对话框）；块缓存复用前校验数据文件完整性，损坏自动重建；加载被系统取消时不再无痕退出（留日志）；远程打开全链路增加设备可见日志（logcat 过滤 `REMOTE` 即可），以后再出问题可直接抓日志定位。
- **测试侧配套（ci/autotest）**：失败时自动保存 logcat（此前保存功能实际失效，44 轮运行零现场日志）+ 失败分支补 crash 扫描；兼容"打开应用继续阅读"特性（冷启动阅读器盖住主界面导致入口检测偶发失败）；首页最近阅读封面项无文字标签、书停在最后一页向前翻页无效等场景的用例适配。

**对使用者的影响**：书本模式下可以正常在线读远程书；远程书读完会出现在首页最近阅读，断网后可从那里直接重开（离线分块缓存）；"点书没反应"类问题从静默变为有提示、有日志可查。

**验证**（MI9 真机 48fee174，Pro debug 1.3.2，2026-09-12~13）：
- 修复后 **FN-30 连续 3 轮 PASS**（56.8s/59.5s/重启后 125.4s），**FN-46 连续 3 轮 PASS**（含"手机重启后首轮复跑"——覆盖此前的"重启后复现"条件）；12:15 轮次 **3 PASS / 0 FAIL / 0 SKIP**。
- 全量 4 变体（pro/fdroid × debug/release）构建通过；证据：`ci/autotest/results/20260913-121533_L1_ui-device/` 等轮次 report.md + evidence（含新修复的 fail_logcat 留痕能力）。

**内部佐证（辅助）**：
- `ExtUtils.isValidFile(Uri)` 对 remote:// URI 经 `uri.getPath()` 丢失 scheme 误判无效 → `openHorizontalView` 静默 return（真根因）；`AppDB.addRecent` / `AppData.getAllRecent` 两处 `new File().isFile()` 过滤掉远程书；`RemoteBookOpener` 串行 AsyncTask 改 `THREAD_POOL_EXECUTOR`；`RemoteBookSession.readAt` 静默短读改抛 IOException；`BlockCacheStore.open` 复用分支补 `data.bin` 长度校验；`ViewerActivityController` 取消门补 REMOTE 日志；`driver.py` save_logcat/失败分支/crash 扫描补全。

---

## [2026-09-13] L1 自动测试覆盖扩展到 48 例：新增 18 个用例 + 5 个遗留 SKIP 转 PASS + 用例禁用机制（纯测试侧，不改应用）

**做了什么（用户视角）**：
- **测试覆盖从 30 例扩展到 48 例**：新增 FN-31~48 共 18 个用例，覆盖抽屉菜单、收藏列表页、隐藏文件开关、阅读页锁定、对比度/亮度、蓝光滤镜、文件信息、标记已读未读、阅读偏好参数调节、阅读统计详情、在线阅读设置、WebDAV 同步定时器、设置备份导出、断网离线阅读、fdroid 门禁探针、退出确认等此前未覆盖的功能。当前 **41 PASS / 2 FAIL / 4 SKIP / 1 禁用，通过率约 87%**（不含禁用项）。
- **上一轮遗留的 5 个 SKIP 全部转 PASS**：
  - **FN-07 TTS 朗读 / FN-16 跳页**：真机+代码定位到根因——这两个入口（底栏「文字转语音」「前往页面」图标）只在水平翻页模式显示，垂直滚动模式永远隐藏；且偏好的模式切换只存在内存、应用重启即丢。测试改为「偏好→单机行为→左右翻页 → 不重启直接开书」绕行，零应用代码改动。若希望垂直滚动模式也有入口，应用侧只需一行补丁（DocumentWrapperUI.java:2100 处补 setVisibility）。
  - **FN-13 标签管理**：之前的用例找错了入口（长按书行弹的是书菜单/信息对话框，没有「添加标签」菜单项）。按真实入口重写：书行 ⋮ → 文件信息 → Add tags → 创建/勾选/应用，并核对标签落到 app-Tags2.json。
  - **FN-28/29 SMB/SFTP**：彻底解决 MIUI 密码框崩溃问题——采用「配置文件预置服务器行 + 一次性凭据入库」方案：首次由测试自动完成一次添加（密码经 adb 键入、按钮坐标预取后 adb 点击，全程绕开 uiautomator 与 MIUI 密码保险箱的冲突），密码由应用加密入库；之后每轮测试直接复用、零 UI 输入、秒级完成。
- **新增用例禁用机制**：cases.yaml 里给用例加 `enabled: false` 即可整体禁用（当前禁用 FN-25 OPDS，因其依赖外网），执行时自动过滤并留日志。
- **框架加固**（长跑稳定性）：用例开头自动唤醒屏幕（MIUI 长时间运行会熄屏，熄屏期间点击/截图全部失效）；u2 瞬态崩溃自动重试；对话框残留自动关闭；书库行定位改用列表视图+行内 ⋮ 菜单（书架视图的书名是图片无法定位）。

**对使用者的影响**：回归测试更全面（48 例覆盖功能列表的主要章节），网络协议用例从「每轮都可能卡在输密码」变为稳定复用；`--cases` 单用例调试与 `enabled` 禁用让维护更省事。所有改动仅在 `ci/autotest/` 测试侧，**不涉及应用代码，无需重新构建 APP**。

**验证**（MI9 真机 48fee174，1.3.2 Pro，2026-09-13 多轮）：
- 修复/新用例分批真机复跑：FN-07/13/16/26/28/29 全 PASS（FN-28 首次凭据入库 167s，复用轮 39s）；FN-31~48 中 13 PASS / 4 SKIP / 1 FAIL。
- 4 个 SKIP 均有明确原因：FN-34 锁定开关语义待勘探、FN-40/41 偏好入口文案待勘探、FN-47 为 fdroid 专属（pro 包按设计跳过）。
- **2 个 FAIL（FN-30/46，同因）**：远程书在线打开管线在尾部预读成功后**静默失败**（无后续请求、无提示、无崩溃）。已系统排除测试框架、设备状态（重启复现）、服务器因素（日志核实 206 正常）；同一 APK 昨天通过——**疑似应用侧间歇性回归，需开 IS_LOG 的构建定位**，非本轮测试改动引入。
- 证据：每轮 `ci/autotest/results/<时间戳>_L1_ui-device/report.md` + evidence/ 截图与 dump。

**后续建议**：
- **优先**：用开启 IS_LOG 的 debug 构建复现远程打开静默失败（我的文件→远程目录→点书，观察尾部预读 206 之后的走向）。
- FN-34/40/41 三个待勘探入口可结合偏好页源码定位后补齐。
- FN-47 需构建 fdroid 包后执行 `python run_all.py --level L1 --flavor fdroid --cases FN-47` 验证门禁。
- 应用侧可选优化：垂直滚动模式补跳页/TTS 入口（一行补丁，见 COVERAGE.md「关键真机结论」）。

---

## [2026-09-13] 鸿蒙批次2补齐：DOCX/RTF 文档打开（0.9.3）

**做了什么（用户视角）**：鸿蒙版现在**可以直接打开 .docx 和 .rtf 文档**了（此前打开后是 0 页空白）。打开时应用会在后台把文档转换成内部网页格式再阅读，转换结果有缓存——同一本书第二次打开几乎瞬时；阅读进度、书签始终记在**原始文件**上，多次打开不丢。旧版 .doc（Word 97-2003 二进制格式）暂不支持，点击会明确提示「请先另存为 .docx」而不是打开空白页。**从 WebDAV/SMB/SFTP 下载的 docx/rtf 在线书**同样支持：整本取回后自动转换打开。

**实现要点**（佐证）：
- 新增 `model/DocConvert.ets`：DOCX 用 `@ohos.zlib` 解压后扫描 `word/document.xml`（w:p/w:t/w:b/w:i/br/tab，XML 实体解码 + HTML 转义）；RTF 自写状态机（组栈、\par/\b/\i/\tab、\uN UTF-16 转义 + \ucN 回退跳过、\'hh 十六进制、\fonttbl/\colortbl/\info 等 15 种目标组整组丢弃）。产物为缓存 HTML（`<cacheDir>/converted/<名>_<大小>_<时间>.html`，键确定性 → 缓存复用且进度键稳定），由 MuPDF 现有 html handler 打开、支持字号/行距重排。
- 接线在 `Reader.initOrRemote`：docx/rtf 先转换（logicalPath 保原路径，进度/书签按原文件存取）；转换失败 toast 并退出，.doc 明确提示。
- 修复过程中发现并修正两处渲染问题：`<w:p>` 段落开标签缺失（首段无 <p>）、`<title>` 内容被 MuPDF 渲染进正文（改空 title）。

**验证**（Pura 90 模拟器 + 50.23 测试服务器）：WebDAV 下载 `book_docx.docx`（96KB 中文长文）→ 整本确认 → 自动转换 → **46 页正常阅读**（段落/中文/粗体正确，文档内嵌代码示例按字面显示符合预期）；自构造含中文 \uN 转义/粗斜体/特殊符号的 RTF → 静默取回 → 转换 → **1 页内容逐字正确**（「中文」「海泽」等 Unicode 转义全对）；转换缓存与源文件绑定（同文件秒开）。双变体 8 件套（07:03-07:06）构建通过并冒烟。已知边界：旧版 .doc 不支持（明确提示）；复杂样式（表格/图片/页眉页脚）不保留，纯文本阅读向。

---

## [2026-09-13] 鸿蒙批次1补齐：音量键翻页/缩放 + 点击分区动作扩充（0.9.3）

**做了什么（用户视角）**：鸿蒙版阅读器补上安卓版的**音量键操作**：阅读时按音量+/-键可以直接**翻上一页/下一页**（系统音量不会被误调），在偏好设置里可把它改成**缩放**（音量键放大/缩小页面）或**关闭**；开启自动滚动时音量键自动变为**调节滚动速度**（与安卓行为一致）。同时把阅读器「手势配置」面板的三个点击分区（左/中/右）动作从每区 3-4 个扩充为统一的 **8 种动作**（上页/下页/菜单/目录/书签/亮度/全屏/无操作），并修复了一个老问题：此前手势面板点「保存」其实**并没有真正保存**（重启后丢失），现在会持久化，重启后手势配置原样恢复。偏好设置的「阅读配置」组也新增了「音量键功能」一行，三档弹层选择。

**实现要点**（佐证）：
- 音量键走鸿蒙官方应用侧通路 `inputConsumer.on('keyPressed')`（API 16+，专用于音量键订阅），阅读器打开时注册、退出时注销（`Reader.ets` registerVolumeKeys/unregisterVolumeKeys）；根组件另挂 `onKeyEventDispatch` 兜底拦截。实测模拟器上翻页模式按音量键**不会**弹出系统音量条（消费配置生效）。
- 点击分区动作统一为 8 个动作码（0=上页 1=下页 2=菜单 3=目录 4=书签 5=亮度 6=全屏 7=无操作），`onTapZoneAction` 重写为纯动作分发；偏好里全局「单击动作」仍按安卓 readingMode 语义覆盖中区（0=面板默认/1=菜单/2=无操作）。
- 手势面板 chips 改 Flex 换行布局（8 个动作窄屏也完整可见）；`Settings.ets` 新增 `volumeKeyAction` 与 `tapLeft/Center/RightAction` 四个字段（字段级默认合并，旧数据兼容）。
- **顺手修正**：早期版本「单击动作=打开菜单」偏好实际会把中区设成"全屏"（历史错位），已按文案语义纠正。

**验证**（Pura 90 模拟器，HowRead-v0.9.3-x86_64 debug）：音量下/上键翻页方向正确且页码真实变化（1↔3）；缩放档 Zoom 1.50↔1.75x；关闭档音量键不再被应用消费；偏好三档切换跨重启持久化；手势面板 8 动作选中态保存后 force-stop 重启恢复（中区=亮度生效）；中心点击弹菜单、音量键翻页回归正常；双变体 8 件套（dist/debug+release × HowRead/HowRead-Pro × arm64/x86_64）构建通过并冒烟启动。待真机复验：实机音量键手感与自动滚动调速分支。
...
## [2026-09-13] 全量出厂态回归跑通：测试前置加固 + 测试脚本修复（应用 APK 零改动）

**背景**：按"每轮测试前重新安装 APK 并重置到默认状态"的要求，在 MI9 + KSA 两台真机上以 --reset（出厂态）模式跑全量 L1 回归（53 用例/台）。首轮暴露 10+ 个失败/跳过，逐一定位后确认几乎全部是**测试脚本自身**的问题，而非应用缺陷。

**做了什么**（全部在 ci/autotest/ 测试脚本层）：
1. **复位不彻底修复**：应用的全部设置其实持久化在外部存储 `HowRead/profile.HowRead/<机型>/app-State.json`，"清除数据"后下次启动会原样恢复旧配置——导致 KSA 上书库/浏览全找不到测试书（MI9 因旧配置恰好正确而掩盖问题）。--reset 现在一并删除该目录，应用真正回到出厂态。
2. **浏览页导航修复**：真出厂态的「我的文件」页没有文件列表、也没有 Download 文件夹行（只有一张默认书夹卡片，显示为"0"）；6 处"我的文件→Download→书"的测试导航统一改为先点书库文件夹卡片进根目录再进 Download。
3. **工具条误判修复**：脚本把阅读器页脚常显的页码行当成"工具条已显示"的依据，出厂态下工具条按钮行实际隐藏，导致 TTS/目录/跳页/字号/行距/亮度等整批用例跳过；现改为以工具条按钮自身元素判定。
4. **用例级修复**：TTS 对话框慢开误判（轮询确认+补点）；笔记用例长按时长不足（0.5s→1.5s，真机实测 0.5s 从不弹选词层）；出厂态开书停封面页（先翻页再选词）；统计详情点错卡片（"总阅读量"实为跳书库列表，改点"阅读时长"）；目录章节恰为当前章时页码不变的误判（换章重试）。
5. **状态自愈**：误点"锁定"按钮的锁定态会写盘并跨运行残留（禁用拖拽翻页），每轮开始自动复位；Pro 解锁标记在所有启动路径兜底确保。

**影响/效果**：全量结果从 73 通过/10 失败/27 跳过 提升到 **84 通过/11 失败/13 跳过**（两台合计 106 例执行），修复后单例复验 FN-07/22/46 再转 PASS（有效 87 通过）；剩余 11 失败均有明确归类（2 设备键盘限制、6 待勘探、3 已打补丁待复验）。所有结果对应真正"出厂态"的应用行为，可信度显著提高；应用 APK 零改动，无需重新出包。

**如何验证**：`python ci/autotest/run_all.py --level L1 --serial <serial> --reset`；报告在 `ci/autotest/results/<时间戳>/report.md`；坑点沉淀见 `ci/autotest/docs/PITFALLS.md` 26~30 条。

## [2026-09-13] 全量出厂态回归批次化修复收官：91 通过 / 5 失败 / 12 跳过（应用 APK 零改动）

**做了什么**（续当日早前条目,全部在 ci/autotest/ 测试脚本层）：
1. **远程服务器凭据录入修复**：EMUI 键盘弹出会顶起对话框,「添加」按钮的预取坐标失效（KSA 的 SMB/SFTP 一直录不进凭据）——现在收键盘后重新计算按钮位置再点。KSA SMB/SFTP 由失败转通过（190s 级,与 MI9 同速）。
2. **导航补强**：「我的文件」页在添加远程条目后大幅变高,书库文件夹卡片可能被挤出屏幕——查找前先上滑;收藏列表用例改为自置收藏状态（不再依赖前一用例残留）并点分区「更多」进完整列表;统计卡片在小屏上滚到完整可见再断言。
3. **判定方式修正**：目录跳转选章时跳过当前章（点当前章是无效操作）,跳转成功与否改用含章节名的指示器完整文本/顶栏章节副标题判定（每章章内页码都是"1/N",数字比较会漏判——截图证实跳转其实一直在成功）;笔记长按加长到 1.5 秒才触发选词;TTS 对话框慢开改轮询;锁定用例改行为式断言（锁定后拖拽应翻不动）。
4. **状态自愈**：每轮开始自动复位"锁定"残留、兜底确保 Pro 解锁,避免跨轮污染。

**效果**：全量出厂态回归最终 **91 通过 / 5 失败 / 12 跳过**（两台合计 106 例;当日历程 73/10/27 → 84/11/13 → 91/5/12）。剩余 5 个失败均有明确归类（锁定语义、位置行交互、KSA 晚场导航各 1-2 项,待下一轮勘探或应用侧确认）;12 个跳过里 4 个是设计使然、其余为偶发或已定位的待勘探项。

**如何验证**：`python ci/autotest/run_all.py --level L1 --serial <serial> --reset`;报告在 `ci/autotest/results/<时间戳>/report.md`;坑点沉淀见 `ci/autotest/docs/PITFALLS.md` 26~33 条。
