...
struct pdf_document
{
	...
	/* HowRead: per-page [w,h] sizes harvested at tree-walk time or injected
	 * from the app's persisted sidecar; page-size queries then run without
	 * loading each page object (remote books: one network fetch per page). */
	int *howread_sizes;
	int howread_sizes_count;
	/* HowRead: page-tree walk telemetry, filled by pdf_load_page_tree. */
	int64_t howread_walk_ms;
	int howread_walk_pages;
