...
/* HowRead: forward declaration — defined further down this file. */
void pdf_page_obj_transform(fz_context *ctx, pdf_obj *pageobj, fz_rect *page_cropbox, fz_matrix *page_ctm);

/* HowRead: monotonic wall-clock in ms (fz_now does not exist in 1.23.x). */
static int64_t howread_now_ms(void)
{
	struct timespec ts;
...
pdf_load_page_tree_imp(fz_context *ctx, pdf_document *doc, pdf_obj *node, int idx, pdf_cycle_list *cycle_up)
{
	...
	else if (pdf_name_eq(ctx, type, PDF_NAME(Page)))
	{
		...
		/* HowRead: harvest the page size while the page dict is already in
		 * hand — zero extra I/O; the app's page-size queries later run
		 * without loading any page object (remote: one fetch per page). */
		if (doc->howread_sizes && idx < doc->howread_sizes_count)
		{
			fz_rect cropbox, bound;
			fz_matrix ctm;
			float w, h;
			pdf_page_obj_transform(ctx, node, &cropbox, &ctm);
			bound = fz_transform_rect(cropbox, ctm);
			w = bound.x1 - bound.x0;
			h = bound.y1 - bound.y0;
			doc->howread_sizes[2 * idx] = (int)(w < 0 ? -w : w);
			doc->howread_sizes[2 * idx + 1] = (int)(h < 0 ? -h : h);
		}
		++idx;
	}
	else
...
pdf_drop_page_tree_internal(fz_context *ctx, pdf_document *doc)
{
	...
	fz_free(ctx, doc->howread_sizes);
	doc->howread_sizes = NULL;
	doc->howread_sizes_count = 0;
	doc->howread_walk_done = 0;
...
pdf_load_page_tree_internal(fz_context *ctx, pdf_document *doc)
{
	...
	int64_t howreadWalkT0 = howread_now_ms();
	...
	fz_try(ctx)
	{
		...
		doc->howread_sizes = Memento_label(fz_calloc(ctx, doc->map_page_count, 2 * sizeof(int)), "pdf_howread_sizes");
		doc->howread_sizes_count = doc->map_page_count;
		...
		doc->howread_walk_ms = howread_now_ms() - howreadWalkT0;
		doc->howread_walk_pages = doc->map_page_count;
		doc->howread_walk_done = 1;
	}
	fz_catch(ctx)
	{
...
/* HowRead: persisted page-tree map. */
int *
pdf_get_page_object_numbers(fz_context *ctx, pdf_document *doc, int *count)
{
	if (count)
		*count = 0;
	if (!doc || !doc->fwd_page_map || doc->map_page_count <= 0 || !doc->howread_walk_done)
		return NULL;
	int *out = fz_malloc_array(ctx, doc->map_page_count, int);
	for (int i = 0; i < doc->map_page_count; ++i)
		out[i] = (int)pdf_to_num(ctx, doc->fwd_page_map[i]);
	if (count)
		*count = doc->map_page_count;
	return out;
}

void
pdf_set_page_object_numbers(fz_context *ctx, pdf_document *doc, int n, const int *objnums)
{
	if (!doc || n <= 0 || doc->fwd_page_map)
		return; /* already loaded: nothing to inject */
	fz_try(ctx)
	{
		doc->map_page_count = n;
		doc->rev_page_map = fz_calloc(ctx, n, sizeof(pdf_rev_page_map));
		doc->fwd_page_map = fz_calloc(ctx, n, sizeof(pdf_obj *));
		for (int i = 0; i < n; ++i)
		{
			if (objnums[i] <= 0)
				fz_throw(ctx, FZ_ERROR_GENERIC, "bad page object number");
			doc->fwd_page_map[i] = pdf_new_indirect(ctx, doc, objnums[i], 0);
			doc->rev_page_map[i].page = i;
			doc->rev_page_map[i].object = objnums[i];
		}
...
/* HowRead: per-page sizes harvested at tree-walk time or injected from the
 * app's sidecar (pagemap.bin v2). */
int
pdf_get_page_size_at(fz_context *ctx, pdf_document *doc, int idx, int *out_w, int *out_h)
{
	(void)ctx;
	if (!doc || !doc->howread_sizes || !out_w || !out_h)
		return 0;
	if (idx < 0 || idx >= doc->howread_sizes_count)
		return 0;
	*out_w = doc->howread_sizes[2 * idx];
	*out_h = doc->howread_sizes[2 * idx + 1];
	return (*out_w > 0 && *out_h > 0);
}

int *
pdf_get_page_sizes(fz_context *ctx, pdf_document *doc, int *count)
{
	if (count)
		*count = 0;
	if (!doc || !doc->howread_sizes || doc->howread_sizes_count <= 0 || !doc->howread_walk_done)
		return NULL;
	{
		int i, n2 = doc->howread_sizes_count * 2;
		int *out = fz_malloc_array(ctx, n2, int);
		for (i = 0; i < n2; ++i)
			out[i] = doc->howread_sizes[i];
		if (count)
			*count = doc->howread_sizes_count;
		return out;
	}
}

int64_t
pdf_get_walk_ms(fz_context *ctx, pdf_document *doc)
{
	(void)ctx;
	if (!doc || doc->howread_walk_ms <= 0)
		return 0;
	return doc->howread_walk_ms;
}

void
pdf_set_page_sizes(fz_context *ctx, pdf_document *doc, int n, const int *wh)
{
	if (!doc || n <= 0 || !wh)
		return;
	if (doc->howread_sizes)
		return; /* already present */
	if (doc->map_page_count > 0 && n != doc->map_page_count)
		return; /* mismatch with the page count: ignore */
	{
		int i, n2 = n * 2;
		doc->howread_sizes = fz_malloc_array(ctx, n2, int);
		for (i = 0; i < n2; ++i)
			doc->howread_sizes[i] = wh[i];
		doc->howread_sizes_count = n;
	}
}
