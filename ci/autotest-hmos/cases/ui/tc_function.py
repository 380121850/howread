...
def _ensure_home(dev, cfg):
    ...
    # language guard: the app may be left in English by a prior language case;
    # the zh selectors below require Chinese UI
    if not dev.exists_text('首页') and dev.exists_text('Home'):
        ...
        for _ in range(8):
            ...
            time.sleep(1.0)
        dev.click_text('语言') or dev.click_text('Language')
        time.sleep(1.2)
        dev.click_text('中文') or dev.click_text('Chinese')
        time.sleep(1.0)
        dev.stop_app()
        dev.start_app()
        if not dev.wait_home(30) or not dev.exists_text('首页'):
...
def fn03_jump_page(dev, case_id, cfg, fixtures):
    ...
    with dev.step(case_id, 'jump dialog to page 50'):
        ...
        if not dev.wait_text('跳转到页', timeout=8):
        ...
        m = re.search(r'\[(\d+),(\d+)\]\[(\d+),(\d+)\]', a.get('bounds', ''))
        if not m:
            raise TestFail('jump input bounds missing')
        jx = (int(m.group(1)) + int(m.group(3))) // 2
        jy = (int(m.group(2)) + int(m.group(4))) // 2
        dev.click_xy(jx, jy)
        time.sleep(0.8)
        dev.shell('uitest uiInput text 50')
        time.sleep(0.8)
        a2 = dev.find(id_='reader_jump_input')
        if not a2 or '50' not in (a2.get('text', '') + a2.get('hint', '')):
            # typing didn't land — retry with coordinate inputText
            dev.click_xy(jx, jy)
            time.sleep(0.6)
            dev.shell('uitest uiInput inputText %d %d 50' % (jx, jy))
            time.sleep(0.8)
        dev.click_text('跳转') or dev.click_text('Go')
        ev = dev.wait_event('jump_page_done', timeout=10)
        if not ev or ev.get('kv', {}).get('page') != '50':
            dev.save_dump('jump_fail')
            raise TestFail('jump_page_done wrong: %s' % (ev and ev['raw']))
        if not dev.wait_text('50/105', timeout=8):
            raise TestFail('indicator did not show 50/105')
    _exit_reader(dev)
...
def fn04_toc(dev, case_id, cfg, fixtures):
    ...
    with dev.step(case_id, 'TOC panel opens'):
        ...
        if not dev.wait_text('目录', timeout=8):
...
def fn05_bookmark(dev, case_id, cfg, fixtures):
    ...
    with dev.step(case_id, 'add bookmark if entry exposed'):
        if dev.click_text('加入书签') or dev.click_text('添加') or dev.click_text('Add'):
            ev = dev.wait_event('bookmark_added', timeout=8)
            if not ev:
                raise TestFail('bookmark_added not fired')
        else:
            raise TestSkip('bookmark add button not exposed in panel (icons only)')
    _exit_reader(dev)
...
def fn07_night_mode(dev, case_id, cfg, fixtures):
    ...
    with dev.step(case_id, 'toggle night mode'):
        _open_book(dev, 'Alice', expect_pages=105)
        dev.click_xy(ICON_SET, TB)
        if not dev.wait_text('夜间', timeout=8):
            raise TestFail('settings panel (夜间) not shown')
        if not _click_toggle_near(dev, '夜间'):
            dev.click_text('关')  # text-button fallback
        time.sleep(1.0)
        dev.key('Back')  # close panel
        time.sleep(1.0)
    with dev.step(case_id, 'restore day mode'):
        dev.click_xy(ICON_SET, TB)
        if dev.wait_text('夜间', timeout=8):
            if not _click_toggle_near(dev, '夜间'):
                dev.click_text('开')
            time.sleep(0.8)
            dev.key('Back')
...
def fn09_library_search(dev, case_id, cfg, fixtures):
    ...
    with dev.step(case_id, 'full-text search across library'):
        dev.click_id('app_tab_1') or dev.click_text('书库')
        if not dev.wait_text('全文', timeout=8):
        ...
        time.sleep(1.0)
        dev.key('Back')  # dismiss keyboard / trigger search commit
        ev = dev.wait_event('lib_search_done', timeout=40)
...
def _fill_webdav_dialog(dev, ts, name='CI-WebDAV'):
    ...
    _input_field(dev, value=ts.get('user', ''), hint='Account')
    _input_field(dev, value=ts.get('password', ''), hint='Password')
...
def fn14_smb_test(dev, case_id, cfg, fixtures):
    ...
    with dev.step(case_id, 'SMB add + test'):
        ...
        if not dev.exists_text('testbooks'):
            ...
            _input_field(dev, value='CI-SMB', hint='显示名')
            _input_field(dev, value=ts.get('host', ''), hint='主机')
            _input_field(dev, value=ts.get('smb_share', 'testbooks'), hint='共享名')
            _input_field(dev, value=ts.get('user', ''), hint='账号')
            _input_field(dev, value=ts.get('password', ''), hint='密码')
...
def fn15_sftp_test(dev, case_id, cfg, fixtures):
    ...
    with dev.step(case_id, 'SFTP add + test'):
        ...
        _input_field(dev, value='CI-SFTP', hint='显示名')
        _input_field(dev, value=ts.get('host', ''), hint='主机')
        _input_field(dev, value=str(ts.get('sftp_port', 22)), hint='22')
        _input_field(dev, value=ts.get('user', ''), hint='账号')
        _input_field(dev, value=ts.get('password', ''), hint='密码')
        dev.click_text('测试连接') or dev.click_text('Test')
        if not dev.wait_text('连接成功', timeout=25):
...
def fn16_remote_open(dev, case_id, cfg, fixtures):
    ...
    with dev.step(case_id, 'open remote txt online'):
        ...
        dev.hilog_clear()
        if not dev.click_text('book_txt.txt'):
            raise TestFail('book_txt.txt not listed in panel')
...
def fn17_ai_config(dev, case_id, cfg, fixtures):
    ...
    with dev.step(case_id, 'configure AI to mock and test'):
        ...
        time.sleep(1.0)
        _input_field(dev, hint='http', value=url)
        _input_field(dev, hint='key', value='ci-test-key')
        _input_field(dev, hint='model', value=mock.get('model', 'howread-test-model'))
        dev.hilog_clear()
        if not (dev.click_text('测试') or dev.click_text('测试连接')):
...
def fn18_language(dev, case_id, cfg, fixtures):
    ...
    with dev.step(case_id, 'switch language to English'):
        ...
        ev = dev.wait_event('language_applied', timeout=8)
        if not ev:
    ...
    with dev.step(case_id, 'restore Chinese'):
        ...
        time.sleep(1.0)
        dev.click_text('中文') or dev.click_text('Chinese')
        dev.wait_event('language_applied', timeout=8)
        dev.stop_app()
        dev.start_app()
